...
**Touching the RT accumulation path → run `scripts/rt_noise_gate.py`.** Frame-to-frame |delta| per RT channel on a paused static scene, against committed ceilings; nightly on main via `trunk_health.py`, on demand for you. It exists because the 11x flicker cut of 2026-07-30 was won by measuring and nothing else stops it being given back. A channel that goes silent fails as inert rather than passing as calm — on origin/main, three of four back-to-back runs read every RT channel at zero while the composite still rendered (BUG-mw0x, intermittent all-zero RT channels), so "stable" and "dead" look identical without that check.
