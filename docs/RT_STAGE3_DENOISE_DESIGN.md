...
`POST_LUMA_SIGMA_SCALE` 2.0 (range 1–4; anchored to `ATROUS_LUMA_SIGMA_SCALE` = 8 acting on RAW variance — 16–50× the post-accumulation variance, so the honest equivalent is ≈1.1–2), `POST_LUMA_SIGMA_FLOOR` 0.02 (range 0.01–0.05), `POST_SPATIAL_GAIN` 2.0 (range 1–4, anchored to `ATROUS_REFL_VARIANCE_GAIN` = 2.0), `POST_EARLY_OUT` 0.004 ≈ 1 8-bit level (range 0.002–0.01). **The spatial term is damped by `min(2/n_eff, 1)`** (added at P4 review, 2026-08-28): spatial spread cannot tell Monte-Carlo noise from a real lighting edge, so it speaks only while temporal evidence is thin (cold start, reset, gesture snap); converged texels run on the pure temporal stop.

**The furnace oracle is pinned to denoise-Off** (both runtime constructors in `rt_furnace_oracle.rs`): its sun-disc energy bar was calibrated on a noise-inflated measurement — the unfiltered traced read carries the firefly sparkle (max 0.67) the filter exists to remove (filtered max 0.32), so the filtered pipeline "fails" the bar (29–42% vs ≥50%) precisely because it works. The furnace certifies the trace-side ED5 clamp and the accumulator, both upstream of the post-filter; the filter's own proofs live in `rt_atrous_post.rs`. Assertion 1 (firefly ceiling) is unaffected and still runs.
...
|---|---|---|---|---|---|
| `POST_LUMA_SIGMA_SCALE` | raytrace.rs `atrous_post` const | 2.0 | 1–4 | residual boil on noisy texels (wider luma tolerance → more averaging) | over-blur / soft GI gradients (narrower → less averaging) |
| `POST_LUMA_SIGMA_FLOOR` | same | 0.02 | 0.01–0.05 | boil on converged-but-noisy texels | flat-region detail loss |
| `POST_SPATIAL_GAIN` | same — damped by min(2/n_eff, 1) | 2.0 | 1–4 | post-cue/gesture raw-frame harshness (the cold-start term) | halo risk at true lighting edges through gestures |
