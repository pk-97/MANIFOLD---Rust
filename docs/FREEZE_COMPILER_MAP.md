...
| `// @derived_uniform_member: <first_field> words=<n> <type_id> [<camera_port>] [<member_port>=<fused_port> …]` (P0/D7, 2026-07-12; port pairs BUG-e3p6 (user-authored scene modifiers) P2, 2026-09-17) | fused texture/buffer codegen, one per region member with non-empty `derived_uniforms()` | The contiguous uniform-buffer block that member's derived fields occupy, and how to refresh it every frame: `wgsl_compute::evaluate()` calls `derived_uniform_registry::recompute(type_id, ctx)` (ctx = frame clock + the routed `@camera_external` value, if named + `array_len` for declared array ports) and packs the result through each field's real `UniformMemberType`. Trailing `<member_port>=<fused_port>` pairs map the array ports the recompute consults (`DerivedUniformRecompute::array_ports`) to their fused-kernel names — `src_<e>` for a wired external, the `count` sentinel for a region-internal register (its length IS the kernel's element count); absent ports (unwired optionals) map to nothing and degrade to length 0, matching `run()`. Fused kernels rename inputs to `src_<k>`, so a name-based `array_len` lookup misses without this mapping. Consulted by `introspect()` to exclude these fields from the generic port-shadow/param set. |
