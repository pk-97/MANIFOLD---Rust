...
**Pilot implementation:** `WaveGrid`, `WaveRing` and `WaveSpiral` are factory generators in `crates/manifold-renderer/assets/generator-presets/`, using the existing `.json` v2 format. They contain 1,000, 256 and 512 cube instances respectively. Play transport for beat-driven motion; Amount controls vertical displacement, Rate is cycles per beat, Frequency sets spatial repetition, Phase offsets the cycle, and Size changes copy scale. Camera Distance and Camera Tilt adjust the parked view. They use ordinary preset bindings, so existing modulation and persistence infrastructure applies.
