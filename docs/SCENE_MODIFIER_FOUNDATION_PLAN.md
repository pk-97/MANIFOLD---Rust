...
Buffer admission uses the same pure array-allocation plan as the GPU allocator, counting physical allocations once and aliases as shared storage. Each graph owns its admission policy, so callers cannot omit it. Modifier buffers have the 256 MiB per-scene limit; baseline buffers are reported separately. Five allocator tests and the focused native Metal fluidsim_buffer_fusion_renders_like_unfused proof pass. Runtime CPU tests cover watched/fused canonical loading, live copy writes and ray tracing enabled by the initial live manifest. These checks do not establish photoscan visual parity, live modulation, clip-event correctness or the full GPU landing gate.
