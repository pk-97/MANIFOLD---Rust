...
**Status:** DONE — P0–P3 ALL SHIPPED 2026-07-10; the wave is complete. Every main-window + editor render the harness touches now runs the app's real render path (seams extracted to `ui_frame.rs` / `editor_frame.rs`; byte-identical proof + Peter's live-app L4 pass). REFRAMED 2026-07-10 (Opus + Peter) — read the "Reframe 2026-07-10" block below before anything else; it supersedes D2, demotes D4a/section 5 (full-sequence replay), and restates P0. Honest gap: BUG-097 (ui-snap-render-overlay-pass-uses-wrong-traversal) — latent, pre-existing, tracked. Deferred (unchanged): monitor/output windows, CI wiring, Retina 2x, perform-mode surfaces, playback-stepped capture. Per-phase as-built record: the "As-built record" section at the end.
...
- **P0:** faithful full-app render + drawer filmstrip through the real `UICacheManager`; BUG-071 dump fix; feature-clippy debt BUG-057 (ui-snapshot-dead-blit-pipeline)/067/093 cleared.
- **P1:** the two seam functions (`apply_ui_frame_invalidations`, `composite_main_ui_frame`) extracted into `crates/manifold-app/src/ui_frame.rs`; the app and the P0 harness render through them; byte-identical proof (same `frame.png` sha256 from both the old atlas-readback path and the new composite path) + Peter's live-app L4 pass. As-built signatures differ from the section 4 (seam sketch) — the `ui_frame.rs` module doc records 5 deviations.
- **P2:** the script Runner's parallel rebuild logic DELETED and repointed at the seam (one update+composite path, three callers: live app, `cache_path_full_render`, script Runner); `render_ui_to_png`'s panel pass repointed; D9 captures shipped (Runner-level filmstrip + CPU-side pointer stamps); composite/save helpers deduplicated. Negative gate clean (no `needs_rebuild`/private `rebuild` in `script.rs`); both shipped ui-flows exit 0 through the seam. BUG-097 surfaced by P2's mandated audit.
- **P3:** the headless `editor` scene was itself a LOOKALIKE (3 scratch trees vs the live editor's one merged `render_tree_range`) — found + killed. `composite_editor_frame` + `build_editor_preview_column` extracted into `editor_frame.rs` (mirroring `ui_frame.rs`, 4 documented deviations); live editor and headless `editor` scene render IDENTICAL code; editor structural harness RED-tested; no `UICacheManager` in the editor path (D5).
- History: Opus (1M) authored 2026-07-09, Fable-reviewed (D8–D9 added), Peter approved for Sonnet execution; BUG-060 root-caused + closed out-of-band same day — the trigger for the reframe away from a BUG-060 differential.
