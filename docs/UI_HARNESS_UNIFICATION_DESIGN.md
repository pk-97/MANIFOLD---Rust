...
The one allowed variation is **input presence, never caller identity** (the `HARNESS_FIDELITY_INVARIANT_PROPOSAL.md` §3 caller test): a pass may skip itself when its *input* is `None` (no content-thread thumbnail atlas, `video: None`, no spectrogram columns) exactly as the live app skips it when its input is absent — but never on an `is_harness`/`headless` flag only the harness sets. A harness-only branch is the parallel fork reborn inside the seam. `VERIFY-AT-IMPL: does this pass match?` is retired as a gate: there is no parallel copy left to match. Built as `HARNESS_FIDELITY_INVARIANT_PROPOSAL.md` §4 step 2; closes BUG-097 by construction.
