...
**Status:** DONE — **P0–P3 ALL SHIPPED 2026-07-10; the wave is complete.** REFRAMED 2026-07-10 (Opus + Peter) — **read the "Reframe 2026-07-10" block immediately below before anything else; it supersedes D2, demotes D4a/section 5, and restates P0.** P3: the headless `editor` scene was itself a LOOKALIKE (3 separate scratch trees vs the live editor's one merged `render_tree_range`) — found + killed. Extracted `composite_editor_frame` + `build_editor_preview_column` into `crates/manifold-app/src/editor_frame.rs` (mirroring P1's `ui_frame.rs`, 4 documented deviations); the live editor and the headless `editor` scene now render the IDENTICAL code; editor structural harness (`editor_window_harness`, node-renders-at-declared-rect via internal color variance, RED-tested) passes; editor readback faithful (read by orchestrator, L2); no `UICacheManager` in the editor path (D5). Every main-window + editor render the harness touches is now the real path. Deferred (unchanged): monitor/output windows, CI wiring, Retina 2x, perform-mode surfaces, playback-stepped capture. **P0 + P1 SHIPPED 2026-07-10.** P0: faithful full-app render + drawer filmstrip through the real `UICacheManager`; BUG-071 dump fix; feature-clippy debt BUG-057 (ui-snapshot-dead-blit-pipeline)/067/093 cleared. P1: the two seam functions (`apply_ui_frame_invalidations`, `composite_main_ui_frame`) extracted into `crates/manifold-app/src/ui_frame.rs`; the app and the P0 harness now render through them; **byte-identical proof** (`frame.png` sha256 `5fe4ff45…`, reproduced from both the old atlas-readback path and the new composite path) + Peter's live-app L4 pass. As-built signatures differ from the section 4 sketch — see the `ui_frame.rs` module doc (5 documented deviations) and the section 4 note below. P2: the script Runner's parallel rebuild logic DELETED and repointed at the seam (one update+composite path, three callers: live app, `cache_path_full_render`, script Runner); `render_ui_to_png`'s panel pass repointed at the seam; D9 captures shipped (Runner-level filmstrip + CPU-side pointer stamps); the composite/save helpers deduplicated into `composite_resources.rs`/`render.rs`. Verified: negative gate clean (no `needs_rebuild`/private `rebuild` in `script.rs`), both shipped ui-flows exit 0 through the seam, pointer-stamp + filmstrip artifacts read by the orchestrator (L3). One honest gap: **BUG-097 (ui-snap-render-overlay-pass-uses-wrong-traversal)** (renumbered from BUG-094 — concurrent-session collision; pre-existing `render_ui_to_png` overlay pass uses `render_tree_range` vs the live app's `render_sub_region` — surfaced by P2's mandated audit, latent, tracked, not P2-introduced). **P3 remains** (editor window). History: Opus (1M) authored 2026-07-09, Fable-reviewed (D8–D9 added, section 4 `&mut`, section 5 full-sequence replay, BUG-071 pulled into P0), Peter approved for Sonnet execution; BUG-060 root-caused + closed out-of-band same day, which is what triggered the reframe away from a BUG-060 differential.

---

## Reframe 2026-07-10 (Opus + Peter) — general faithful-render harness; the red bracket is retired

**Read this before section 1. It changes the goal, not the machinery.** Peter's redirect: *"Don't worry about BUG-060 now — it's been patched. This upgrade is no longer BUG-060 focused."* And, on why the harness never needed a failing test to prove itself: *"Why do we need to watch it fail? We just need to watch it render the full app properly."*
...
- **D4(a) / section 5 differential atlas-band is demoted from reliability-critical to a shelf tool.** The byte-for-byte band comparison is no longer P0's centerpiece or any phase's gate. It stays documented and may be revived *only* if a specific stale-pixel regression is ever reported. Nothing in the wave asserts it.
...
Peter's directive, verbatim: *"I would want it to extend and generalise across all UI surfaces and windows."* And: *"I want to check that your proposal isn't hyper focused into just this one bug and is a general headless UI and Interaction harness."* Both are load-bearing and shape section 6 and the phasing.
...
| Graph-editor render (separate path) | `app_render.rs` `present_graph_editor_window` (3206) → `ui_offscreen` + `render_tree` (3311) | **No `UICacheManager`, no atlas, no `panel_valid`.** Recomposites from scratch every render. The stale-pixel class cannot exist here — decisive for section 6. |
...
Classification: the render path, the cache, the input driver, the fixtures, and the readback all **exist** — this is mostly wiring. **Genuinely new:** two extracted shared functions (section 4), a differential atlas-band assertion (section 5), a heavy fixture (section 7), and the discipline that proves faithfulness (D2). The design shrinks to those four.
...
**D3 hardening (2026-07-10, `HARNESS_FIDELITY_INVARIANT_PROPOSAL.md`, APPROVED).** P1 shared the composite but left the *immediate-mode pass assembly* (clips, names, lanes, overlays, VQT…) as a parallel harness copy — `draw_immediate_passes` — kept honest only by a `VERIFY-AT-IMPL: does this pass match?` audit. That audit is "by discipline," the exact thing this decision rejects, and it produced two lookalikes in one session (the editor 3-tree topology; BUG-097's overlay pass calling `render_tree_range` where the live app calls `render_sub_region`). The hardened invariant:

> **The harness contains no parallel render-pass assembly.** The full main-window offscreen pass sequence — composite + *every* immediate pass drawn into the offscreen — is a single shared seam the live app also calls. The harness calls it; it never re-sequences passes or re-chooses a per-pass render call. `draw_immediate_passes` and the harness overlay pass are deleted, not kept-behind-an-audit.

The one allowed variation is **input presence, never caller identity** (the `HARNESS_FIDELITY_INVARIANT_PROPOSAL.md` section 3 (Proposed invariant) caller test): a pass may skip itself when its *input* is `None` (no content-thread thumbnail atlas, `video: None`, no spectrogram columns) exactly as the live app skips it when its input is absent — but never on an `is_harness`/`headless` flag only the harness sets. A harness-only branch is the parallel fork reborn inside the seam. `VERIFY-AT-IMPL: does this pass match?` is retired as a gate: there is no parallel copy left to match. Built as `HARNESS_FIDELITY_INVARIANT_PROPOSAL.md` section 4 (Fix path (incremental, pattern already proven)) step 2; closes BUG-097 by construction.
...
**D5 — Unification is at the scaffolding layer, not the render path.** One harness *framework* — input driver, readback, assertion helpers, fixture system — that every window plugs into with its own render entry and its own invariants. Peter asked for it to "extend and generalise across all UI surfaces and windows"; the technical shape that honors that is shared scaffolding, because the windows do not share a render path: the main window is a stateful atlas cache, the graph editor is cacheless immediate-mode (`present_graph_editor_window` → `render_tree`, no `UICacheManager`; audit section 1). The stale-pixel class cannot exist in the editor, so asserting a cache invariant there is meaningless, and forcing one render abstraction would bury the real per-window behavior. **Dissent recorded (per DESIGN_AUTHORING section 9):** Peter's instinct is "unify everything"; my position is "unify the scaffolding, keep the render entries distinct" — the reason is the cacheless editor path, verified at `app_render.rs:3311`. Both positions are here so the choice is his at review. This delivers full cross-surface *coverage* without a false abstraction — and is a truer test, because each window is asserted against what it actually does.
...
**D7 — Fixtures are realistic, matching the repro conditions, not toy scenes.** A heavy generator fixture (multiple effects on a generator layer + dense modulation draws, per Peter's observed repro) plus the existing `project:<path>` real-project load. Rationale: Peter reports BUG-060 is content-sensitive — worst with many effects and heavy modulation while scrolling. A light scene may not reproduce it; a harness that passes only because its fixture is too small is fixture-overfitting (DESIGN_AUTHORING section 5). Rejected: *toy fixtures only* — risks a green harness on a bug that reproduces only under load.
...
Two functions, extracted verbatim from the blocks in section 3, called by both the live app and the harness. They live in a new module `crates/manifold-app/src/ui_frame.rs` (App-internal; no new crate boundary, no new dependency). The App owns them; the UI thread calls them; no thread residency change.
...
**AS-BUILT (P1 SHIPPED 2026-07-10) — the committed signatures above were a sketch; the real ones are in `crates/manifold-app/src/ui_frame.rs`'s module doc, with 5 forced deviations, each argued behavior-preserving and covered by the byte-identical proof:** (1) `apply_ui_frame_invalidations` takes `cache: Option<&mut UICacheManager>` — the invalidation block runs before GPU init when the cache is `None`. (2) both functions take `ui_root: &mut UIRoot` (real mutations: `build()`/`rebuild_scroll_panels()`/`clear_dirty_range()`). (3) `composite_main_ui_frame` also takes the App-owned `atlas_pipeline`/`atlas_sampler`/`blit_pipeline`/`blit_sampler` (cached, built once at GPU init — not reachable from the 6 sketch params, and recreating per-frame would break hot-path discipline), plus `scale_factor` and `video_source_dims`. (4) `apply_…` also clears `scrolled_in_place` (one-shot, like `needs_rebuild`). (5) `render_dirty_panels` now runs only on the non-fast-path branch (inside `composite_main_ui_frame`) — behavior-preserving because the atlas is blitted to the offscreen only on non-fast-path frames, so painting it on a cached-offscreen fast-path frame was already invisible. No new `Arc<Mutex>`/shared state; no per-frame allocation added.
...
| Readback + band comparison (section 5) | Invariants asserted: main = atlas-band differential + containment; editor = cacheless "recomposite is complete" checks; monitor = output-fit checks |
...
- **Read-back:** read section 3 and section 4 of this doc, `app_render.rs:950–965` and `2815–2852` and `3882–4064`. Restate the seam signatures, the may/must-escalate line, and that this is the live show's per-frame path.
- **Seam brief (per DESIGN_DOC_STANDARD section 6):**
  ...
  - *Misfit escalation:* if any signal the block reads is neither a `UiFrameSignals` field nor reachable from `ui_root` (the section 4 VERIFY-AT-IMPL), stop and ask — do not widen the struct silently.
...
3. The shared seam is in `manifold-app/src/ui_frame.rs`; the app and every harness call it. P1 shipped `apply_ui_frame_invalidations` + `composite_main_ui_frame`. **Hardened 2026-07-10 (`HARNESS_FIDELITY_INVARIANT_PROPOSAL.md`):** the seam owns the *full* main-window offscreen pass sequence (every immediate pass, not just the composite); the harness contains no parallel pass assembly; a pass may vary only on input presence, never caller identity. `draw_immediate_passes` and the harness overlay pass are deleted. (D3 + D3 hardening)
...
11. ~~The differential asserts "a skipped panel's pixels never change," re-baselined after every full clear; the P0 escalation branch fires only after the full section 5 repro sequence has been replayed. (section 5, amended 2026-07-09)~~ **RETIRED with D2/D4a by the Reframe 2026-07-10** — no differential gates any phase; the section 5 gesture sequence survives only as the motion the filmstrip captures.
12. `apply_ui_frame_invalidations` takes `&mut UiFrameSignals` — the block writes residual flags back (the drag-defer case keeps `needs_rebuild` set). (section 4, amended 2026-07-09)
...
- **`ui-snap diff` ignores rebuild-bookkeeping fields** (first-consumer finding, SCENE_BUILD P3, 2026-07-10) — `diff` currently compares the volatile per-node `gen` (generation) counter, so any interaction that triggers a full rebuild (a section fold, a collapse toggle, most `--interact`/`--script` clicks) bumps `gen` on every node and the diff reports all ~hundreds of nodes as changed — burying the real structural delta (rows added/removed, text/rect changes). This makes the verb unusable for its headline use case ("what changed across an interaction"). Fix shape: skip `gen` (and any pure rebuild-bookkeeping field) by default, or add `--ignore-gen`. Worked around in P3 by counting labels in each dump directly. Revive next time a UI phase needs a cross-rebuild structural diff.
- **Generator node-output pixels aren't headlessly provable through this harness** (first-consumer finding, SCENE_BUILD P5, 2026-07-10) — proving that "+ Object" actually renders a visible cube (not just that the graph gained the right nodes/wires) would need a real GPU frame of the generator, but `ui-snap graph`/`editor`/`gltfeditor*` explicitly skip generator node thumbnails ("rendering structure only" — `HEADLESS_UI_HARNESS.md`'s own `graph` row already documents this: "a single raw-executor frame can't produce correct generator output... Driving generators through `GeneratorRenderer` is the follow-up"). `GeneratorRenderer` (`crates/manifold-renderer/src/generator_renderer.rs`) exists and is already used by GPU-feature tests, so the plumbing to point it at a specific (possibly mutated) `EffectGraphDef` and dump one real frame is plausible, just not built. P5 proved the gesture at the CANVAS level (new group/light box + rewired ports, `render_graph_editor_to_png`'s production seam) and left the pixel-level proof as this open item. Revive when a bug or feature needs headless proof of actual generator pixel output, not just graph structure — same trigger as the `graph` scene's existing thumbnail gap, now with a second consumer.
