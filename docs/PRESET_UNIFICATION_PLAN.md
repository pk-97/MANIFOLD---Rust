...
**Status:** SUBSTANTIALLY COMPLETE — 9 of 10 worklist steps landed 2026-06-05/06 (one commit per step; the 7th prior attempt, structured to land one fork at a time to beat the "too big to hold" failure mode). The unification's substance is DONE: one `PresetDef`/`PresetKind`, the catalog disk-loaded as the single source (no compiled shadow — the range-shadow bug class is structurally gone), one `PresetContext`, unified binding storage (graph-native single list + v1.4.0 save migration), and the graph host / runtime apply-loop verified already-shared from prior work. Step 10 (live hot-reload) is the only item not landed — it trips the no-new-shared-state hard rule and is gated on Peter's approval (see Step 10 below). Capability-gap FEATURES (skip-mode on generators, string-bindings on effects, etc.) are documented as enabled follow-up, not forks. The 2026-06-05 audit corrected a stale earlier status (it had claimed the param-storage keystone was deferred when it was already done) — that stale map is why six prior attempts misfired.
