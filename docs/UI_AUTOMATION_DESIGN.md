...
**Execution contract:** read `docs/DESIGN_DOC_STANDARD.md` section 5 (Phase briefs)–section 6 and section 8 before starting any phase. P1–P2 are full-hardened against today's code; P3–P4 carry pre-flight re-derivation commands.
...
| Request/reply channel shape | `docs/MCP_INTERFACE_DESIGN.md` section 3 (`McpRequest { kind, reply }`) | DESIGNED, not built. section 4 reuses the *shape* (per-request bounded(1) reply channel), not the crate. |
