...
**Status:** SHIPPED — P1–P7 on main 2026-08-21 (P7: clip-topology chain enumeration, D17/D18). Corrosion probe cold touches during playback: 70 → 0. BUG-p8oe (warmup-p5-residue-cold-touches) closed · k3 (lead)
**Prerequisites:** none
...
- **Gate:** the Corrosion probe is the acceptance — `cargo build -p manifold-app --features perf-soak && RUST_LOG=info ./target/debug/manifold rt-capture '<Corrosion path>' --frames 360` reports `cold touches during playback: 0` (a named, explained residue is an escalation, not a pass); existing warmup tests stay green; landing_gate green.
...
- **Result (2026-08-21):** D13–D15 landed; D16 descoped as above. The probe read 55 post-P6 — but 49 of those were a counter bug: the cold-touch detector armed at warmup START, so the warm's own expected first-touches counted against playback. Detector now resets at warmup END (residue lane, same day). True residue: 6 pipeline compiles in the first ~60 playback frames — 5 `render_scene` RT pipelines keyed on scene topology (D11 pre-rolls each layer's FIRST clip only; later clips' scenes compile on stage) + 1 `value_overlay`. Fix shape: extend pre-roll coverage beyond first clip (unique-scene-topology walk or keyframe sampling) — a warmup-contract change, candidate P7. Tracked in BUG-p8oe (warmup-p5-residue-cold-touches).
