...
- **Authoring = single-source:** each atom's math is written once as a `wgsl_body` fragment; its standalone `cs_main` is **generated** from that body, oracle-validated per-atom against the existing production shader, then the hand shader is deleted. One source of truth.
...
2. **Step 1b (gated per-atom cutover, NOT inert):** for each atom — generate its standalone `cs_main` from the body, oracle-validate vs the existing shader (tight tol: both single-kernel, near-bit-exact), rewrite `run()` to consume the generated kernel, delete the hand shader. **Strictly per-atom**; the load-bearing gate is the **full workspace parity sweep + `bundled_presets`** (deleting these 7 shaders re-shaders ~18 presets) green for THAT atom before moving on.
