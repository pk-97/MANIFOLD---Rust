...
**Status:** v1 LANDED 2026-06-03 (section 12.3 steps 0–5). **`docs/FREEZE_COMPILER_MAP.md` is the authoritative current-state map; this doc's body sections are partly superseded — read the MAP first.** Originally drafted 2026-06-02 on branch `freeze-compiler`. Companion to `GRAPH_FREEZE_PHASE0_FINDINGS.md` (the measurements this design is aimed at). This is the keystone design — the `wgsl_body` calling convention (section 4 (The cut rules — when fusion says no)) and the region model (section 3 (The pipeline, end to end)) are the decisions to sign off before implementation rollout.

Decisions here are proposals; nothing is rolled out across the atom library until reviewed.

**⚠ ADVERSARIAL REVIEW COMPLETE (2026-06-02) — read section 11 before building.** An 8-dimension adversarial review (5 dimensions returned structured findings; 3 — codegen, freeze-specialization, debuggability — failed to emit structured output and should be re-reviewed) found **6 blockers + 17 majors**, all grounded in code. The spine held (WGSL-out + `Backend` trait + spirv-opt structural fusion; the unfused-oracle framing; ColorGrade as first target). But several body sections are now superseded by section 11 — specifically: section 4 (the `wgsl_body` convention was under-specified), section 7 (the oracle tolerance is unsound as written), section 0 pillar 2 / section 5 / section 9 (Vulkan barriers are NOT free — explicit build item), and section 4b/section 10 (the buffer-domain win is unmeasured and partly mis-targeted). **section 11 catalogs every finding with its resolution. Do NOT build the harness or compiler until the section 11-flagged keystone decisions are signed off.**
...
1. **Correctness is gated, not hoped.** Every fused output is verified against the unfused oracle (section 7). Nothing ships unverified. A fused kernel that can't be proven equivalent falls back to unfused.
...
4. **Automatic, invisible, no new authoring surface.** No "freeze button" (section 6). Fusion + specialization are driven by graph structure + expose-state, compiled in the background, cached. The author composes atoms as today.
...
- **Per-chain (LTO)** — the runtime already splices a layer's effect rack into one `ChainGraph` and rebuilds on rack edits; the fusion pass runs over that unified graph, fusing **across** card boundaries. Cards must therefore stay optimized *graphs*, never opaque kernels (section 6).
...
- **Buffer element:** `fn body(e: Elem, index: u32, /*params*/) -> Elem` where `Elem` is the channel-struct emitted from the wire's `_SPECS` (section 5).
...
4. **Buffer intermediate types** come from the channel system: read the wire's `_SPECS`, emit `struct Elem { … }` with std430 offsets mechanically (CHANNEL_TYPE_SYSTEM.md section 16.3.1). Dead-channel elimination: load/store only the channels the region touches (section 16.3.2). Pure rename/reorder atoms erase (section 16.3.4).
...
2. **Verification harness** (section 7) first — the oracle gates everything after.
3. **`wgsl_body` convention** (section 4) + convert a first batch of texture-pointwise atoms; ship inert (atoms still run normally, now also expose a body).
...
**Deferred for review before rollout:** the broad atom conversion, the generic multi-domain compiler, and chain LTO — all ride on the section 4 convention + section 3 region model being signed off.
...
- **[MAJOR] `barrier_analysis` doesn't exist; `pipeline_barrier` has zero callers; the RAW read/write sets aren't materialized** (lifetime analysis tracks slots, not barrier transitions). **CORRECTION:** reclassify section 0-pillar-2/section 9 "drops in with zero compiler changes" → a scoped build item over both texture and buffer resources.
...
- **[MAJOR] Add resolution + canvas-scale to the fuzz axes; use real multi-res presets (Bloom, OilyFluid, FluidSim) as fixtures** — to catch the section 11.A resolution-seam class, which a single-canvas harness never reproduces. **DECIDED.**
...
- **[MINOR] Dead-channel DCE is harder than section 5.4 implies** — the particle shaders are hand-written structs (full load/store), not Channels-typed (`channel_get`). **FLAGGED:** either gate buffer dead-channel on a Channels migration, or measure whether spirv-opt's existing load/store-elim already drops untouched-field stores.

### 11.F Net assessment & gate
The spine is sound. Before building: revise section 4 (convention) and section 7 (oracle) per the above and **sign them off** (keystone decisions), make the barrier API kind-complete (section 11.C), and **measure the buffer break-even** (section 11.E). Re-review the 3 dimensions that didn't return (codegen, freeze-specialization, debuggability) — **done in section 11.G**. The texture-pointwise ColorGrade path is the lowest-risk first build once section 4/section 7 are settled; the buffer domain is gated on measurement + the register cost-model + the kind-complete barriers.
...
**Codegen (section 5).** Grounded against `manifold-gpu/src/shader_common.rs` (the real spirv-opt pass list: `InlineExhaustive`, `ConditionalConstantPropagation`, `AggressiveDCE`, `EliminateDeadConstant`/`Members`, `Local{Single,Multi}StoreElim`).
- **[MAJOR] section 5 contradicts section 6 on baked params.** section 5.3 merges *all* params into one uniform struct; section 6 bakes unexposed params for CCP+DCE to fold. But a uniform read is **not** a compile-time constant — `ConditionalConstantPropagation` cannot fold it, `AggressiveDCE` cannot drop the branch it guards. **DECIDED:** codegen splits the param set — **exposed → uniform field; baked (unexposed) → emitted as a WGSL `const`/literal** (or pipeline-override constant) so CCP actually folds and DCE actually prunes. The "merge into one uniform" applies to the *exposed* subset only. (This is why fusion-only step 4, where every param stays a live uniform, gets structural fusion but NOT the constant-fold/branch-prune win — that win arrives with baking in step 5.)
- **[MAJOR] Symbol collision on mechanical body-chaining.** Two atoms (or two instances of one atom) that each define a helper `fn hash(...)` / a private struct collide when concatenated into one module. **DECIDED:** namespace every atom's body + its private helpers with a per-node unique prefix at codegen; this also makes codegen **deterministic** (stable prefixes), which the cross-session pipeline cache (section 6) needs for hits.
...
- **[MINOR] Gather read path.** section 5.2 "one input read" is stale w.r.t. section 11.B: a region containing a gather has two read paths — codegen-supplied register values (pointwise/coincident) AND a declared sampled-texture+sampler arg (gather, exact filter/address-mode preserved). Cross-ref section 11.B; no new issue.
...
- **[MAJOR] Async hot-swap must not introduce shared mutable state.** section 6's "swap the fused kernel when ready" cannot be a shared pipeline cell (`no_new_shared_state` hard rule). **DECIDED:** the worker posts a compiled artifact; the **content thread installs it between frames** via the existing command/state path — the render thread only ever reads an immutable snapshot. Spec this as a content-thread state transition, not an `Arc<Mutex<Pipeline>>`.
...
**Debuggability (section 8).** No blockers — the retained-unfused-graph architecture (it's the oracle AND the immediate-run path AND the fallback) makes this dimension structurally sound. Gaps are additive:
...
- **[MINOR] Dump must reach past WGSL.** section 8 dumps generated WGSL, but the inline+fold (and the section 11.C f16/portability bugs) happen at SPIR-V/MSL lowering. The debug artifact should optionally include post-spirv-opt disassembly, not just the WGSL.
