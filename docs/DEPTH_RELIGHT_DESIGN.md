...
**D8 — Relight fuses; the P5 fusion veto is a stopgap, not the contract.** (Added 2026-07-18, from the perf report: one relight-on card un-fuses its whole chain segment — N dispatches where the chain normally costs ~1, a step-function frame cost invisible to the tick-rate FPS readout.) The fix is ordering, not new machinery: `relight_augment` is already a pure `EffectGraphDef → EffectGraphDef` transform, and every template atom is fusable (`Pointwise`; `ssao_gtao`/`heightfield_shadow` gather via `GatherTexel`, covered by D6's fp32 rule inside fused regions). Run the augment *before* the fusion compiler — on `effective_def` ahead of `fused_view_for` (single-card path, `preset_runtime.rs` ~1320) and per-member ahead of segment folding (`classify_segment_member` ~737) — then delete both vetoes. The fused kernel then *contains* the relight template instead of being opaque to it, which removes the original reason for the veto.

Two sub-decisions make the toggle knob-safe:
- **(a) Knob-invariant cache key.** `relight_augment` currently bakes live knob values into the minted nodes' params (`relight.rs` ~255–329); fusing that def naively would recompile a kernel on every Light-X drag — a mid-performance hitch, worse than the unfused cost. Instead: augment with `RelightParams::default()` so the def content (and therefore the fused-view cache key and generated WGSL) is knob-invariant, and write the live values into the fused node's seeded uniform params at splice/dispatch time — the `rl_<handle>` ids are already deterministic, so the knob→param mapping is static. Relight fields are live-driven by definition; value-baked kernels are forbidden globally (COMPILE_CONTRACT_DESIGN D2 — the `@static_param` mechanism itself is deleted, P3). Consequence for the card UI: knob edits become param writes, not structural rebuilds — the P5b `DispatchResult::structural()` per-drag-tick cost disappears as a side effect.
- **(b) `height_from` may recompile.** The Height From enum changes template topology (different height tap), so switching it legitimately compiles a new variant — same class as any topology edit, editor-time, acceptable. The toggle itself on/off likewise recompiles (already true today).
...
| **P4** ✅ LANDED @ f2684402 | D6(a) fp32 intermediate allocation + D6(b) BUG-216 copy fallback | — | clippy; GPU suite; BUG-216 repro (feedback→final_output) accumulates trails; update BUG-216 Status |
| **P5** ✅ LANDED (P5 backend on `lane/depth-relight-p5`, P5b card UI on `lane/depth-relight-p5b`, both not yet merged to main) | Card toggle + params surface (D2/D3/D4), EditingService command path | P3 | clippy; focused tests; ui-snap `editor`/`inspector` render eyeballed |
...
| **P7** ✅ BUILT 2026-07-18 (`lane/depth-relight-p7-impl`; K3 first pass + Fable debug/finish) | Fuse the relight path per D8: augment-before-fusion on both paths, both vetoes deleted, knob-invariant cache key + live-uniform knob writes, knob edits no longer structural. Plus two compiler additions the parity gate forced: `@input_access`/`@precision_critical` markers so a fused kernel reports its members' texture-access + fp32 needs (without them the executor's mixed-consumer rule vetoed the D6(a) fp32 height promotion). **The parity gate caught BUG-253 (shininess-and-tone-map-uniform-order-drift-weird…)** — `node.shininess`/`node.tone_map` hand uniform structs disagreed with the generated kernel layout (the production "weird tints"); fixed in the same lane. | P5 | ALL GREEN 2026-07-18: clippy; fused-vs-unfused parity on Mirror+ColorGrade at strict tolerances; toggle-off golden; knob-drag cache-HIT test; mixed segment fuses both template regions with `rl_` nodes present (the original "ONE fused region" wording was wrong — the template's blur/GTAO stages are legitimate cut points); WGSL snapshot golden regenerated (intentional marker change); workspace 3765 green; full GPU suite green (lib 1892 + gpu_proofs 63 + conformance) |

P1/P2/P4 are independent — dispatched in parallel. P3 is the core and follows P1+P2. Lanes land per the merge-trunk protocol; batch landings per 2–3 phases. P7 can run before or alongside P6, but P6's perf-soak should be re-run (or run once) after P7 lands — the soak numbers with the veto in place would be obsolete the day P7 ships.
