...
**Status:** APPROVED 2026-07-17 (Peter, in-session, after the rendered probe sweep) — EXECUTING; P1–P4 ON MAIN as of 2026-07-18. P1 `node.heightfield_shadow` + P2 `depth_rule` on all registered nodes (landed `b6a43be7`). P3 `relight_augment` (`node_graph/relight.rs`) + the `relight: bool` compile-level toggle through `splice_def_into_chain` / `Generators::create_with_override` (off = byte-identical, golden test in `relight.rs`; toggle-on proven by rendering Caustics + Watercolor via `graph_tool render --relight`). P4 fp32 precision-critical intermediates (incl. texel-exact conversion of `surface_bumps`/`edge_slope`) + BUG-216 copy fallback (FIXED). P5 BACKEND LANDED on `lane/depth-relight-p5` (not yet merged to main): `PresetInstance.relight` + `relight_params: RelightParams` (D3's six knobs + D4's Height From), `ToggleRelightCommand`/`SetRelightParamCommand`/`SetRelightHeightFromCommand` in `manifold-editing` addressed via `GraphTarget` (works for both effect and generator cards), and the real render path (`splice_def_into_chain`, `GeneratorRegistry::create_with_override`, the effect topology hash, the generator rebuild sweep) all read the live per-instance state instead of a hardcoded `false` — proven end to end by a gpu-proofs test that toggles the field and confirms `rl_` nodes appear/disappear on rebuild. Two P3 correctness gaps surfaced and got fixed in the same lane: the template's node ids were a random UUID per rebuild (any card binding would have dangled) — now deterministic (`rl_<handle>`); and both the single-card and segment fusion paths could silently fuse a relight-on card into an opaque kernel with no topology to splice onto — both now veto fusion while relight is on. **P5b CARD UI LANDED on `lane/depth-relight-p5b`** (not yet merged to main): a "3D" header icon on every effect and generator card (same unconditional pattern as the ON/OFF toggle, sitting between it and the cog), plus the six D3 knob rows (Light X/Y, Relief, AO Intensity, Shadow Softness, Gain) and the D4 Height From segmented row, always drawn below the normal params and greyed (not hidden) when the toggle is off — proven end to end via `ui-snap inspector` (Mirror/Bloom effect cards) and `ui-snap editor --preset Caustics` (generator card). Every relight edit bumps `bump_graph_structure_version` and the app dispatch returns `DispatchResult::structural()`, since the D3 template is spliced in at compile time rather than read per-frame — a live drag re-splices every tick, a known perf cost P6's soak needs to measure. P6 (acceptance sweep + perf gate) still remains, and now has a user-reachable toggle to test.
