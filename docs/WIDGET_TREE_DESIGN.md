...
**Status: PROPOSED — design complete, awaiting Peter's review · 2026-07-20 · Fable**
**Prerequisites:** W2-B (BUG-265 root fix — tree-bounds hit-testing; runs in the parallel Wave-2 session) before P3 of this design. SCENE_PANEL_EXPOSURE_CONVERGENCE P2 (deletes the scene funnels) before P5. UI_WIDGET_UNIFICATION (SHIPPED) and INPUT_IDENTITY_UNIFICATION (SHIPPED, archived) are the groundwork this rides.
...
| Transcription #4: per-family bridge dispatch | `ui_bridge/mod.rs` `dispatch` :158 (18 args); `ui_bridge/inspector.rs` `resolve_graph_target` :224, dual-edit twins :62–223; scene funnels `resolve_scene_write` :256, `scene_bound_slot` :297, `resolve_mod_target` :401 | Scene funnels die via convergence P2 (not this design). Param actions already carry `(GraphParamTarget, ParamId)` — the bridge keeps one generic handler per action kind (P4 consolidates the effect/generator twins). 18-arg `dispatch` signature: Deferred, see §10. |
| Geometry snapshots (BUG-265's disease) | `param_card.rs` `card_y` :1197 (written only at build :2213/:2977), `compute_height` :1611 (+ effect/generator twins :1618/:1690); consumed by `panels/inspector.rs` `update_card_drag` :1692–1708 | DIES (P3, after W2-B lands tree-bounds hit-testing). The live tree is already the only scroll-current source. |
...
| Intent-at-build | `IntentRegistry<A>` (generic since overhaul Phase 6); `param_card.rs` `register_intents` :4665 (partial) | EXISTS. Clicks that need no card state ride it; stateful gestures use the row index (§4). |
...
| INV-3 The tree is the only geometry source | `rg 'card_y|compute_height' crates/manifold-ui/src/panels` → 0 (P3 gate); geometry tests compute drop targets from tree bounds under scroll + mid-tween states |
| INV-4 Row widget identity survives reorder/insert/fold | `param_card` keyed-identity test shaped like `tree.rs:1747` (P2 deliverable) |
| INV-5 No row-element id hoards | `rg 'Vec<Option<NodeId>>' crates/manifold-ui/src/panels/param_card.rs` → 0 (P2 gate) |
| INV-6 Per-frame value stream matches rows | `debug_assert!(slots.len() == rows.len())` in `sync_values` (P1) |
...
### P1 — Model swap: `ParamSurface` in, `ParamCardConfig` dead (vertical slice)

- **Entry:** `rg -n 'struct ParamCardConfig' crates/manifold-ui/src/panels/param_card.rs` hits ~:305; `rg -n 'fn preset_to_config' crates/manifold-app/src/ui_bridge/state_sync.rs` hits ~:3092; re-derive both (Wave-2 lands in parallel — if `inspector.rs`/`state_sync.rs` moved, re-anchor before touching).
- **Read-back:** this doc §2–§3, `param_card.rs:305–520` + `:1932–2000`, `state_sync.rs:3043–3299`, PARAM_STORAGE_BOUNDARIES D4/D12 section. Restate D1/D2/D3/D7, the forbidden moves, entry findings.
- **Deliverables:** `param_surface.rs` types; `param_surface()` projection (absorbing `preset_to_config`/`build_card_modulation`/`build_audio_card_state`); `configure()`/`build()`/`sync_values()` re-pointed at rows (mechanical field-source rewrite, bodies otherwise untouched); editor card assembly re-pointed; `ParamCardConfig`+`ParamInfo` deleted (compiler-driven); INV-1/INV-6/INV-7 checks.
- **Gate (positive):** existing card/golden tests green untouched; `rows_mirror_manifest` green incl. Liveschool fixture; both shipped ui-flows exit 0; full-app + editor headless PNGs produced and read (unchanged layout). **Gate (negative):** `rg 'ParamCardConfig|ParamInfo\b|driver_active|trim_min|id_to_index' crates/` → only history/docs hits.
- **Acceptance demo:** the two PNGs + flow exit codes. **L3** (flows drive the real input path). **Performer gesture:** scrub a layer-effect slider — value follows, one undo entry (the `undo_baseline` suite must stay green untouched — it is the behavior contract for this swap).
...
- **Deliverables:** keyed row-widget minting (`ParamId`-derived salts); `RowIndex`; `row_action()`; `handle_click`/`handle_pointer_down`/`handle_drag` re-pointed; deletion of the gauntlets + hoards + `pid_at`; INV-2/INV-4/INV-5 checks; the click→command dispatch test family seeded (one test per `RowRole` variant, `Harness` pattern) — this is W2-A's pattern library, coordinate so tests live once.
...
- **Entry:** W2-B (BUG-265) landed — verify `git log --grep BUG-265` shows the root fix; if not landed, STOP and coordinate (this phase builds on tree-bounds hit-testing, never re-implements it).
- **Read-back:** INSPECTOR_DRAG_TAB_FINDINGS (all three root causes), W2-B's landed diff. Restate D6.
- **Deliverables:** `card_y`/`compute_height`(+twins) deleted; every remaining consumer re-pointed at laid bounds; geometry test family (drop-target math under scroll offsets and mid-tween heights, pure).
- **Gate:** INV-3 rg → 0; geometry tests green; drag flow (the `drag-clip.json` pattern, card variant) exits 0. **Acceptance demo:** the L3 drag flow. **Performer gesture:** scroll the inspector, then drag a card — the indicator lands where the cursor is (BUG-265's exact complaint).
- **Forbidden moves:** updating snapshots instead of deleting them (the findings doc's named stopgap) · asserting pixels for geometry.
...
- **Wave 2 (parallel session):** W2-B (now sequenced first) is P3's prerequisite. W2-A test families — P2 seeds the same patterns; whoever lands second rebases onto the other's test module, no duplicate harness. W2-C's drag survey findings may extend the geometry test family's cases.
...
4. Row widgets are keyed by param-derived salts; identity survives reorder. (D4)
