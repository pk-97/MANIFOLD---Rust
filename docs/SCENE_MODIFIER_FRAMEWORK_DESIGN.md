...
**Successor implementation underway (2026-09-12, not landed):** [Mathematical Scene Modifier Programme](SCENE_MODIFIER_PROGRAMME.md) and [Preset Architecture](SCENE_MODIFIER_PRESET_ARCHITECTURE.md) define file-authored, repeated modifier instances and ordered expansion. The [foundation plan](SCENE_MODIFIER_FOUNDATION_PLAN.md) migrates Loop/Fog plus Elastic Sculpture, Surface Peel and Vortex Fragments, then deletes runtime descriptors. This document's derived-list/Rust-descriptor rules remain current until that migration lands; geometry, saved control identities and compatibility invariants must be preserved. Do not add new per-look registrations as the long-term authoring mechanism.
...
- **First-class coupled-row concept in the descriptor contract** (P4 finding) — trigger: a third kind wants coupled rows. P4's `coupled_writes` decl + app-side resolution is the minimal patch, but a coupled control is the one place "a new control = ParamDef + whitelist line" stopped being literally true, and the modulation-write surfaces (OSC/Ableton/MIDI) bypass the coupling today (BUG-6dh6 (modifier-coupled-rows-bypassed-by-modulation-writes) — the fix likely moves the expansion into the command so every write path inherits). A plan-level "coupled row" in the descriptor, expanded at the command layer, removes both the per-app special-casing and the bypass class.
- **Descriptor-level range-transform hook** (P4 finding) — trigger: a second kind curates a stamped range from scene data. Spacing's auto×0.25..4.0 curation is factored into one fn but called from two sites (plan builder + load migration); a descriptor hook would make it declarative.
