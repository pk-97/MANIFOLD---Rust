...
Status: **v1 SHIPPED 2026-06-17** (section 11 steps 1–5 ✅: modulation plumbing, drawer UI, feature×band matrix, spectrogram overlays). Step 6 (onset) shipped as the SuperFlux per-band transients detector. **Step 7 (pitch/ridge tracking) is now designed: [AUDIO_OBJECT_TRACKING_DESIGN.md](AUDIO_OBJECT_TRACKING_DESIGN.md) (2026-07-06)** — it inherits section 6 (Plausible-wrong architectures, forbidden by name)'s commitments; note its D2 supersedes section 6's synchrosqueezing choice (harmonic-sum salience on the shared VQT column instead, synchro kept as a named precision fallback). Known stale labeled question: the section 12 device-selection default was due "before step 2 ships" — step 2 shipped without closing it in-doc.
