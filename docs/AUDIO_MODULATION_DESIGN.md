...
5. **UI.** First the **drawer API** (§10.2), then the per-slider audio drawer as a `DrawerSpec`, then the Audio Setup panel (§10.1).
   ...
   - ⏳ **Audio Setup panel** (§10.1) — remaining. Adds input-device selection (capture currently uses the system default) and per-send multi-channel routing / labels / per-send analysis toggles. Until it lands, sends created from the drawer's ＋ default to channel 0 on the default device, so single-source modulation works but multi-source separation (kick vs bass on different channels) needs this panel.
