...
5. **UI.** First the **drawer API** (§10.2), then the per-slider audio drawer as a `DrawerSpec`, then the Audio Setup panel (§10.1).
   - ✅ **Drawer API** — built + unit-tested (`panels/drawer.rs`).
   - ✅ **Per-slider audio drawer** — the "A" button + drawer (send selector with ＋new, feature selector) on every effect/generator slider, wired through `PanelAction` → `ui_bridge` → the audio-mod EditingService commands. Functional end-to-end on the default input device. (Migration of the existing driver/envelope/Ableton drawers onto the API is a deferred cleanup — they remain bespoke for now; the audio drawer uses the API so no fourth copy was added.)
   - ✅ **Audio Setup panel** (§10.1) — `panels/audio_setup_panel.rs`, a centered modal toggled with ⌘⇧A (Escape closes). Input-device cycle + one row per send (channel stepper, gain stepper, delete) + Add-Send. Clicks resolve to the project-level audio-setup `PanelAction`s → tested EditingService commands; `state_sync` configures it (device enumeration + sends) each frame it's open. Multi-channel routing now works: route kick to channel 3, bass to channel 4, etc.
   - **Deferred polish (not blocking):** migrating the existing driver/envelope/Ableton drawers onto the drawer API (they remain bespoke; the audio drawer uses the API so no duplication was added); per-send text-field rename (sends auto-label "Audio N"); the v2 per-send analysis toggles (pitch tracking); multi-channel downmix per send (the panel edits a single channel per send in v1).
