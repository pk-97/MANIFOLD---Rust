...
| `node.plasma_pattern_2d` | 8 plasma variants (sin-based math) | **Atom decompose per-variant.** Most variants decompose into **`centered_uv`** + **`sin_term`** + **`math`** + **`mix`**. Audit each variant: if any one is tightly coupled enough that atoms can't reach, that single variant becomes wgsl_compute-backed while the rest atomize. |
...
| `node.polygon_shape` | Regular polygon vertex generation | **Atom decompose.** Sampling N points around a circle — same `generate_range` + `array_trig` family as Lissajous, plus a closed-loop packer. Likely shares atoms with the Lissajous decomposition. |
| `node.concentric_outlines` | Ring stacker for polygon | **Atom decompose.** A scaling/replication loop over an input polygon array — atomizable as `array_replicate` + `array_scale` or similar. |
