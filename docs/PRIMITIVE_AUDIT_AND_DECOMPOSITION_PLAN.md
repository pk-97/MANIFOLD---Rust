...
- **Recommended sequencing:** (a) Peter's **visual checkpoint** on the 5 queued Phase-2 effects (validates the `coordinate→remap→mix` + field/displace foundation, unblocks bundle deletions); (b) `threshold` + Bloom; (c) palette generator + Infrared (after Peter picks the ramp-vs-enum fork); (d) ColorGrade / HighlightBoost decompositions; (e) Dither; (f) delete Halation; (g) Glitch / Watercolor / Strobe / Voronoi; (h) Plasma.
