...
- **Phase 3 landscape — audited this session. Unlike Phase 2 there is NO remaining "mechanical batch"; every remaining effect is its own sub-project.** Surveyed every monolithic (3-node `source → bundle → final`) effect preset + the temporal ones. They sort into four buckets:
  1. **Already correctly atomic — NOT decomposition targets (§0 second guardrail).** `InvertColors` (`node.invert`), `Transform` (`node.affine_transform`), `EdgeGlow` (`node.edge_detect`), and almost certainly `ColorGrade` (`node.color_grade`) and `HdrBoost` (`node.highlight_boost`) — these are curated mid-level ops at the altitude a user reaches for (same class as `levels`/`color_grade`/a Color TOP), not compositions wearing single-dispatch clothing. **Judgment fork for Peter:** confirm HighlightBoost (soft-knee threshold + EV boost + colour-ratio rescale) and ColorGrade (lift/gamma/gain/sat) stay whole. The only *trapped* reusable atom in HighlightBoost is the soft-knee threshold curve, which Bloom's prefilter also needs — extract it there, not by exploding HighlightBoost.
  2. **Genuine decomposition targets needing new infra / multi-pass.** `Bloom` (`node.bloom`) + `Halation` (`node.halation`): real multi-pass composites — prefilter (soft-knee `threshold`, currently a no-op stub) → downsample pyramid → `node.blur` (✅ already fully implemented: separable Gaussian/Box/Radial, 2-pass) per level → combine/`mix` → (Halation: tint). Need the `threshold` impl + `node.downsample` pyramid; delete the `mip_chain` no-op stub. `Dither` (`node.dither`): §0-named target — the trapped reusable atom is the **ordered-dither/halftone pattern generator** (Bayer/Halftone/Lines/CrossHatch/Noise/Diamond → threshold field). Clean split = `dither_pattern` (NEW gen) → `dither` (refactored to consume an external `pattern` texture + do the biased-quantize/hue-rescale/crossfade) → done. Cost: split shader + Rust, rewrite/delete the existing `tests/parity/dither_pattern.rs` (6-algo parity — deletable under the visual-effects-skip-parity policy), rewrite preset, mod.rs.
  3. **Blocked on infrastructure.** `Infrared` (`node.infrared`): bakes 10 *multi-stop* palette LUTs at 512×1; `color_ramp` is only two-stop, so a faithful decomp needs palette/LUT-data infra + per-slot texture resolution (the file documents it as intentionally monolithic for this reason). `WireframeDepth` (`node.wireframe_depth`): DNN wrapper + a parallel agent is already working on it — leave it.
  4. **Temporal reconstructions (cluster together).** `Voronoi Prism`, `Strobe`, `Glitch`, `Watercolor` — beat/time-driven; share a likely-common beat-fade / per-cell pattern. Build the shared temporal atoms once, then do them as a set (StarField-class). `Plasma` decomposes separately (6 sin-variants → atoms muxed on `pattern`; Noise/Fractal → bespoke `wgsl_compute`, per [[project_graph_compiler_initiative]]).
- **Recommended sequencing for Phase 3:** (a) Peter's **visual checkpoint** on the 5 queued Phase-2 effects first — it validates the whole `coordinate→remap→mix` + field/displace foundation before more is built on it, and unblocks the bundle deletions; (b) implement `threshold` + `downsample` and do the **Bloom/Halation/HighlightBoost-prefilter** cluster (shared infra); (c) `Dither` (self-contained, pattern-gen extraction); (d) the **temporal cluster** (Voronoi/Strobe/Glitch/Watercolor) with shared beat atoms; (e) `Plasma`. `Infrared` waits on palette infra.
