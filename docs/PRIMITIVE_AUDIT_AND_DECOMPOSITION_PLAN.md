...
- **Remaining to migrate (in order):**
  - **ColorGrade** (concrete plan): decomposes to `gain (exists) → luma-saturation (node.luminance → node.mix Lerp by saturation) → hue (node.hue_saturation, hue only, sat=1 value=1) → contrast (NEW node.contrast: `(c−0.5)·k+0.5`; can't reuse `node.levels` — its `pow` NaNs on the negatives contrast produces) → colorize (NEW node.colorize: tint-toward-a-hue masked by smoothstep(luma highlights) × (1−smoothstep(saturation)) × focus, then `mix(c, tint_rgb·graded_luma, colorize·mask)`) → node.mix(Lerp, amount) → node.clamp_texture(≥0)`. ~10–12 nodes, 9 bindings, 2 new atoms. Legacy math in `effects/shaders/color_grade.wgsl` (read it — gain, luma-sat with Rec.709, K-matrix HSV hue, pivot-0.5 contrast, the colorize pass lines 70–92, final amount + max(,0)).
  - **Strobe** (`node.strobe`): 10-entry note-rate table (→ inline `mux_scalar` option-table per [[feedback_inline_mux_option_table_params]]) + beat square-wave (lfo/beat_gate or a small atom) + 3 composite modes (Opacity/White/Gain). Temporal.
  - **Glitch** (`node.glitch`, time-driven): block-hash-displace (NEW atom) + scanline jitter + `node.chromatic_displace` (exists) + per-block invert. UV-warp family.
  - **Watercolor** (`node.watercolor`, time-driven): 7-pass feedback sim — flow (`flow_field_noise`/fbm) + `uv_displace_by_flow` + blur + `node.feedback` + wet/dry. Biggest.
  - **Voronoi Prism** (`node.voronoi_prism`): needs a per-cell-hash atom (likely `voronoi_2d` gains a `cell_id` output) — see [[project_voronoi_prism_is_temporal_reconstruction]]. Beat-driven temporal.
  - **Plasma** (`node.plasma_pattern_2d`?): 6 sin-variants → atoms muxed on `pattern`; Noise/Fractal → bespoke `wgsl_compute` ([[project_graph_compiler_initiative]]).
