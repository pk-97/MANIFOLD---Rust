...
**Status:** DIRECTION captured 2026-07-11 · Fable + Peter · not designed to STANDARD, not built; per-piece designs graduate from here. Remaining ungraduated: section 7 (HDR) only.
**Graduated:** section 2 → [GBUFFER_DESIGN.md](GBUFFER_DESIGN.md); section 6 (Deferred) → [CAMERA_AND_LENS_DESIGN.md](CAMERA_AND_LENS_DESIGN.md) + [CINEMATIC_POST_DESIGN.md](CINEMATIC_POST_DESIGN.md); section 8 → REALTIME_3D section 11 (PCSS); section 4 (Phasing) volumetrics → [VOLUMETRIC_LIGHT_DESIGN.md](VOLUMETRIC_LIGHT_DESIGN.md); section 9 RT → [RAYTRACING_DESIGN.md](RAYTRACING_DESIGN.md); section 10 animated glTF → shipped via [GLTF_ANIMATION_DESIGN.md](GLTF_ANIMATION_DESIGN.md). **section 5 (Decided) export tiers DROPPED by Peter 2026-07-12** ("Ignore the explicit per-scene export quality please"). section 3 (Invariants) materials v2 DEPRIORITIZED (game models/photoscans) — captured, not designed.
**Companion docs:** [REALTIME_3D_DESIGN.md](REALTIME_3D_DESIGN.md) (base scene pass; its section 10 (instancing addendum) documents the memoryless-depth constraint), [MATERIAL_SYSTEM_DESIGN.md](MATERIAL_SYSTEM_DESIGN.md), [PERF_BUDGET_GATE_DESIGN.md](PERF_BUDGET_GATE_DESIGN.md) (the measuring instrument), [VULKAN_BACKEND_DESIGN.md](VULKAN_BACKEND_DESIGN.md).
...
(section 2). Hardware note, Peter verbatim: "The Macbook is not the ceiling for hardware,
...
- **This doc reviewed/amended as scenes land**; per-piece STANDARD designs authored
  after intake (Opus block). HDR-out (section 7) and physical camera (section 6) are separable and
...
(zero bandwidth; REALTIME_3D section 10 records why and rejected a depth-compositor on this
...
6. **Transmission/refraction LAST** — light through petals, refraction through
   ...
   memoryless constraint as section 2 — it is gated on the G-buffer decision.
...
candidate (section 5). Scene 1 (fog + sun + canopy) is its intake evidence — note its
first fog finding is already a bug (BUG-118 washout), so section 4's design starts from
...
Focal length, aperture, shutter — one param set that ties DoF (needs section 2 depth),
motion blur (needs section 2 motion vectors), and exposure together so they behave like
...
diamond), short-range emissive bounce eventually subsuming section 3.1's derived light.
...
motion vectors + stored depth (gated on section 2); denoiser tuning. P8 instances map
...
- **Emissive-as-real-GI in the rasterizer** — every-surface-asks-every-surface is
  the rendering equation; no per-element shortcut exists. The derived light (section 3.1)
...
- **Depth-aware two-pass compositor** — already rejected by name in REALTIME_3D
  section 10; the G-buffer (section 2) does not reopen it (storing depth ≠ compositing passes;
...
- **Per-instance material variation** — P8 deferral stands (instances share the
  ...
  groups. Trigger lives in REALTIME_3D section 10 Deferred.
...
- Scene-ladder findings folded in (requirements for section 3/section 4; confirm cutout need,
  scan-vs-scene-lighting reality; BUG-118 root cause feeds section 4).
- Perf-gate numbers for section 2 (bandwidth, deform-twice cost) → the G-buffer decision.
- The two section 3 audit items (emissive channel; normal input path) — run before the
