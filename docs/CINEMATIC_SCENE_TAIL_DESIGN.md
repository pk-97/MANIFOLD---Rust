...
**Status:** IN PROGRESS — P0+P1+P2+P4 executed 2026-08-26 (BUG-136 (motion blur no visible effect) root-caused: no code defect; import tail wired; v1.13.0 migration; P4: enabled toggles on motion_blur/bokeh_gather, shutter default 180, Camera-section stamps + Scene Setup rows, 4K tail measured +2.3 ms, near reduction blocked by BUG-rdy0 (RT zero-intensity sun direction leak)) · 2026-08-27 P4 repair merge: f-stop band 0.5–32 actually reaches the panel (the widen rule had stretched every stamped slider to the 1000 seed), toggles labeled "Depth of Field"/"Motion Blur", DoF off = bokeh `enabled` seeded false (no f-stop value is off on close-ups), focus ≤ 0 honors the hyperfocal contract (was max blur), load-time repairs cover existing projects · P3 look-pass RUNNING (Peter) · owed: half-res DoF design phase (revived, figure on record) · k3 (lead)
...
- **I1 — Default lens is a bit-clean pass-through through the whole injected tail on a static scene.** Enforcement: gpu_test — import-assembled graph with default lens, noise-texture scene, `motion_blur.out` byte-compared against the SSAO-mix output (extends CINEMATIC_POST I2 (pinhole pass-through) to the assembled graph, not just the reference preset). Amended 2026-08-26 night (P4): the default shutter is now 180, so the invariant holds via the static scene's zero velocity field collapsing every tap — no longer via shutter 0. Re-verified green at P4 execution.
...
- "set the near camera clip plane to 0 and the default shutter angle to 180" → near: EXECUTED-as-blocked at P4 (2026-08-26 night, k3 (lead)): 0 is impossible (`perspective_rh` computes `clip.w = range * z_near * z` — z_near = 0 zeroes clip.w for every vertex, NaN NDC across the frame), and the dissent path's 0.01 deterministically breaks two RT byte-identity gpu gates (rt_multi_caster_shadow / rt_object_cast_shadows — cure-tested 0.05 green / 0.01 red; both compared renders share the near, so the nearer plane EXPOSES a zero-intensity sun direction leak in the RT shadow path, it doesn't cause one — bead BUG-rdy0 (RT zero-intensity sun direction leak) filed; the default moves once the leak is fixed). Note: imported scenes never read this default — the import stamps a scene-scaled near (`front_margin * 0.5`, floor 5e-5). Shutter default 0→180: SHIPPED (atom default, assembly stamp, migration lens stamp). I1 re-verified (static-scene zero velocity keeps the byte-identity) and its doc text updated (section 4 (Invariants & enforcement)).
...
- "the blur does have a pretty huge hit to FPS even with just 4K raster scenes... can't render 4K 60 with it?" → 4K quadruples the full-res gathers, paid even at neutral defaults. The enabled toggles + shutter-0 auto-skip fix "off costs money"; the deferred half-res DoF phase (section 7 (Deferred)) is REVIVED for "4K60 with DoF on" — its revival trigger (I4 budget binding on the heaviest show project) fired with this report. MEASURED at P4 execution (2026-08-26 night, apricot_tl05 import, I4 harness at 3840×2160): tail delta mean **+2.3 ms** (with-tail frame mean 14.2–15.2 ms vs stripped 11.9–13.0 ms; ~2x the 1080p delta, not 4x — fixed per-dispatch overheads don't scale). The tail alone does not break 4K60; the whole-frame mean is what sits near budget. The figure for the half-res phase is on record.
