...
**Settled decision (revised — tabs are a view, not a selection change):** a tab is a **scope pin** over the live selection, never a selection edit. The pin generalises the original Master-only pin to all rungs: `scope_pin: Option<(InspectorTab, u64)>` on `UIState`, active iff the stored `selection_version` still matches — so any selection change auto-clears it (zero edits to the selection mutators), and the inspector falls back to the selection-derived default scope. Clicking a tab calls `pin_scope(tab)` and repoints only the inspector's layer *focus* (`active_layer`): Clip/Layer focus the selection's own layer, Group focuses its group ancestor, Master needs none. **The timeline selection is never touched**, so the whole `Clip · Layer · Group · Master` chain stays available no matter which rung you're viewing, and the clip you were on stays selected.

This replaces the earlier *strict-mirror* tradeoff (where clicking `Layer` selected the layer and dropped the clip, collapsing the `Clip` rung). The rung set is now derived from the **selection's own layer** (the clip's layer, or the selected layer) rather than `active_layer` — since `active_layer` follows the pinned tab, deriving the chain from it would have lost intermediate rungs (e.g. viewing `Group` would drop `Layer`). Deriving from the stable selection keeps the full chain regardless of focus.

**Where it landed:**
- `InspectorTab` gains `Group`; `is_layer_scope()` folds Group into the layer column everywhere.
- `UIState`: `scope_pin` + `pinned_scope()` / `pin_scope()` / `clear_scope_pin()` — the generalised pin.
- `InspectorCompositePanel`: `active_tab` + `available_tabs` + `configure_tabs`/`set_active_tab` drive the existing `*_visible` gates so only the active scope renders; `build_tab_strip` renders the rungs; tab clicks route through `route_click` → `SelectInspectorTab`. Both scroll columns still build every frame (inactive one collapses to zero width) so no node ids go stale.
- `sync_inspector_data` derives the rung set from the **selection's** layer, picks the active rung as `pinned_scope()` (filtered to the available rungs) else the selection default, then `configure_tabs`.
- `inspector_select_tab` (in `ui_bridge`) pins the scope and repoints `active_layer` — it never mutates the selection.
