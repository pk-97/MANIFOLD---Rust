...
**Status:** IN PROGRESS — Tier 1+2, the motion class, reflections R1–R3, T3-8 multi-bounce GI, section 12 (screen-space AO), section 13 (temporal denoiser rebuild), section 14 (traced env diffuse), section 16 (thin-surface translucency) TL-A, and the quality wave A1–A3 (oracle harness, per-term cost, split trace dispatch) landed; phase records in section 9.6 (Phases). Section 15 (many-light: caster cap + emissive RIS) and section 16 TL-B/TL-C APPROVED 2026-08-02, not built. OWED: Peter's look at multi-bounce, R2 constants, the denoiser under fast camera motion, ED-A env-diffuse on a real hero scene; a `trace_ms` 2-vs-1 number from a heavier scene; A3 split-mode matrix remeasure (racy-build numbers void). P5 export (D13), P6 frame interp stay show-need-triggered. · 2026-08-03 · K3 + Peter
