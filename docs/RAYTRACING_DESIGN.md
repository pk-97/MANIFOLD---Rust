...
| Hybrid RT direction | RENDERING_INFRA_V2_DESIGN.md §9 (Hybrid ray tracing (post-release) — GRADUATED 2026-07-21 → `RAYTRACING_DESIGN.md`) | Direction + backend seam decided in principle; this doc is its graduation. Its rejections stand (no real-time path tracing; emissive-as-real-GI in raster rejected). |
...
- *Read-back:* D4/D5; §5.1 self-emission gap note; prototype GI gather; VOLUMETRIC_LIGHT_DESIGN.md P1 findings (fog state of play, BUG-118 (render-scene-fog-washes-out-instead-of-depth-gra…) context — DEFERRED, do not touch).
...
- *Deliverables:* environment shading at the reflection hit point (the hit surface's own IBL
  ...
  otherwise it stays deferred (§9.7).
...
**Phasing-completeness check:** every §9 commitment appears exactly once — toggle (Base traced
...
- **Shading-normal prepass target** — trigger: RD3's mismatch in Base traced reflections' (R1)
  demo or Peter's look; when the trigger fires it is absorbed by Raster-parity reflections (§9.6).
