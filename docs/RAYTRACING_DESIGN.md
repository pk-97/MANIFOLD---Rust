...
**Dynamic geometry successor:** [Scene modifier RT](SCENE_MODIFIER_RT_DESIGN.md) specifies the replacement for D17’s content-settle/private-submit mechanism, with caller-ordered updates, preparation before publication and no mid-frame CPU wait. Its phase/status markers govern implementation; D17 above records existing behavior until that work lands. [Acceptance definitions](SCENE_MODIFIER_RT_ACCEPTANCE.md) are owned by `BUG-e3p6.4`.
