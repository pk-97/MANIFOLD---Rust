...
**Status:** IN PROGRESS — v1 wave, Tier 1+2, the motion class, base reflections (R1), raster-parity reflections, and stable reflections (R2) all LANDED on main (per-phase records: section 8 tiers, section 9.6 phases). Peter's L2 look PASSED 2026-07-24 and RT is usable under model motion. OPEN: textured roughness (R3) next; converged-static traced-detail wash + motion speckle (R3 territory); rotation-artifact look needs a real multi-frame repro; Tier 3 items 6/8/9 (many-light first); P5 export (D13); P6 frame interp (Tahoe, D6/D8). R2 constants untuned (`rt_r2_sweep_dump --ignored`) — tuning + the camera-sweep motion verdict are Peter's look (D-61). Perf profiling DEFERRED by Peter until the pipeline is complete. · 2026-07-24 · K3
