...
**Status:** IN PROGRESS — Tier 1+2, motion class, reflections R1–R3, T3-8 multi-bounce GI, section 12 (screen-space AO), section 13 (temporal denoiser), section 14 (traced env diffuse), section 16 TL-A (translucency), quality wave A1–A3 (oracle harness, per-term cost, split trace dispatch) landed; phase records in section 9.6. Section 15 (many-light) and section 16 TL-B/TL-C approved, not built. OWED: Peter's look at multi-bounce, R2 constants, denoiser under fast camera motion, ED-A on a real hero scene; `trace_ms` 2-vs-1 from a heavier scene; A3 split-mode matrix remeasure (racy numbers void). P5 export (D13), P6 frame interp show-need-triggered. · 2026-08-03 · K3 + Peter
