...
**Status:** RT v1 WAVE LANDED on main 2026-07-23 (overnight run, D12; wave tip merged `519d01ee`+C4 `bff0fa15`, full gate green: nextest 3879, clippy+deny clean, gpu-proofs 77/77). SHIPPED: W0 stored G-buffer (D14, BUG-136 (CINEMATIC_POST motion blur has no visible…) velocity math proved correct), P1 hard shadow rays (D16 forward integration; BUG-308 (rt-p1-integrated-scene-shows-no-shadow-effect) accel race + BUG-309 (rt-shadow-not-confined-to-occluder) bias epsilon fixed en route, D17; BUG-310 (rt-tracer-pipeline-jit-hitches-first-rt-frame) tracer prewarm fixed at landing), P2 soft shadows + AO + demodulated temporal accumulation with D3/RT-D2 node-local resets, P3 emissive GI + sun-bounce + RT volumetrics (D4/D5), P4 MetalFX temporal SEAM-ONLY (scaler + shared TemporalResetDetector + jitter + toggle landed; the live reduced-res-render→upscale path into scene output is NOT wired — follow-on). 2026-07-23 Peter's first look: three integration bugs found+fixed same day (toggle visibility `6e44894e`, ambient-knob wiring `7b3d8dd2`, sun double-count `818a06b0`); verdict = v1 is a landed skeleton, not stage-ready — ghosting (BUG-311) + noise (BUG-312 (rt-ray-noise-speckle)) + depth-derived normals are structural gaps. Tier 1 BUILT + LANDED 2026-07-23 (section 8.1 D18–D20, overnight wave: real vertex normals, reprojected validity-tested accumulation, variance-guided à-trous + blue-noise; BUG-311 (rt-accum-no-reprojection-ghosts-on-motion)/312 FIXED, BUG-316 (né 315; id ceded to main's stale-roughness bug…) resolved-as-oracle-confound (id 315 ceded to main at merge)). Tier 2 BUILT + LANDED 2026-07-23 (section 8.2 D21/D22: alpha-aware rays via shared alpha-test walk; live MetalFX temporal at 2/3 render scale, native mode machine-diff-identical). 2026-07-23 Peter's motion look: RT was static-tableau-only — moving a mesh flickered (BUG-320: accel key hashed transforms, so a moving object never refit and every motion pause rebuilt+dropped to raster) and then shimmered (T2-C section 8.3: accumulation had no per-object motion term, so a moving object lost all history mid-gesture). Motion class CLOSED 2026-07-23: BUG-320 (accel refit), BUG-321 (per-object motion reprojection) and BUG-322 (normal history compared across object orientations — the actual cause of the helmet shimmer, `d68e07bb`) all fixed, the last CONFIRMED by Peter in-app on `RT Testing.manifold`. **RT is now usable under model motion.** section 8.3 records the diagnosis and the method lesson. 2026-07-24 Peter's L2 look: RT PASSED ("looks good") — Tier 1+2 motion/still/upscale verdict is IN. Ray-budget re-judge and all perf profiling DEFERRED by Peter until the full RT pipeline is built (no profiling on unfinished features). REFLECTIONS (Tier 3 item 7) APPROVED + IN PROGRESS 2026-07-24 — folded into section 9 (draft RT_REFLECTIONS_DESIGN.md deleted); K3 review rulings Q1–Q5 + Q1 settling test executed (vertex normals stand — normal-map breakup is sparkle, not shape). Execution: 4 phases (section 9.6), team wave via GLM dispatcher + Flash executors. Base traced reflections (R1) LANDED 2026-07-25 (`163ed86f`). 2026-07-25 Peter's AMG GT3 look: reflections ON renders a metallic textured hero asset black + textureless — RD4's hit shading (emissive + flat albedo × sun-bounce, no env, no maps) substitutes for the prefiltered env that WAS the car's look; confirmed in code + headless `render-import` PNGs same day (K3). Response: phases renamed name-primary (R-numbers survive only attached to names, Peter directive) and **Raster-parity reflections** inserted between R1 and R2 (env + textured shading at the hit point). Raster-parity reflections BUILT 2026-07-25 (`7ad9bd36`: env + textured albedo at reflection hits, material-texture table 4→64; second slot-map miss fixed same day — Rust binds/slot map lagged the MSL move to 68/69, mirror probe read zeros, GLM-4.7 lane misread the failures as expected; K3 review caught + fixed, probes green) Peter's AMG look = gate c PASSED 2026-07-26 (glass specular back, D-57) — OPEN: computed-pixel parity test (phase gate a, folded into the R2 wave as Step 0). BUG-88m (Blend/Mask glass black-holed its env IBL under rt_reflections — substitution domain wider than trace domain) FIXED 2026-07-25 `13653dd5`: out_refl alpha is a validity signal (-1 = no traced value), fs_pbr substitutes only `.a >= 0`; upsample carries -1 through. BUG-326 (imported glbs trace blind — one-shot accel build raced the async glb loader's GPU staging copy; frozen empty BVH → black reflections + silent missing shadows/AO) FIXED 2026-07-25 `0fc39f15`: per-topology accel rerun armed on topo sighting, fired at first ready (`render_scene.rs` `rt_accel_rerun_armed`); gate `rt_bug326_fix_gate` — mechanism SUPERSEDED 2026-07-26 `f0c61b10` (generation-keyed content-settle rebuild; rerun heuristic deleted, section 10 Bug A). NEXT: Textured roughness (R3); rotation-artifact look (one-frame accel refit latency under continuous model rotation, `render_scene.rs:3945` — suspected R2-reprojection territory, needs a real multi-frame repro, 2026-07-25); Tier 3 items 6/8/9 per section 8 (many-light next after reflections); P5 export (D13); P6 frame interp (Tahoe, D6/D8). **Stable reflections (R2) LANDED 2026-07-26 (`74563a4c`, K3-lead wave, D-58):** specular history ping-pong in the same accumulate kernel (one ping clock, I-R2), virtual-image reprojection (D-62 — RD6's `wp + hit_dist·R` was the REAL hit point, correct only against a scene-color history; amended to `wp − hit_dist·V`, exact for planar mirrors), reflection-channel à-trous weight with roughness-narrowed luma stop (shiny = crisp, rough = blurred), binding 43 sources the accumulated history. En route the D-61 gate caught and fixed a second latent bug: `PresetRuntime::render` hardcoded `owner_key`/`frame_count` to 0 — cut-reset AND per-frame GGX jitter were dead on the render path (harness + scene viewports). Gates: scene control-leg (blend 1.22×B no-cut, raw 1.94×B on cut), kernel-level blend/reset value proof, I-R2/I-R3/I-R5 negative greps, gpu-proofs lib 1845 (2 pre-existing BUG-324 (bug221-layout-test-mean-abs-diff-pre-existing)/325), integration 101/101, worst frame 10.22 ms. Constants untuned (`RT_REFL_ACCUM_ALPHA` 0.1, `RT_REFL_VIRTUAL_REPROJ_ROUGHNESS_BLEND` 0.5, atrous sigma 0.05–0.3) — tuning + the camera-sweep motion-quality verdict are Peter's look (D-61; sweep dump: `rt_r2_sweep_dump --ignored`). Mirror probe re-pinned mean-based for the sharpening (on≥1.0, delta≥0.3). Raster-parity gate (a) re-scoped (D-60): env-at-hit + I-R1 + Peter's AMG look carry it; graph-wired-texture proof logged as a bug. R2 entry conditions for R3: MET. 2026-07-26 section 10 transition contract added (derivation-over-side-effects rules) + Bug A (blind-BVH-after-load, `f0c61b10`) LANDED from Peter's RtNoiseTesting repro; OPEN: converged-static washes traced detail (R3), motion speckle (R3). · 2026-07-24 · K3
