...
<!-- index: Proposed v3 graph schema, ordered modifier instances, typed attachment recipes, expansion and performance binding contracts. -->

**Status:** PROPOSED · 2026-09-10 · Codex lead · not implemented.
**Prerequisites:** Existing graph groups, preset metadata, Scene Loop/Fog and parameter surface. No new backend.
...
`PrimitiveRegistry` refers to the existing renderer registry type, imported from its existing module. Errors are structured variants: UnsupportedVersion, MissingScene, AmbiguousScene, MissingTarget, DuplicateIdentity, InvalidRecipe, UnsupportedEndpoint, MissingInput, ConflictingSource, RecursiveModifier, InvalidBinding and CapacityExceeded, each carrying the offending stable path and human-readable detail. No panics for authored input.
...
Unwired Transform has existing identity semantics. Unwired Instances means one original object: materialize an identity instance only when a stage needs an array. Unwired Vertices is not a drawable target and fails applicability for a mesh stage. No invented geometry. Scene bounds unavailable or degenerate: bounds-dependent recipes reject with a useful reason; recipes not using bounds remain applicable.
...
Transforming waves are chainable and may appear repeatedly. Their Enabled=0 must produce exact input data. Amount=0 is also exact identity for every response. A branch bypass may avoid GPU work through existing liveness; disabling must not trigger pipeline creation. Count-expanding operations define a separate capacity contract, not a transform bypass hidden by truncating output.
...
| A9 | Geometry changes reach raster and RT consistently | `scene_modifier_geometry_generation`, F7/F8 |
| A10 | Source-only restrictions are explicit and tested | `scene_modifier_source_order`, F2 |

## 9. Phasing

F1 schema; F2 expansion; F3 commands/bindings; F4 legacy migration; F5 cards/editor/library; F6 Loop/Fog files; F7 first instance wave; F8 release proof. Exact entry checks, seam inventory, deliverables and commands live in the [Foundation Plan](SCENE_MODIFIER_FOUNDATION_PLAN.md), the single owner of this architecture's executable briefs.
