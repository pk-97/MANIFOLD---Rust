...
Periodic modifier controls declare `wraps: true` on their parameter specs so saw LFOs wrap instead of clamping when driven beyond the range boundary. Full-cycle phase controls use 0–1; orientation and mask angles use a complete turn. Vortex Orbit, Peel Curl and Recon Rotation use their existing controls with a fresh 0–360° range. Their primitives apply the full angle before blending the rotated pose with the input by influence, so a full turn returns to the same output at partial influence too. This changes the previous weighted-angle deformation: partial pose blends can compress geometry during a turn and are not rigid rotations. No additional control, animation clock or runtime mode is introduced. FOV, pivots and translations remain nonperiodic.

Load-time `scene_modifier_periodicity::repair_scene_modifier_periodicity` repairs stale local and host flags only for verified primitive/parameter bindings whose mapped range spans complete periods, including Peel's base-plus-burst angle addition. Authored values, defaults, IDs and bindings are preserved. Existing full-turn ranges remain intact; the former stock Recon −90–90° range expands to −180–180°. Custom or calibrated partial ranges are not expanded. Math View remains independent of this parameter contract.

## 5. Sources, identity and bypass

### Clip-edge response contract

The existing examples are authoritative reuse precedents: Strobe wires `trigger_gate → envelope_decay`; NestedCubes gates a pose-table/step response and kick; BasicShapes uses `trigger_ease_to`. The last node samples the currently visible scalar on retrigger and eases to its new target over `window_beats`. The decay node uses seconds internally and suppresses a pulse on first observation. Neither behaviour may be silently changed for old presets.

Three authored response patterns are required vocabulary: hit/return (restart a bounded envelope), advance/hold (select the next authored target), and transition/settle (glide from the visible value to that target). They are reusable graph groups feeding the existing geometry inputs, not per-look callbacks or another animation engine. Clip Trigger is an ordinary trigger-gate control; Strength and Duration/Decay are ordinary live controls. Beat-duration responses use the beat clock directly; `PresetContext` has no BPM port. Existing `envelope_decay` stays seconds-based and must be labelled accordingly. A reusable beat-window envelope and explicit event initialization are demonstrated vocabulary gaps to close before the beat-duration hit/return proof; do not alter legacy decay semantics or invent a hidden BPM conversion. The shape of the gesture may coordinate multiple parameters with different curves/delays. Manual/LFO values remain graph inputs, composed explicitly as base plus bounded trigger contribution or a declared selector; triggering must not overwrite their saved base values.
...
`node.envelope_beats` supplies a bounded linear hit/return response with an optional first-event baseline; ordinary graph arithmetic supplies strength and shaping. `trigger_ease_to` gains optional `initial_count`/`initial_value` inputs for first-event glides while retaining its unwired first-sample snap. CPU trigger state is carried only for matching modifier IDs, unchanged local recipes and explicitly routed eligible nodes. GPU feedback is outside that transfer.

Modifier TriggerCount/TriggerBaseline contexts compile to ordinary CPU `node.value` producers with explicit per-instance event routes. They are separate from the legacy host generator input. Audio pulses retain the firing preset ID and a parameter token; preparation rejects token collisions, and dispatch verifies the exact owner before routing to its modifier. Host clip edges feed each modifier whose own gate mode accepts clips. Event counters and pending baselines carry by modifier ID through reorder; unchanged local latch state uses the stricter recipe compatibility check above. Existing host/effect/master audio behavior stays unchanged. Focused tests prove isolated audio, shared clips, beat-window output, pending-event carry and watched/fused installation. Production transport observation and file-only recipe proof remain unfinished.
...
Modifier cards use the same drag lifecycle, ghost, insertion marker and selection gestures as effect cards, with an explicit separate stack domain. Actions capture the layer and stable modifier IDs; drop cannot cross into the post-effect stack. Group edits publish one admitted undo transaction. Duplication remints instance/macro identity and preserves control/modulation state.

Copy/Paste transfers selected modifier snapshots in stack order, with fresh instance and macro IDs, values, drivers, envelopes, audio modulation and automation. External hardware mappings are not duplicated. Copy, Cut, Paste, Duplicate and Delete use the same selected-card handlers from menus and keyboard shortcuts. Cut is one undo step, and successful add/paste/duplicate selects the new cards through a content-thread selection receipt. Cmd+C/Cmd+V use modifier selection; an empty scene stack accepts the modifier clipboard, and the generator menu offers Paste Modifiers. Selected effect cards retain their existing shortcut routing. Changing a generator preserves its modifier IDs and control state, including mappings, in the same undo transaction. Both operations resolve destination scenes and capture destination mesh frames; explicit object targets must remain reachable. Incompatible transfers reject the whole edit through the existing admission feedback, leaving the original generator and stack intact.

The modifier MOD badge compares the local recipe against the catalog recipe initialized for the owner’s scene bounds, ignoring canvas position. Snapshot presence alone is not divergence; live base values and modulation remain on the host. Missing catalog recipes remain local and modified. Revert uses this same pure initialization path without recapturing mesh frames. Cards share one preset menu vocabulary and app dispatch for Make Unique, import/export, library/project save and revert. Modifier mutations execute on content through existing graph admission; legacy effect/generator optimistic dispatch remains an explicit compatibility boundary. Save publishes a copy; Make Unique/import retarget the captured instance.

Surface Peel and Ordered Recon each expose Clip Trigger on the base card, off by default. Peel adds a bounded beat-envelope burst to independent base lift/curl and keeps its spatial mask. Turning the gate off immediately removes the burst. Recon uses the existing directional attack/hold/return envelope; its Amount is bound to Clip Trigger, so off passes the spatial mask through and restores the ordinary manual deformation. The gate consumes the owner’s pre-event baseline; its downstream envelope starts from the gate’s zero counter. No animation is baked into a scan.

The factory picker contains eight distinct modifiers. SurfacePeelHit, OrderedReconHit, MaskedPeel and WavesEchoes remain loadable compatibility recipes with `available: false`; the picker honors that metadata. Use Surface Peel’s mask controls for a masked peel, or stack Surface Waves and Spatial Echoes. Existing instance snapshots, custom graphs, parameter identities and modulation are preserved; opening a project never replaces its authored recipe with a new factory graph.
