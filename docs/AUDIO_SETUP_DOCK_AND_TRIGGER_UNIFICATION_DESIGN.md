...
**Status:** SHIPPED — Wave 1 (P1–P4, 2026-07-10) and Wave 2 (P5–P8, 2026-07-11) all landed; phase briefs and as-built corrections live in section 7. Wave 2 existed because Peter's morning-after rig review caught the D6 fire meter dead in every transport state (BUG-109, which also reopened and re-closed BUG-082; the reset-once cure is a stated invariant in `CORE_ENGINE_MAP.md` section 6). OPEN: VD-025 and VD-026 — live-rig confirmation of the fire meter and the scope↔drawer linkage needs a live audio device, not claimable by an executor (section 7.3).**
...
the entry joins BUG-079 (missing-preset-fails-silently-no-onscreen-signal)'s "surface load notices in-app" scope rather than growing its
own UI. Round-trip gate in P2 covers: legacy project loads → triggers fire → save →
reload → triggers still fire (the BUG-036 (param-manifest-construction-not-a-unified-safe-g…) lesson: create-path green is half a gate).
