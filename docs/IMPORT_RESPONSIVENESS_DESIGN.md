...
- **P3 — Background worker + progress toasts (one session). BUILT 2026-07-17, pending landing.** D3 + D4 wholesale. Gate: L3 flow — drop a large fixture, assert the stage toast appears while the import is in flight (the harness can assert between frames), assert the layer lands and plays; the corrupt-fixture Failed toast asserted; round-trip gate n/a (no persistence change); `MANIFOLD_RENDER_TRACE=1` statement that the content thread gained zero work. Performer gesture: drop a 43 MB model mid-playback and keep scrubbing the timeline — the flow asserts playback frames keep advancing during the import (transport time strictly increases across snapshots).
  **Built:** `crates/manifold-app/src/import_worker.rs` (new) — `ImportProgress`/`ImportStage`/`ImportRequest` (D3's committed shape verbatim) + `run_import_worker`, the entire old blocking body of `import_model_file` (optional Blender convert → `assemble_import_graph` → `validate_def`, reusing P2's `validation_gpu_device()` handle) minus the command-dispatch tail, sending `Stage`/`Done`/`Failed` over a `crossbeam_channel`. `app_lifecycle.rs`'s `import_model_file` is now spawn+enqueue only (`spawn_import_worker` / `import_queue: VecDeque<ImportRequest>` for the "second drop while busy" case); `finish_import_model` is the extracted, unchanged UI-thread tail (mint preset id, install overlay, dispatch `ImportModelLayerCommand`/`AddClipCommand`), called from the new `drain_import_progress` (`app_lifecycle.rs`, wired into `app_render.rs`'s per-frame drain next to `tick_import_failures`) on `ImportProgress::Done`. `Stage` re-shows `ws.ui_root.toast` (D4, "Importing <name> — <verb>…"); `Failed` shows an accent toast (`manifold_ui::color::RED_BASE`) — never silent. `UserPrefs::ephemeral()` (new, `user_prefs.rs`) carries just the Blender-path pref key across the thread boundary (`UserPrefs` itself stays non-`Clone`).
  **Gate 1 (L3 flow):** the flow driver (`scripts/ui-flows/`) has no verb for a file drop yet — per the brief's own stated fallback, this is the drain-loop integration test instead: `import_worker_drain_verify.rs` (new, ungated — runs in the default sweep, no GPU: `assemble_import_graph` is CPU-only and `validate_def` is never reached) proves `Stage` shows the toast BEFORE `Done` dispatches the layer, `Failed` always shows a toast, and a second drop while busy queues FIFO rather than spawning a second worker.
  **Gate 2 (corrupt fixture):** `import_responsiveness_verify.rs` (new, `journey-proofs`-gated) `corrupt_glb_fails_through_worker_as_failed_event` — a hand-written garbage `.glb` drives the real `run_import_worker`, asserts the terminal event is `Failed` (never `Done`), and that `drain_import_progress` turns it into a visible toast.
  **Gate 3 (performer gesture):** same file, `content_thread_keeps_advancing_while_a_large_import_runs_in_the_background` — a real headless `ContentThread` ticks on its own thread (frame counter in an `AtomicU64`) while the real 43MB `ABeautifulGame.glb` imports via `import_model_file`/`drain_import_progress`; asserts the counter is monotonically non-decreasing and never stalls for the whole span. Measured this session: cold pipeline construction took ~5s (unrelated first-use cost, not import-caused) but once warm the counter advanced every ~1-2 samples across the entire ~2.4s import.
