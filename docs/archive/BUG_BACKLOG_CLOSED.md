...
stranded, zero errors. `CHAIN_FUSION_DESIGN.md` section 6 already flags this as an open item.
...
primitive to reuse, out of scope for this pass. Overrides TIMELINE_INGEST_DESIGN section 2 D1 (see
its section 3 for the full poll-failure writeup, now superseded).
...
directly, and the new `modulation::clear_all_trigger_edges(project)` (section 8 P1,
...
new section 8 edge-state holders, folded into the same reset point per the fix shape
...
documented in KICK_SWEEP_EVENT_DESIGN section P5: offline replay vs live stream legitimately diverge
...
**Found 2026-07-07** while gating U-P1 of `LIVE_AUDIO_TRIGGERS_DESIGN.md` section 9 (the
...
**Found 2026-07-07** while gating U-P2 of `LIVE_AUDIO_TRIGGERS_DESIGN.md` section 9 (the trigger-gate
...
2. `build_toggle_trigger_row` ([param_slider_shared.rs](../crates/manifold-ui/src/panels/param_slider_shared.rs))
   ...
   `is_trigger_gate` rows (section 9 folded them into the shared `ModTab::Audio` path) — this was
...
**Found 2026-07-07 by the PROJECT_IO_MAP read (section 9 E3).** `save_v2_archive` (archive.rs:196)
...
**Found 2026-07-07 by the PROJECT_IO_MAP read (section 9 E4).** `compute_hash` (archive.rs:289)
...
Found 2026-07-10 while wiring F1 (OSC/SMPTE timecode receive path, CORE_ENGINE_MAP section 13.1) —
...
(section 7/section 11: "no threshold — apply every OSC frame so drift never accumulates"), so any error this
...
**Fix:** bumped `COLOR_BASELINE` 200→201 in `crates/manifold-ui/tests/design_tokens.rs` with a comment folding the new pin colour into the same pin-colour debt the section 15 colour ramp will tokenise together (consistent with how the 2026-07-03 graph-editor pass re-baselined its own additions). Tokenizing one pin colour while its seven siblings stay raw would be inconsistent; all eight are section 15-ramp debt.
...
- **Part 2 (compose vocabulary) — LANDED, commit `9462498b`.** section 2.5 audit: genuinely NOT a new primitive — `node.switch_value` (identity default on the idle branch, e.g. `in_0: 1.0`) + `node.math` (Multiply) already compose exactly this, matching FluidSim3D's `vol_factor_mux`/`noise_factor` precedent. Documented as the `trigger_modulate` idiom in `docs/DECOMPOSING_GENERATORS.md` section 4.1 — **name unconfirmed by Peter, flagged below.**
...
1. **The `trigger_modulate` idiom name.** Working name only (Part 2 / `docs/DECOMPOSING_GENERATORS.md` section 4.1) — needs Peter's confirmation or a rename pass.
...
**Symptom:** `ClearLaneCommand` and `RemoveLaneCommand` exist in `manifold-editing` (`crates/manifold-editing/src/commands/automation.rs:306`, `:197`) but neither is referenced anywhere in `manifold-ui` or `manifold-app` — confirmed via `rg -n "ClearLaneCommand|RemoveLaneCommand" crates/manifold-ui crates/manifold-app` returning zero hits. The only shipped point-level edits are the AUTOMATION_LANES_DESIGN.md section 7 vocabulary: double-click a dot deletes it, marquee-select + Delete removes a selection. There's no one-click "clear this lane" or "remove this lane" button/menu item/keybinding.

**Fix shape:** wire `ClearLaneCommand` (clear all points, keep the lane) and `RemoveLaneCommand` (delete the lane entirely) to a UI trigger — most likely a right-click/context-menu item on the lane header, per the lane-header re-enable click precedent already in the design doc (section 7 "State affordances"). Design doc `docs/AUTOMATION_LANES_DESIGN.md` is otherwise implemented per its status board entry; this is a gap in that landing, not a new design.
...
- NOT the just-merged trigger-gate drawer (section 9): the drawer is CLOSED in the repro
...
read-path for an input storage `Array`. Classify cut rule 9 (FREEZE_COMPILER_MAP section 4) makes any
...
**Status:** FIXED @ `450f01c4` — missing-handle arms now reply `DecodeResultStatus::Error`, plus a bounded-wait backstop in `flush_pending_decodes`. Found 2026-07-12 during the MEDIA_EXPORT_MAP.md mapping pass (full read of `manifold-media`). See that map section 12 for the pipeline context of BUG-127..133.
...
**Status:** FIXED @ `63937590` (encode side `b692bb9a`) — shared `manifold_srgb_encode`/`manifold_srgb_decode` in `ColorTransferFunctions.h`, ported literally from `still_exporter.rs`'s tested constants, now used by both the SDR export shader and the decoder's linearization. Found 2026-07-12, MEDIA_EXPORT_MAP.md pass. Full transfer-function table: MEDIA_EXPORT_MAP.md section 7.
...
item, per design section 6 P2) will be the first real data point at show scale; if the shortfall grows
...
goes from the old fixed 25 to 97 (25 → 4× density, capped). **section 2.5-equivalent audit finding worth
...
**Root cause:** not investigated — `rg "pbrSpecularGlossiness|SpecularGlossiness"` across `gltf_import.rs`/`gltf_load.rs` returns zero hits; the extension is simply never read. Not in `GLB_CONFORMANCE_DESIGN.md` D5's scoped extension list and not covered by any section 7 deferred item (D5 names sheen/iridescence/anisotropy-the-extension/volume/Draco/KTX2/meshopt as the deferred set; spec-gloss isn't in that list — a genuinely new gap this session's audit surfaced).
...
**Status:** FIXED 2026-07-20 (script deleted per its own fix shape, W1-B)
...
### BUG-265 (inspector-card-drag-indicator-stale-geometry) — blue drop indicator/target index wrong after in-place scroll; hit-test used snapshot `card_y` + live `compute_height()`
**Status:** FIXED 2026-07-20 (W2-B lane `8cb1c437` + orchestrator fixup `94632d65`, merged `f2ac71d9`).
**Fix:** drag path hit-tests `ParamCardPanel::live_bounds(tree)` (read-through to the border node's scroll-current `UITree::get_bounds`); `card_y` field+accessor deleted crate-wide; after-last drop-index fallback now `max(effect_index)+1` (contiguous-tail assumption removed). 3 in-file conviction tests (scrolled red-before-fix off by scroll delta, mid-animation no-rebuild, settled regression guard).
**Residue (Peter-priority calls, unfiled):** drag auto-scroll, multi-select drop footprint, Esc-cancel — findings doc section Smaller improvements.
