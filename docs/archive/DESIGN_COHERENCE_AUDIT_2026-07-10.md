...
`DESIGN_BUILD_ORDER.md` section 1 claims to cover "every APPROVED-not-built design doc." Checked
...
**Present but shipped (should be removed per section 5 maintenance):**
...
- section 3 item 4: "MULTI_DISPLAY P2 ⏸ BLOCKED pending section 6.1 seam-hardening" — board says section 6.1a
  resolved 2026-07-06. Check DESIGN_HARDENING_QUEUE item 2.
- section 3 item 6: "MEDIA_BACKEND P1 ⏸ PARKED (section 3 addendum needed)" — board says section 3a addendum
...
- **REALTIME_3D** — surfaces: `render_scene.rs`, light.rs, camera atoms, viewport/gizmos (P5/P6), atmosphere port (P3), shadows (P2). Amended by SCENE_BUILD section 8 (D3/D8/P6 — annotated in status line, coherent). Anchors: `OBJECT_SLIDER_MAX=64` shipped (objects UNCAPPED on main); `MAX_LIGHTS=4` still in code (UNBOUNDED_LIGHTS unbuilt).
...
- **SIMULATIONS** — surfaces: new solver atoms, array_feedback state. Prereq REALTIME_3D P1 (shipped ✓). section 8 rigid-body deferral carries the BOX3D supersede note (cross-patched 2026-07-07 ✓).
- **BOX3D_PHYSICS** — surfaces: new `manifold-physics` crate, PortType::BodySet/ColliderSet, `node.physics_world`, render_copies (consume only). Supersede of SIMULATIONS section 8 recorded on BOTH sides ✓. Collider-vocab reconciliation explicitly deferred to SIMULATIONS section 2.5 audit ✓. Clean pair.
...
- **ML_NODES** — surfaces: manifold-native inference module, node.camera, Keypoint2D array, model cache. Prereq: ONNX tier needs VULKAN ✓ (in build order). Seams to verify in later clusters: MULTI_DISPLAY section 12 calibration; VIDEO_IO vs its deferred "NDI/Syphon in, same slot later" source claim.

**Cluster findings (kill-passed against second doc's text + code):**
- **F2 (HIGH) — REALTIME_3D P2 shadows is priced against caps that no longer exist.** D3/D4/section 6/section 7.3 assume "8 objects, 4 lights; the cap exists so the worst case stays bounded" (40 geometry passes). Shipped code: objects=64. Approved sibling UNBOUNDED_LIGHTS: lights→64 soft/127 hard. Worst case becomes 64 objects × 64 casters ≈ 4096 geometry passes; shadow-map cache keyed (node, light_slot) inflates likewise. Neither sibling amends the shadow story (UNBOUNDED_LIGHTS scope excludes it; shadows unbuilt). **REALTIME_3D must be amended before P2 executes**: caster budget/policy (e.g. cast_shadows honored for first-K lights, K a constant) + strike the stale cap text in D3/section 7.3. Opus-level (a real cost-policy decision).
...
- **F-clean:** SIMULATIONS↔BOX3D mutual supersede coherent; SPLATS P4 lazy-depth matches REALTIME_3D section 3's promise; SPLATS↔CHANNEL_TYPE_SYSTEM consistent; SCENE_BUILD prereq on BOUNDARIES now satisfied (update header phrasing when amending).
...
- **MULTI_DISPLAY** — surfaces: stage.rs, layer_compositor chain maps (P2 seam table section 6.1a), content_pipeline present path (P3), StageUniform + 3 atoms (P4), stage view UI (P5), LED/DMX/MVR/NDI/Syphon (P6). section 5a ONE-Stage-surface amendment claims matching addenda in PROJECTION_MAPPING/LED_STRIPS/APP_SHELL — APP_SHELL section 8 verified ✓; other two checked in LED cluster.
- **GIG_RESILIENCE** — surfaces: autosave/alerts (P1 ✓), breadcrumb/--resume (P2 ✓), manifold-understudy crate + perform arming (P3), peripherals (P4). P3's visible widgets = PERFORM_SURFACE section 7.0 ✓ both sides.
- **APP_SHELL** — surfaces: commands.rs table, menu.rs rebuild, two settings windows, AppPrefs. section 8 = the slot-contract reconciliation table (2026-07-06) — the strongest cross-doc coherence artifact in the corpus.
...
- **F7 (MED) — PERFORM_SURFACE still commits to a "Perform button" that APP_SHELL D4 (Peter, verbatim, 2026-07-06) forbids.** PERFORM_SURFACE D1 + Decided-#1 + P2 ("Perform-button context routing"): "One Perform button... No setting." APP_SHELL D4/R5 + section 1 audit: perform entry is menu-only, "no header/transport PERFORM button, no accelerator" — and as-built header has no mode buttons. APP_SHELL section 8 records that it decided the affordance; PERFORM_SURFACE carries no amendment. D1's surviving content (entry follows context: arrangement → timeline perform, session view → session perform) moves onto the menu item's routing. **PERFORM_SURFACE must change** (D1, Decided-#1, P2 wording). Mechanical.
...
- **F9 (LOW) — two deferral triggers have FIRED without the docs noticing:** (a) ABLETON_SHOW_SYNC D16 defers automation-envelope import "once AUTOMATION_LANES is built" — lanes shipped P1–P4 (2026-07-04); the executing session should decide fold-in vs keep-deferred at brief time. (b) ABLETON_SHOW_SYNC section 9 defers session-view scenes "after SESSION_MODE ships" — P1–P3 shipped; the SessionLaunchScene id-mapping SESSION_MODE section 5 punts to "the Ableton-sync project" is designed NOWHERE. Note in both briefs; no doc contradiction.
- **F-clean:** SESSION_MODE↔PERFORM_SURFACE pairing coherent both directions · GIG_RESILIENCEsection 7↔PERFORM_SURFACEsection 7.0 coherent · MULTI_DISPLAYsection 5a↔APP_SHELLsection 8 coherent · DJ_PERFORMANCE↔PRO_DJ_LINK symmetric · APP_SHELL D6↔MCP_INTERFACE catalog rule coherent (verify MCP side in param cluster).
...
- **AUDIO_OBJECT_INGEST** — conformance direction; prereq OBJECT_TRACKING P0–P2 (now satisfied); P1 blocked on Peter (labeled clips). section 8 relation contract mirrored both sides ✓.
- **AUDIO_SETUP_DOCK** — surfaces: ScreenLayout (new column), audio_setup_panel, OverlayId deletion, Layer.clip_triggers, live_trigger.rs, drawer builder. Declares supersessions of AUDIO_SENDS D6/section 3.3 + APP_SHELL section 7 — one-sided (F12). Owns BUG-047/070/082 ✓ matches memory.
...
- **F12 (MED) — AUDIO_SETUP_DOCK's amendments are one-sided.** It amends APP_SHELL section 7 (Audio Setup T2 modal → T1 workspace surface; Decided-#9 "stays a modal" now false), supersedes AUDIO_SENDS_UX D6/section 3.3, and outdates AUDIO_INFRASTRUCTURE section 7's "stays modal" — none of the three carry a back-reference. A reader of APP_SHELL section 7/section 8 or Decided-#9 gets the dead classification with no pointer. Mechanical: supersession lines in all three docs.
...
- **COMPONENT_LIBRARY** — surfaces: ComponentDef/registry (manifold-core), GroupParamDef extensions (section 4a), BindingDef.extra_targets, component picker (after node-groups canvas). Prereq VOCAB apply ✓ shipped. MCP section 5 amendment mirrored on BOTH sides ✓.
- **MCP_INTERFACE** — surfaces: new manifold-mcp crate, mcp channel into content thread, 14 tools, MoodBoard model on Project. APP_SHELL D6 command-table slot coherent (forward reference, one-way by design) ✓. MULTI_DISPLAY section 7.2 stage-summary note additive ✓.

**Cluster finding (kill-passed):**
- **F14 (MED) — SCENE_BUILD declares "GroupParamDef stays unused" while COMPONENT_LIBRARY's macro layer is built on it.** SCENE_BUILD section 1 audit row + Decided-#3 (2026-07-06): Phase D dead, "GroupParamDef stays unused," card bundling = ParamSpecDef.section; section 9 defers group interface params "for swappable 'rack' presets with stable knob surfaces." COMPONENT_LIBRARY (2026-07-02) section 4a extends GroupParamDef (fan-out targets, labels, ranges) as the component macro schema — and its section 9-deferred use case IS components, unnamed. The two compose mechanically (macros are declarations that lower onto ordinary card BindingDefs at expose — no live group-param runtime, so SCENE_BUILD's actual kill target stays dead), but the texts contradict on the type's future and neither cites the other. Amend both: SCENE_BUILD names COMPONENT_LIBRARY as the sanctioned GroupParamDef consumer (declaration-only, lowered to the card path); COMPONENT_LIBRARY notes card sections (ParamSpecDef.section) are orthogonal. Mechanical once the compose-check is confirmed (one Opus/Fable sentence).

### Cluster: io/export/media (COMPLETE — all 4 docs read whole 2026-07-10)

- **VIDEO_IO** — surfaces: manifold-native Syphon/NDI FFI, VideoSendDef in stage.rs/venue file, node.syphon_in/ndi_in source atoms, content-pipeline publish seam. Claims MULTI_DISPLAY P6's "NDI/Syphon outputs" item as its own — correctly, with citation ✓. License clearance section 0 thorough (Opus-read NDI agreement).
- **MEDIA_BACKEND** — section 3a addendum resolved (hardening queue item 1 ✓); P1 RE-ISSUABLE. Superseded section 3 trait kept for the record ✓ exemplary truth maintenance.
...
**Cluster findings (kill-passed):**
- **F15 (LOW-MED, mechanical) — NDI/Syphon ownership moved to VIDEO_IO; two docs still point at the old owner.** MULTI_DISPLAY section 10 P6 still lists "NDI/Syphon outputs" as its own future work; MEDIA_BACKEND's Deferred line says "NDI/Syphon (multi-display section 10 P6 owns those)"; ML_NODES section 4/section 14 defer "NDI in, Syphon in — same slot later" with no forward pointer. VIDEO_IO (2026-07-09) is now the owner and says so. Cross-patch three one-liners (MULTI_DISPLAY P6, MEDIA_BACKEND deferred, ML_NODES section 14) → VIDEO_IO_DESIGN.
- **F16 (folded into section 0) — build-order section 3 items 4/6 stale:** "MULTI_DISPLAY P2 ⏸ BLOCKED" and "MEDIA_BACKEND P1 ⏸ PARKED" — both hardening items resolved 2026-07-06, both phases re-issuable. Fixed in the build-order update.
- **F-clean:** VIDEO_IO↔MULTI_DISPLAY island-send trigger coherent · VIDEO_IO D3 fixture-routing unification carries its own VERIFY-AT-IMPL ✓ · COMMERCIALIZATION↔ANALYSIS_ACCURACY BUG-069 sequencing consistent (build order 13g: P2+P6 before launch-gate item 19) ✓ · MEDIA_BACKEND↔VULKAN section 6/section 8 pairing consistent both sides (verify VULKAN side in dev-infra pass).

### Cluster: LED/output + dev infra (COMPLETE 2026-07-10 — LED_STRIPS, PROJECTION_MAPPING, UI_HARNESS, PERF_BUDGET_GATE read whole; LIVE_RECORDING_PROOFS read through section 3 (decisions+seams — remainder is test detail with no cross-doc surface); UI_AUTOMATION assessed via its shipped P1–P2 + UI_HARNESS's audit of it; VULKAN checked at status/section 8/prereq lines)

- **LED_STRIPS / PROJECTION_MAPPING** — both carry the MULTI_DISPLAY section 5a home amendments (LED section 5a, PROJ section 6) dated 2026-07-06, matching APP_SHELL section 8's account exactly. The section 5a cross-patch claim is fully verified across all four docs ✓.
...
- **VULKAN** — background track; MEDIA_BACKEND section 6 pairing consistent both sides ✓; ML ONNX gating consistent ✓.

**Cluster findings:**
- **F17 (LOW) — three docs each add a pre-gig ritual with no shared home:** GIG_RESILIENCE section 10 (kill-drill), PERF_BUDGET_GATE P2 (perf soak "in GIG_RESILIENCE's checklist"), LIVE_RECORDING_PROOFS Tier 2 (recorder soak). Convergent, not conflicting — but the first to land should make GIG_RESILIENCE section 10 the single checklist naming all three. Mechanical.
...
|---|---|---|---|
| F2 | **HIGH** | REALTIME_3D P2 shadows priced against "8 objects, 4 lights" caps that shipped code (objects=64) + UNBOUNDED_LIGHTS (64/127) killed; section 7.3 "do not reopen" text now false; shadow caster budget undecided | REALTIME_3D (Opus) |
...
| F12 | MED | AUDIO_SETUP_DOCK amends APP_SHELL section 7/Decided-9, AUDIO_SENDS D6/section 3.3, AUDIO_INFRASTRUCTURE section 7 — none carries a back-reference | APP_SHELL, AUDIO_SENDS_UX, AUDIO_INFRASTRUCTURE (mech) |
| F14 | MED | SCENE_BUILD "GroupParamDef stays unused" vs COMPONENT_LIBRARY section 4 macro layer built on GroupParamDef; compose fine (declaration→card bindings) but texts contradict, no cross-refs | SCENE_BUILD + COMPONENT_LIBRARY (mech after 1 compose-confirm sentence) |
...
| F15 | LOW-MED | NDI/Syphon ownership moved to VIDEO_IO; MULTI_DISPLAY P6, MEDIA_BACKEND deferred-line, ML_NODES section 14 still point at the old owner | those three (mech one-liners) |
...
| F17 | LOW | Pre-gig ritual accretes from 3 docs with no shared checklist home | GIG_RESILIENCE section 10 when first lands (mech) |
...
1. ✅ F2 — REALTIME_3D: D4 sets the `MAX_SHADOW_CASTING_LIGHTS = 4` caster cap (first-K by
   slot, extras still illuminate); D3/section 3/section 6/section 7.3 stale "8 objects, 4 lights" cap text struck
...
2. ✅ F5/F6 — IMPORT: reality note refreshed (the assembler `assemble_import_graph` +
   ...
   already ship; placement resolved = manifold-renderer, D1/section 3's manifold-io superseded); P1
...
3. ✅ F10 — OBJECT_INGEST D5 (consume ANALYSIS_ACCURACY's `eval/`, don't fork) + D4 sequenced
   ...
   section 4.3 seam note added.
4. ✅ F4 — GAUSSIAN_SPLATS D10 re-anchored to render_mesh/render_copies; Transform-port
   override deferred to SCENE_BUILD section 9's shared trigger (F4b judgment); SCENE_BUILD section 9 names
   render_splats (F4a/c folded in).
5. ✅ F14 — SCENE_BUILD (audit row + Rejected + Decided-3 + section 9) and COMPONENT_LIBRARY section 4 both
