...
(step 1 = the verification contract in `DESIGN_DOC_STANDARD.md` section 10 + `VERIFICATION_DEBT.md`,
...
- **Build-queue tranche (8 docs):** main-context deep review — every fork classified
  per the section 2 no-unlabeled-forks rule, anchors re-run against the tree, section 10 level
...
Peter's rule as `DESIGN_DOC_STANDARD.md` section 8.9 — **a landing is not done until the
...
| AUDIO_SENDS_UX | Clean, freshest anchors of the tranche (4/4 exact). Fixed: D5 "Source" confirmation hadn't propagated into section 2/P4, which still read "blocked". P2–P4 already gate on headless PNGs (L2). |
...
| REALTIME_3D | Status was "not built" — actually P0/P1/P4/section 9 shipped. **As-built P1 deviation: object transforms NOT port-shadowed yet** (render_scene.rs header) — flagged for P2+. Remaining: P2 shadows, P3 atmosphere, P5–P7 viewport/gizmos/preset. |
...
| SESSION_MODE | Status honest. P4/P5 briefs are one-liners — fine while P4 is Peter's hands-on phase, MUST be expanded to section 5 form before agent delegation. Verified: `ContentState` carries no session play-state yet — P4's snapshot fields are greenfield. |
...
1. **Detect concurrency → block/reject** (AUDIO_CLIP_DETECTION section 10): button disabled
   with visible "detection busy"; no queue ("so the user knows they can't
   auto-populate multiple audio files at once").
2. **Audio bundling → reference by path, never embed** (AUDIO_LAYER section 7/section 10, `e1ea6a7b`);
...
- **Verify surprising tool output before it becomes a doc edit.** A `-rln` flag typo
  ...
  section 10: an oracle you haven't sanity-checked is not an oracle.
- Docs one day old can already lie: PARAM_STORAGE P1 landed the same morning its doc
  said "not built". section 8.9 exists because review-time truth passes cannot keep up with
  landing-time drift.
