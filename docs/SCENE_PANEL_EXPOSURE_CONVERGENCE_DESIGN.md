...
**Status: APPROVED — §5 decisions RESOLVED by Peter 2026-07-19. P1 (exposure stamping) + P2 slice 2a (unified properties card, real params replace the synth-id system) + P2 slice 2b (funnels/hand-tables deleted, scene clicks join the `RowIndex`/`row_action` pattern) COMPLETE, on `lane/scene-panel-exposure-convergence`. P3 (outliner/discovery slimming) and P4 (polish + flow rewrite) remain. BUG-291 (P1 exposure-binding fan-out misattribution) and BUG-292 (scene rows writing via active layer instead of the panel's bound layer) FIXED on the lane — see `docs/BUG_BACKLOG.md`.**
