...
**Status:** SHIPPED (P1+P2) 2026-07-10 — proof suite on main: injection seams + Tier-1 harness; `recording-soak` bin, kill-durability test, runbook in `docs/DEVELOPMENT_REFERENCE.md`. **P3 (in-app record smoke) DEFERRED 2026-07-10 (Peter):** `ui-snap` has no live content thread or compositor, so a real record smoke is a new integration harness — revival trigger in section 8 (Deferred); the button→command→capture glue is L4-verified by live use (VD-023). Carried: VD-022a — the full-scale 4K60 20-min soak is Peter's pre-gig ritual; VD-022b — BUG-086 (recording-audio-track silent audio-drop, LOW after `--realtime`). Release-gating per STRUCTURAL_AUDIT_VERDICTS. · design 2026-07-07 · Fable · approved 2026-07-09 Peter
**Prerequisites:** none
