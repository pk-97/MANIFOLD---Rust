...
### Phase 4 — "Source" rename (strings only; blocked on D5 confirmation)
- **Entry state:** Peter has confirmed the word. `rg -in '"[^"]*send[^"]*"' crates/manifold-ui/src crates/manifold-app/src` re-derived at execution (the baked count WILL be stale).
