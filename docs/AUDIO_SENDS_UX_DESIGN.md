...
Companion docs: `AUDIO_MODULATION_DESIGN.md` (the feature this UX fronts), `AUDIO_INFRASTRUCTURE.md` (capture/analysis stack — **section 3.2 (The Audio Setup — one place to route and label)/section 8 are stale**, corrected by Phase 1), `LAYER_CONTROLS_DESIGN.md` section 5.3 (layer→send routing), `OVERLAY_SYSTEM_DESIGN.md` (modality machinery Phase 3 touches).
...
- **Deliverables:** consumed-set cache in `audio_mod_runtime.rs`; skip logic in the per-send loop; `AUDIO_INFRASTRUCTURE.md` section 3.2 (Analysis — split across the downmix worker and the content thread)/section 8 rewritten to content-thread reality with the measured numbers (16 sends ≈ 0.96 ms mean / 1.27 ms worst per tick, release).
