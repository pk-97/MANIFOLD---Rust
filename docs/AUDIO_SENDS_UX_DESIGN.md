...
Companion docs: `AUDIO_MODULATION_DESIGN.md` (the feature this UX fronts), `AUDIO_INFRASTRUCTURE.md` (capture/analysis stack — **§3.2 (The Audio Setup — one place to route and label)/§8 are stale**, corrected by Phase 1), `LAYER_CONTROLS_DESIGN.md` §5.3 (layer→send routing), `OVERLAY_SYSTEM_DESIGN.md` (modality machinery Phase 3 touches).
...
- **D6 — The panel stops dimming the show and moves aside.** Audio Setup becomes a non-dimming overlay anchored to the viewport's right edge (~38% width, full height); the preview stays visible. Trigger sensitivity, gain, floor, and crossovers are calibration against live output — a dimming 80% modal makes the panel's own purpose impossible. Device picking doesn't need dimming either, so the panel stays one surface. Supersedes `AUDIO_INFRASTRUCTURE.md` §7 (The audio-settings UX) "stays modal" — that call predates the trigger matrix growing in here. Rejected: splitting into a modal Devices panel + docked Tuning panel — two surfaces for one concept re-creates the fragmentation D1 kills. **Superseded in turn (coherence audit F12, 2026-07-10): `AUDIO_SETUP_DOCK_AND_TRIGGER_UNIFICATION_DESIGN.md` (approved 2026-07-09) moves this non-dimming overlay to a real `ScreenLayout` workspace column — the reasoning that killed the dimming modal kills the overlay too (it still occludes the inspector). This D6/§3.3 (Evaluation (manifold-playback)) shape is completed history, not the current target.**
...
- **Deliverables:** consumed-set cache in `audio_mod_runtime.rs`; skip logic in the per-send loop; `AUDIO_INFRASTRUCTURE.md` §3.2 (Analysis — split across the downmix worker and the content thread)/§8 rewritten to content-thread reality with the measured numbers (16 sends ≈ 0.96 ms mean / 1.27 ms worst per tick, release).
...
- **Per-send attack/release in the setup panel** — already rejected in `AUDIO_INFRASTRUCTURE.md` §7 (The audio-settings UX); belongs in the drawer if ever.
