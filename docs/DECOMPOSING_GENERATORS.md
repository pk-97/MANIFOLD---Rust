...
- **Every single-dispatch GPU primitive ships on the freeze codegen path (fusable).** Peter's standing rule (2026-07-11): all nodes, new and existing, must work perfectly with the graph compiler in full. Author a `wgsl_body` fragment + declare `fusion_kind`/`input_access` in the `primitive!`, build the runtime pipeline from `standalone_for_spec::<Self>()`, and prove a generated-vs-hand parity test in `gpu_tests` — never a plain `create_compute_pipeline(include_str!(…))` runtime kernel, which is a **fusion boundary** (N dispatches where a fused run costs ~1; dropped frames on heavy meshes at the rig). Full recipe + the multi-pass scope boundary in [ADDING_PRIMITIVES.md §"The codegen path is mandatory"](ADDING_PRIMITIVES.md). Scope: single-dispatch per-element atoms only — genuinely multi-pass primitives (a barrier-separated scan/reduce like `spawn_from_mesh`/`scatter_on_mesh`) stay hand-authored multi-dispatch, and forcing them into one kernel would violate §1.1's no-fused-monolith rule instead. Existing plain-WGSL atoms are tech debt to convert.
