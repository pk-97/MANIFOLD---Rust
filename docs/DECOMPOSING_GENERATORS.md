...
What's *not* part of that tax is extending adjacent primitives for port-shadow — that work shouldn't exist. New primitives should ship port-shadow on every numeric param by default (see section 6.2). WireframeZoo needed `rotate_3d` and `project_3d` extended only because they were authored before this convention was established. Future primitives should be authored to this convention so the only first-time cost is the genuinely new functionality.
...
3. **Reconcile your sketch against section 1 and section 2.** If your sketch invents a primitive that the reference already covers, drop the invention. If your sketch invents a primitive that's *almost* but not exactly an existing one, the right move is to extend (see section 6.2), not to add. Only after the audit gives you a list of (existing primitives you'll reuse, existing primitives you'll extend, genuinely new primitives you'll build) do you start workflow step 1.
...
`wgsl_compute_0in_1tex`, `wgsl_compute_1tex_1tex`, `wgsl_compute_2tex_1tex`. **Read section 5 before reaching for these.**
...
The `wgsl_compute_*` nodes let a JSON preset embed raw WGSL. The escape hatch is a sharp tool with a narrow remit: irreducible multi-pass coupled shaders. Most things that *feel* like section 5 cases aren't.
...
- **"Too much infra for this one generator."** The new primitive is almost always worth it — it earns its keep when the next generator needs the same shape. The audit precondition (section 2.5) is how you verify the shape doesn't already exist before paying that cost.
...
Apply this to anything you're tempted to call a section 5 case. If the answer is "no, this primitive is only useful for this one generator," the path is decomposition into atoms (which other generators *will* want), not a per-shader wrap. The only exception is the genuinely irreducible chain — and "irreducible" means the precondition test in (1) above is met: both coupling and per-pass format choices are load-bearing.
...
**Default: every curated kernel atom-decomposes.** The math becomes visible by being expressed as a graph of named atoms in the editor, not by exposing shader source on a sealed primitive. TouchDesigner's CHOP pattern is the canonical reference: a Lissajous curve in TD isn't a "Lissajous CHOP" — it's a Pattern CHOP (generates the t-parameter as a sample array) → Math CHOP (multiplies by frequencies) → Function CHOP (sin / cos applied elementwise) → Merge CHOP (combines x and y channels) → To-SOP. The math is visible *as the graph itself*. If a user wants a Rose curve instead of a Lissajous, they swap the Math CHOP's formula or rewire the Function CHOPs. That's the section 0 framing in practice: the primitive library is the product, the graph is the authoring surface, the math is read-and-recomposable through the editor without ever touching shader code.
...
- **Every barrier-free per-element GPU atom ships on the freeze codegen path (fusable).** Peter's standing rule (2026-07-11): all nodes, new and existing, must work perfectly with the graph compiler in full. Author a `wgsl_body` fragment + declare `fusion_kind`/`input_access` in the `primitive!`, build the runtime pipeline from `standalone_for_spec::<Self>()`, and prove a generated-vs-hand parity test in `gpu_tests` — never a plain `create_compute_pipeline(include_str!(…))` runtime kernel, which is a **fusion boundary** (N dispatches where a fused run costs ~1; dropped frames on heavy meshes at the rig). Full recipe + the authoritative in/out scope test in [ADDING_PRIMITIVES.md section"The codegen path is mandatory"](ADDING_PRIMITIVES.md). Scope in one line: the kernel must be a barrier-free pure per-element function (one thread per output element — no workgroup memory/barriers/atomics, no cross-frame state, no CPU IO). Exempt: barriered reductions (`peak`, `luminance`) and multi-pass scan/reduce (`spawn_from_mesh`/`scatter_on_mesh`) — forcing them into one kernel would violate section 1.1's no-fused-monolith rule; cross-frame state (`temporal`); IO/bridge atoms; the `render_*` draw calls. An atom that passes the test but lacks a codegen read-path (the `draw_*` family's array-into-texture read, BUG-114 (draw-family-blocked-on-array-into-texture-codege…)) is BLOCKED — a tracked compiler gap, not an exemption. Existing plain-WGSL atoms are tech debt to convert.
...
- **`composition_notes` in the `primitive!` macro slot** — the AI authoring surface. Even though no UI consumes it today (per audit section 8), every primitive must populate it with a short, specific note describing when/how to compose this primitive. This is the vocabulary an MCP/API agent will read.
...
- **f16-starved stateful loop.** A `wgsl_compute`/feedback loop whose per-frame increments are small (reaction terms, decay constants, force integration ~1e-4) silently dies or freezes when any hop in the loop is f16: the increment underflows the representable step at working values (~3e-4 at 0.3) and quantizes to zero. Bit-parity tests at one frame won't catch it — the loop looks alive for its first seconds. Fix: fp32 end-to-end around the loop (`outputFormats` override on the feedback node, `rgba32float` storage in the kernels, no f16-locked atom like `mix` inside the loop — fold injections into the first kernel). This is section 5's "per-pass format choices are load-bearing" observed in the wild (Gray-Scott, 2026-07-16). Related trap found the same session: textbook simulation constants often belong to a differently-scaled formulation and die faithfully — verify the discretization with a 20-line NumPy ground truth BEFORE debugging the graph.
- **Reference-preset feature copied to the wrong stage.** When mirroring a tuned preset (FluidSim is the usual reference), check WHICH path each feature lives on before copying: FluidSim's gaussian blurs smooth its *force field*, never the displayed density — copied onto the display path they read as "blur all over it"; its `radius_mode: Dynamic` expects a resolution-scaled radius wire and misbehaves unwired; its display brightness depends on the `canvas_area_scale → tone intensity` wire, without which brightness shifts with project resolution. The section 2.5 read-the-reference-end-to-end step exists exactly for this — follow each feature's wires to see what it actually feeds (Cymatics, 2026-07-16).
- **Additive feedback loop eats the alpha channel.** Symptom: the RGB provably carries the image (raw f16 readback shows HDR values) but the layer displays black — the premultiplied compositor multiplies by an alpha that is 0 everywhere. Cause: `node.mix`'s standardized alpha rule is `out.a = mix(a.a, b.a, amount)`, so a `Max`/`Add` accumulation loop whose `b` leg comes through `node.feedback` locks alpha at the loop's initial (black, transparent) state forever — the strike/injection alpha on `a` never enters. Every downstream pointwise atom (scale_offset, tone maps, flash) passes alpha through, so the 0 rides to the output, and the harness PNG *and* the app both show black. Fix at the display terminus: `node.set_alpha` (alpha=1) after the tone map — the composable form of the `alpha = 1.0` that `resolve_scatter`/`resolve_accumulator` bake in-kernel. Debug recipe that found it: bisect taps of each stage to `final_output` + a raw f16 readback dump — the PNG encoder is alpha-aware, so "black PNG" is NOT evidence the RGB is black (Lightning, 2026-07-16).
- **Dead-state param or input.** Symptom: performer drags an outer-card slider and nothing changes visually, no error, no log. Caused by a code path inside a primitive's `run()` (or shader) that branches on mode / wiring / state and skips reading certain params or inputs on one branch. The render_lines `edges`-wired path was the canonical instance (wiring `edges` activated a topology code path that didn't read `animate` / `speed` / `window`); smoothstep_texture's Bipolar mode was the canonical *mode-gated* version (the slider was always interactable, but `low` was inert when `Mode = Bipolar`). The section 7 invariant above ("No dead-state params or inputs") is the design rule. Review diagnostic: for every conditional branch in `run()` (and in the shader), list which params/inputs each branch reads, and check that every declared param/input shows up in *every* branch. If the answer is "no", the primitive needs the section 7 rework, not a warn.
...
If you're starting a decomposition: re-read section 3 (the workflow). If you're stuck partway: re-read section 5 (escape hatch) and section 6 (keep it small) — most stuck moments are one of those two. If you find a new bug class, write it up in section 8 and update the post-audit log.
