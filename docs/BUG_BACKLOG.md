...
| BUG-160 | **editor-window-unification-inspector-card-layout-regressions** | Root cause FOUND + fix APPROVED 2026-07-14 (Fable): `CardContext::Author` changes row GEOMETRY (chevron lane subtracted from `slider_w`, `param_card.rs:2469`/`:2702`) so the editor's cards lay out differently from the main window's by construction; label clipping reproduced in `ui-snap editor`. Design: GRAPH_EDITOR_INSPECTOR_UNIFICATION.md **Change 4** (layout invariance — context-independent geometry, one row-geometry helper, tick parity, tree-equivalence gates). Overnight lane executes P1+P2. MED-HIGH (UI regression). |
