...
**Status:** OPEN — MED (blocks a fair Stage-1-vs-ADTOF read; the B3 verdict's electronic kick 0.31 is measured through this bug, so Stage-1's true ceiling is unknown; ADTOF path unaffected — this is the challenger's front-end).
...
**Status:** CLOSED 2026-07-18 — fixed in SCENE_PANEL_CARD_CONVERGENCE_DESIGN.md C-P1c (`wave/scene-card-convergence`). `EyeSlot::Dimmed` deleted from `scene_setup_panel.rs`; the Camera/World/Light outliner rows now use `EyeSlot::Empty` (draws nothing, keeps the reserved slot width) — Custom Light rows use the same empty slot, Custom Object rows keep the dimmed glyph (Object DOES carry a real `visible` param, this instance just isn't addressable).
