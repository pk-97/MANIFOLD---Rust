...
### BUG-266 (scene-panel-card-path-rows-write-via-active-layer-not-the-panels-bound-layer) — P2 slice-2a scene rows dispatch through the generator card path, which resolves the target through the app's `active_layer` instead of the scene panel's own bound layer — a wrong-layer-write regression vs. the old scene rows
**Status:** OPEN
