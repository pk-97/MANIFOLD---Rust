...
### BUG-149 (mute-chip-press-motion-teleports-hit-bounds-after-scroll) — the mute chip's press animation re-applies a stale build-time Y after the scroll fast-path moves rows, teleporting its draw + hit bounds off the row — HIGH (perform-surface control fails first click)
