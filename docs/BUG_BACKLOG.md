...
### BUG-175 (glb-vertex-colors-not-wired-color0-never-read) — glTF's `COLOR_0` vertex attribute is never read anywhere in the mesh pipeline, so per-vertex color (the entire point of `BoxVertexColors.glb`) has no path from import to pixel
...
### BUG-176 (gltf-import-manual-is-multiple-of-clippy-lint) — `cargo clippy -p manifold-renderer --tests -- -D warnings` fails on two pre-existing `while len % 4 != 0` loops clippy's `manual_is_multiple_of` lint now flags
