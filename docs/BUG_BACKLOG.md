...
**Status:** OPEN (crash still unreproduced) — P1 done 2026-07-17 (found no reproduction, RSS evidence recorded); **P2 SHIPPED 2026-07-17** (`lane/import-responsiveness`, this session) — D2's duplicate-device deletion landed and verified; the crash's actual cause is still unconfirmed (P1's UI-thread-stall/watchdog hypothesis is untested), so P3 (background worker, `IMPORT_RESPONSIVENESS_DESIGN.md`) is what's expected to close this. IMPORT_RESPONSIVENESS_DESIGN.md P1 (2026-07-17, lane/import-responsiveness) drove the REAL interactive path (`Application::import_model_file`, unmodified — see below) and did **NOT reproduce the crash**, across 2 process runs of 3 sequential imports and a 3rd process run of 8 sequential imports, all with a real background content-shaped GPU runtime alive concurrently (`bug219_interactive_import_sequential_pressure` at `crates/manifold-app/src/bug219_verify.rs`, on this machine's build at HEAD of this branch). D2/D3/D4 proceed regardless per the design's D1 — the throwaway validation `GpuDevice` and the doubling this entry already narrowed are still deleted in P2 on their own merits, independent of whether P1 convicted them for the crash specifically. See the P1 finding below for what the evidence actually supports and Peter's build/hardware is asked for to close the gap.
