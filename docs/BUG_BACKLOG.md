...
### BUG-181 (hdri-exr-files-fail-or-fail-silently) — Peter's real .exr HDRI files don't work through `node.hdri_source`, despite the atom's claimed .exr support — MED (glb-import lighting / HDRI env_mode)
**Status:** OPEN — logged 2026-07-16 at Peter's report ("HDRi needs to support .exr for my HDRi files").
