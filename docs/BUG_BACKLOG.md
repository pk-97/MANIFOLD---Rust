...
|---|---|---|
| BUG-200 | **khr-animation-pointer-channels-fail-to-deserialize** | Any glTF asset with a `KHR_animation_pointer` animation channel fails at the raw serde deserialize step (`missing field 'node'` — the extension legally omits `target.node`, the pinned `gltf-json` 1.4.1 doesn't), even when the extension is only `extensionsUsed`. Renumbered from a duplicate BUG-187 2026-07-17. Real scoped work (crate patch or raw-JSON preprocessing), queued behind Peter's call on GLTF_ANIMATION follow-ups. |
...
### BUG-200 (khr-animation-pointer-channels-fail-to-deserialize) — any glTF asset with a `KHR_animation_pointer` animation channel fails to import at all, even when the extension is only `extensionsUsed` (not required)
**Status:** OPEN — found 2026-07-16 during GLTF_ANIMATION_DESIGN.md A1 (BoxAnimated.glb rigid-animation vertical slice). (Renumbered from BUG-187 2026-07-17 — that id was already taken by the mesh-quantization xfail, an ID collision between two concurrent sessions; the manifest's `xfail:BUG-187` stays with mesh-quantization.)
