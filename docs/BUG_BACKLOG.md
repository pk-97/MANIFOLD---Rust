...
Probe run 2026-07-11 (Peter live, flicker CONFIRMED on-screen during the window): every filmstrip counter zero — captures ~1/s, no alloc fails, no thumb-pass skips, no empty layouts, surface_timeouts=0. The FILMSTRIP/ATLAS PATH IS EXONERATED, and so is the fence-timeout handler for this repro. Flicker correlates purely with GPU saturation (fence_wait_ms 20-44 even semi-idle at 4K) and stops at low instance counts. Next: probe the UI clip-body/lane draw itself (which element paints blue, front_index per frame, thumb quads drawn) or a 5s screen recording during flicker to identify the flashing element.
