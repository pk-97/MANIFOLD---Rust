...
| BUG-160 | **editor-window-unification-inspector-card-layout-regressions** | Root cause FOUND + fix APPROVED 2026-07-14 (Fable; twice corrected by Peter same day — final): card content does not fit the card's HEIGHT — rows draw past the card's bottom edge; likely mechanism is the never-ticked editor panel instance (stale drawer-height state, D4), with width-fit gaps and the Author chevron lane as secondary. Execution order: Change 4 P2 (tick parity/height) FIRST, then P1. Design: GRAPH_EDITOR_INSPECTOR_UNIFICATION.md **Change 4** — D3 fit-at-every-width (primary), D7 width value per-window / width policy shared (Peter: everything except width "fundamentally the same"), context-independent geometry, tick parity, tree-equivalence gates. Overnight lane executes P1+P2. MED-HIGH (UI regression). |
