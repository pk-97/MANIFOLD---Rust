...
| BUG-099 | **design-tokens-raw-color-literal-count-drifted-past-baseline** | `manifold-ui`'s `no_new_raw_color_literals` ratchet fails on main (baseline 200, live 201) — pre-existing, confirmed via `git stash` against P2's base commit, nothing this session touches UI color code. LOW. |
...
### BUG-099 (design-tokens-raw-color-literal-count-drifted-past-baseline) — `manifold-ui`'s `no_new_raw_color_literals` ratchet test fails on main (baseline 200, live count 201) — LOW
**Status:** OPEN — found 2026-07-10 running SCENE_BUILD_AND_GROUP_PARAMS_DESIGN.md P2's full workspace sweep gate; confirmed pre-existing via `git stash` against the P2 base commit `ab215ab8` (fails identically with zero P2 changes applied) — nothing in P2 touches UI color code, so this is drift some earlier landing on `main` left behind without bumping the baseline or adding a `// design-token-exempt:` marker.
**Symptom:** `cargo test -p manifold-ui --test design_tokens` fails: `Raw Color32::new( count rose to 201 (baseline 200)`.
**Root cause:** unknown — the test only counts occurrences, it doesn't name the new call site. Suspect: a recent UI landing added one `Color32::new(...)` without going through a `color::` token or adding the exemption comment the guard accepts.
**Fix shape:** `rg -n "Color32::new\(" crates/manifold-ui` and diff against the last known-good count to find the new site; either route it through an existing `color::` token or annotate it `// design-token-exempt: <reason>` if it's a genuine one-off, then the baseline in `design_tokens.rs` can stay at 200. Out of scope for P2 (no UI files touched this session) — left open rather than fixed blind.
