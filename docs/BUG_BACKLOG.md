...
### BUG-139 (bug-status-rebuild-drops-fixed-pointer-lines) — bug_status.py's rebuild() silently drops the ## Fixed archive-pointer lines — MED, found not fixed 2026-07-12 (log_bug.py authoring session)
**Status:** OPEN
...
### BUG-140 (glb-import-non-square-aspect-distortion) — glTF import appears to distort aspect ratio — color-checker card renders non-square — MED, found not fixed 2026-07-12 (reported by Peter, screenshot only, not investigated)
**Status:** OPEN

**Symptom:** Peter screenshot of an imported glb scene (flower photogrammetry scan) shows the reference color-checker chart rendered visibly stretched/non-square, suggesting the glTF import path is not preserving correct aspect ratio for at least some geometry or texture mapping.
**Root cause:** unknown — not investigated. Suspects: UV/texture aspect handling in the glTF texture import, mesh scale/transform applied on import, or camera/viewport aspect at capture-preview time.
**Fix shape:** Reproduce with the same asset, compare source glb dimensions vs imported scene; check gltf_mesh_source / glb import transform code and any hardcoded UV or camera aspect assumptions.
