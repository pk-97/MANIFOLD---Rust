...
### BUG-100 (pristine-generator-outer-routings-miss-per-instance-user-bindings) — a pristine (undiverged) generator instance's own exposed-param bindings never surface in the graph canvas's `outer_routings` — MED (breaks the "↳ outer" hint AND SCENE_BUILD_AND_GROUP_PARAMS_DESIGN.md D6's group-face mirror rows for exactly the instances that need them most: glTF imports)
**Status:** OPEN
...
**Symptom:** open the graph editor on a freshly glTF-imported generator layer (or any other pristine — no per-instance graph override — generator instance that has PER-INSTANCE exposed/user-added card bindings, as opposed to bindings baked into the bundled/embedded preset's own compile-time metadata): the card correctly shows the exposed sliders (it reads the instance's own manifest), but the canvas never shows the "↳ <outer label>" hint on the driving inner-node row, and (as of this phase) a group box containing that inner node never shows the D6 mirror row either — because the canvas's `outer_routings` for this instance simply doesn't contain the binding.
**Root cause:** `ContentThread::graph_snapshot` (`crates/manifold-app/src/content_thread.rs:1307-1420`) branches on `layer.generator_graph()`: when `Some` (per-instance override — e.g. after `ToggleNodeParamExposeCommand` diverges the instance), it correctly re-derives `outer_routings` from the override def's OWN `preset_metadata.bindings` (`content_thread.rs:1373-1397`), capturing exactly the newly-added per-instance binding. When `None` ("pristine" — tracks the shared preset by id, which is what `ImportModelLayerCommand` deliberately leaves a glTF import at, per its own comment "No override graph: the instance keeps `graph: None`… mechanism A"), it instead calls `loaded_preset_view_by_id` + `snapshot_for_view` (`content_thread.rs:1405-1406`), which derives `outer_routings` PURELY from the canonical/embedded def's own static `preset_metadata.bindings` (`loaded_preset_view.rs:232-261`) — a `&'static` object shared by every instance of that preset type, structurally unable to see any ONE instance's own per-instance `user_param_bindings`/manifest-level bindings. Verified empirically: `fixtures::gltf_scene()`'s imported layer's `outer_routings` (via this exact pristine path) contained only the 9 compile-time-static camera/sun/environment bindings — none of the per-object Metallic/Roughness bindings the SAME layer's card correctly shows (those live on the instance's own manifest, seeded by the importer via a different write path than `preset_metadata.bindings`).
**Fix shape:** the pristine branch needs to merge the instance's own per-instance user bindings (however PARAM_STORAGE_BOUNDARIES now stores them — the manifest/`user_param_bindings`, whichever is the single source of truth post-boundaries-wave) into `snap.outer_routings` alongside the canonical static ones, mirroring what the override branch already does correctly for a diverged instance. Likely lands as a small addition inside `graph_snapshot`'s pristine arm (project the instance's own bindings the same way `outer_routings_from_bindings` in `param_binding.rs:764` does for effects) rather than a new mechanism. Out of scope for SCENE_BUILD_AND_GROUP_PARAMS_DESIGN P4 (display + dispatch only, no data-plumbing changes) — D6 itself is proven correct independently by the `manifold-ui` unit suite (`graph_canvas::tests::group_face_row_appears_for_inner_targeted_card_param` et al.) and by a synthetic-but-real-render-seam fixture (`ui-snap groupdemo`); this bug is upstream of D6, in the snapshot feed, not in the row-rendering mechanism.
