...
|---|---|---|
| BUG-149 | **glb-import-fog-slider-per-world-unit-cliff** | The glb importer's Fog Density card slider (0–1) maps 1:1 onto `node.atmosphere`'s **per-world-unit** `fog_density`, ignoring scene scale — at the importer's own auto-framed camera distance (~28 units on the apricot fixture) slider 0.13 is optical depth ~3.6 ≈ 97% fog, so the whole mesh flattens to fog-grey, and the shaft march (in-scattering ∝ fog_density × sun intensity) blows out the entire frame at God Rays 0.21. Usable slider range on that model is ~0.00–0.04. Fix shape: normalize in the importer by its own framing distance / bounds radius (e.g. density = slider / camera_distance) so the fader is perceptually scaled on any model. MED-HIGH (shipped card control unusable live). |
...
| BUG-147 | **bokeh-gather-cpu-reference-helpers-dead-without-gpu-proofs** | `bokeh_gather.rs`'s `#[cfg(test)]` module declares `BOKEH_N`/`BOKEH_GOLDEN_ANGLE`/`bokeh_hash_angle`/`Plane4` (+ its `texel`/`sample` methods)/`bokeh_gather_texel` — the CPU-reference parity harness — but they're only consumed by the inner `#[cfg(all(test, feature = "gpu-proofs"))]` submodule, so a plain `cargo test`/`cargo nextest` (no `gpu-proofs`) compiles the outer `#[cfg(test)]` module without ever using them and rustc emits `dead_code` warnings. Cosmetic (the standard scoped `clippy -p manifold-renderer -- -D warnings` gate, no `--tests`, doesn't compile test code at all and stays clean) — found via IDE diagnostics while gating an unrelated P6 (`bilateral_blur`) session, not caused by that diff (`bokeh_gather.rs` untouched, `git log` shows its last change is P4's original commit `d85c6dc0`). Now confirmed SYSTEMIC: the identical shape reappeared in `bilateral_blur.rs` the moment it compiled post-merge. Fix shape: nest the outer items under the same `#[cfg(feature = "gpu-proofs")]` gate as the submodule that uses them, or move them inside it directly. LOW (lint-only, no correctness impact — same family as BUG-126). |
| BUG-146 | **render-scene-atom-pipelines-never-prewarmed** | A generator layer's first frame pays every atom's lazy codegen-pipeline compile even with `prewarm_all` run — confirmed with `node.cube_mesh` (~41.5ms on frame 0 with a control scene that has shadows/shafts fully off). `GeneratorRegistry::prewarm_all` only builds bundled presets' graph *structure*, never renders a frame, so any atom whose pipeline compiles inside its own `run()` (not covered by an explicit prewarm call like BUG-037's or BUG-145's) stays cold until some project's live first frame hits it. Fix shape: render one throwaway frame of every bundled preset at startup, or prewarm every registered atom's codegen pipeline unconditionally. LOW-MED (first-frame stall, not steady-state). |
| BUG-144 | **prewarm-cache-tests-flake-under-full-lib-parallel-run** | `render_scene::gpu_tests::prewarm_pipelines_populates_the_shared_render_cache` and `gltf_texture_source::gpu_tests::prewarm_pipeline_populates_the_shared_compute_cache` both assert a process-global shared-pipeline-cache count grows by their own prewarm call (`before=N, after=N` failure — no growth observed); reproduces deterministically under the full `cargo test -p manifold-renderer --features gpu-proofs --lib` parallel run but PASSES in isolation (`--test-threads=1` or filtered to just that test). Confirmed pre-existing and order-dependent on FOUR independent occasions now: bisected via `git stash` during VOLUMETRIC_LIGHT_DESIGN P1 (2026-07-13), and separately during the BUG-111/BUG-054 landings and CINEMATIC_POST P6/P5 (`bilateral_blur`/`ssao_gtao`) — all on the unmodified tree, unrelated to any of those changes. Root cause: two tests racing to prewarm the SAME global cache key concurrently — whichever runs second finds the entry already there from the other, so its own "before != after" growth check fails. Fix shape: either serialize these two tests against each other (a named-lock or `#[serial]`-style guard), or key each test's assertion off a cache entry unique to that test's own prewarm call instead of the raw global count. LOW (flaky-gate under parallel `--lib`, not a functional regression; doesn't affect `nextest` since these live behind `gpu-proofs` and are excluded from the default sweep). |
...
**Status:** OPEN — found 2026-07-14, Peter live on `SceneLadders.manifold` (apricot-blossom glb, one session after `59010e84` added the sliders).
...
**Status:** OPEN — found 2026-07-13 as the residual left over after BUG-145's shaft/shadow prewarm fix, during VOLUMETRIC_LIGHT_DESIGN P3's `MANIFOLD_RENDER_TRACE` content-thread perf gate.
...
**Root cause** — `GeneratorRegistry::prewarm_all` (`crates/manifold-renderer/src/generators/registry.rs`) only (a) builds every BUNDLED preset's graph *structure* via `PresetRuntime::from_json_str_with_device`, which never calls any node's `run()` (the comment there says so explicitly), and (b) as of BUG-037, explicitly prewarms `RenderScene::prewarm_pipelines` (now also covering BUG-145's shadow/shaft pipelines) + `GltfTextureSource::prewarm_pipeline`. Any atom whose GPU pipeline is compiled lazily inside its OWN `run()` (the `primitive!` codegen path's `self.pipeline.get_or_insert_with(...)`, e.g. `node.cube_mesh`'s `GenerateCubeMesh::run` at `generate_cube_mesh.rs:97`) is never touched by either mechanism unless some bundled preset's *rendered* first frame happens to hit it — and prewarm never renders a frame. `node.cube_mesh` is a confirmed example (used by NestedCubes/DigitalPlants per its own doc comment, so likely already latent in any project layering one of those generators fresh); there is likely a whole class of atoms in the same state — anything reachable only via a codegen `standalone_for_spec` pipeline that no *already-warm* project has forced yet.

**Fix shape** — the general fix is bigger than one atom: either (a) extend `GeneratorRegistry::prewarm_all` to also RENDER one throwaway frame of every bundled preset (not just build its structure) at startup, so every atom any preset actually uses gets its lazy pipeline compiled once for free (bounded cost, done once, off the content thread's hot path); or (b) add a registry-wide "every atom prewarms its own codegen pipeline once, unconditionally" hook driven by `PrimitiveRegistry::with_builtin()`'s full atom list rather than by preset coverage, so atoms no bundled preset happens to use (or new user-authored graphs) are covered too. (a) is cheaper to land; (b) closes the whole class rather than just "whatever's in `assets/generator-presets` today".
...
**Status:** OPEN — found 2026-07-13 during VOLUMETRIC_LIGHT_DESIGN P1's gate run; bisected to the unmodified tree, confirmed pre-existing. Independently reproduced on three OTHER occasions too: landing the `mechbugs` wave gating the BUG-111 and BUG-054 fixes (same session, `before=368 after=368` / `before=17 after=17`), and CINEMATIC_POST P6/P5 (`bilateral_blur`/`ssao_gtao`, on the unmodified pre-lane tip) — four confirmations total, all agreeing on cause.
...
**Why not fixed this session:** `coc_from_depth`'s actual neighbors in `CinematicScene` (P1's only consumer) never form a fusable region with it — its upstream `depth` input comes from `node.render_scene` (always `Boundary`, a draw call) and its downstream consumer `node.variable_blur` reads it via a Gather wire (gather-consumed wires never union per the union gates), so `coc_from_depth` is always an isolated single-node region and always renders via the (now-fixed) standalone path in this preset's topology. The gap is real but not load-bearing for P1's deliverable, and fixing the fused path is compiler work beyond a one-atom phase's scope.
**Fix shape:** mirror `generate_fused_buffer`'s dedup-and-prepend `includes: Vec<&'static str>` loop inside `generate_fused`'s texture path — collect `node.node_includes` across `region.nodes` (dedup by value), prepend the joined text before the fused kernel's first `n{i}_body` definition (same spot the buffer path uses, before body emission). No test currently exercises this path (would need a second `wgsl_includes`-declaring Pointwise TEXTURE atom with a fusable same-space neighbour — `test_camera_pointwise_fixture.rs`'s I6 fixture has empty `derived_uniforms`-only coverage but no `wgsl_includes`, so it doesn't catch this either).
