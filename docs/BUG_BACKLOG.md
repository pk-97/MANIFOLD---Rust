...
| ~~BUG-161~~ FIXED | **ui-snapshot-feature-build-broken-again-arc-effectgraphdef** | FIXED 2026-07-14 (bug-wave3 lane C, drive-by) — `ui-snapshot` feature build broke again (a regression under the same feature BUG-033 covered, different symptom): 8 sites in `ui_snapshot/mod.rs` passed `view.canonical_def` (now `Arc<EffectGraphDef>`, from an upstream caching change) where `render_graph_to_png`/`render_graph_editor_to_png`/`AddSceneObjectCommand::new`/`AddSceneLightCommand::new` expect `&EffectGraphDef`/`EffectGraphDef`. Fixed with `&view.canonical_def` (6 sites) / `(*view.canonical_def).clone()` (2 sites) — pure borrow/deref, no semantic change. Found + fixed only because this session needed the harness to capture before/after PNGs for BUG-048/049/068; logged since the feature is dev-only and not in the default sweep, so a future regression here won't surface until someone needs `ui-snap` again. |
...
### BUG-161 (ui-snapshot-feature-build-broken-again-arc-effectgraphdef) — `ui-snapshot` feature build broken again, different symptom than BUG-033 — LOW (dev-only feature, not in the default sweep)
**Status:** FIXED 2026-07-14 (bug-wave3 lane C, drive-by)

**Found 2026-07-14** while building the `ui-snapshot` feature to capture before/after PNGs for
BUG-048/049/068. `cargo build --features ui-snapshot -p manifold-app --bin manifold` failed with
8 `E0308` errors, all the same shape: `ui_snapshot/mod.rs` passes `view.canonical_def` where
`render_graph_to_png`/`render_graph_editor_to_png` (6 sites) expect `&EffectGraphDef` and
`AddSceneObjectCommand::new`/`AddSceneLightCommand::new` (2 sites) expect an owned
`EffectGraphDef` — `LoadedPresetView::canonical_def`'s type is now `Arc<EffectGraphDef>` (an
upstream caching change), and these 8 call sites were never updated. BUG-033 (the original
"ui-snapshot doesn't compile on trunk") was already marked FIXED 2026-07-07 — this is a fresh
regression under the same dev-only feature, not a reopening of that bug.

**Fix:** `view.canonical_def` → `&view.canonical_def` at the 6 by-reference call sites,
`view.canonical_def.clone()` → `(*view.canonical_def).clone()` at the 2 by-value sites — pure
borrow/deref through the `Arc`, no semantic change. `cargo build`/`cargo clippy -- -D warnings`
clean with the feature on.

**Why logged rather than silently fixed:** this feature isn't in the default sweep
(`cargo nextest run --workspace` never builds it), so a regression here is invisible until an
agent or Peter next needs `ui-snap` — which is exactly what happened. Worth a note in case the
next session hits the same wall and wonders whether BUG-033 reopened.
