...
**Status:** OPEN — found 2026-07-16, reported by Peter (plasma layer invisible under a skull GLB layer; expected the transparent void to key).
...
### BUG-183 (e6-texture-completion-invalidates-two-stale-goldens) — `CompareSpecular.glb` and `CompareVolume.glb` genuinely regress in `glb_conformance_sweep` after E6's texture-completion sweep wires `specularTexture`/`specularColorTexture`/`thicknessTexture` for the first time — expected consequence of fixing the gap, not a shading bug
