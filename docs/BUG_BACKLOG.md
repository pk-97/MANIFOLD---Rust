...
**Root cause (diagnosed, not fixed):** two independent gaps. (1) The body paint material carries its look in a carpaint-style glTF extension (report-line doctrine D9 makes this visible, by design — same family as IMPORT_FIDELITY Deferred #1's clearcoat trigger, which this asset was predicted to fire). (2) `dropped_over_cap: 14` — the per-object texture-wire cap drops 14 maps on this 78-material asset, so even core-material textures beyond the cap never reach the renderer.

**Fix shape:** (1) map the carpaint extension's base color into the core material at import (or land Deferred #1's clearcoat lobe with its parse); (2) revisit the texture cap for many-material assets — either raise it, or prioritize base-color maps when rationing wires. Both importer-side; no shader work needed.
