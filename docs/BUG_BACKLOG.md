...
| BUG-160 | **editor-window-unification-inspector-card-layout-regressions** | Root cause FOUND + fix APPROVED 2026-07-14 (Fable, corrected same day per Peter's screenshot): card content does not FIT the width the card has — chip strips/labels/sliders don't adapt (no elide/min-width/wrap), visible at the editor lane's narrower default; the Author chevron lane is a secondary identity violation, NOT the bug. Design: GRAPH_EDITOR_INSPECTOR_UNIFICATION.md **Change 4** — D3 fit-at-every-width (primary), D7 one shared width source for both windows (Peter: "FUNDAMENTALLY the same object in code"), context-independent geometry, tick parity, tree-equivalence gates. Overnight lane executes P1+P2. MED-HIGH (UI regression). |
| BUG-158 | **mapped-param-edits-snap-back-no-two-way-binding** | Root cause understood (two paths: `apply_bindings` stomp for card-bound, `scalar_or_param` shadow for wire-driven); ALL design decisions now resolved in **docs/PARAM_TWO_WAY_BINDING_DESIGN.md** (APPROVED 2026-07-14, Fable) — inverse reshape + dispatch-layer reroute, driven-state treatment. Overnight lane executes P1+P2. MED-HIGH (authoring surface). |
