...
### BUG-233 (headless-script-harness-shows-stale-value-after-nontrivial-dispatch) — a `--script` flow's PNG/tree-dump keeps showing a param's PRE-write value after a real, correctly-dispatched write — found 2026-07-17 during SCENE_PANEL_CARD_CONVERGENCE_DESIGN.md C-P1a
...
### BUG-232 (scene-setup-camera-world-light-eye-toggle-reads-as-dead) — the Scene Setup outliner's dimmed eye glyph on Camera/World/Light rows reads as a broken button, not a disabled one — found 2026-07-17, Peter live-testing the dock ("The params and visibility buttons for all of these cameras, world, lights, etc don't work either. They do nothing currently.")

**Status:** OPEN — MED (UX misread, not a dispatch bug — see root cause). Fix scoped into SCENE_PANEL_CARD_CONVERGENCE_DESIGN.md C-P1 as an orchestrator amendment (eye-slot rule); not implemented this session.
...
### BUG-182 (hdri-exr-files-fail-or-fail-silently) — Peter's real .exr HDRI files don't work through `node.hdri_source`, despite the atom's claimed .exr support — MED (glb-import lighting / HDRI env_mode)
**Status:** OPEN, narrowed 2026-07-17 (IMPORT_ANYTHING_WAVE Lane W5) — probable root cause identified and the silent-failure half fixed at the log-line level; the UI card-level error surface and a confirmed repro against one of Peter's actual files are still owed.
...
**Status:** OPEN (crash still unreproduced) — P1 done 2026-07-17 (found no reproduction, RSS evidence recorded); **P2 SHIPPED 2026-07-17** (`lane/import-responsiveness`) — D2's duplicate-device deletion landed and verified. **P3 BUILT 2026-07-17** (same branch, session 3, pending landing) — D3+D4: `import_model_file` no longer runs synchronously on the UI thread at all (spawn+enqueue only, background worker in `import_worker.rs`), which directly removes the "UI-thread-stall/watchdog" hypothesis P1 named as the untested suspect — a watchdog can no longer fire against this path since nothing blocks the UI thread waiting on the import anymore. `bug219_verify.rs`'s two `#[ignore]`d harness tests were re-run against the P3 code (adapted to spawn+drain instead of a blocking call) and still show no crash across 3 sequential imports, same as P1/P2. This narrows but does not close the bug: the crash was never reproduced on this dev machine/build in the first place (P1's finding, unchanged), so P3 removing the leading hypothesis is evidence, not confirmation — Peter's build/hardware is still asked for to close the gap for real, per P1's original ask. IMPORT_RESPONSIVENESS_DESIGN.md P1 (2026-07-17, lane/import-responsiveness) drove the REAL interactive path (`Application::import_model_file`, unmodified — see below) and did **NOT reproduce the crash**, across 2 process runs of 3 sequential imports and a 3rd process run of 8 sequential imports, all with a real background content-shaped GPU runtime alive concurrently (`bug219_interactive_import_sequential_pressure` at `crates/manifold-app/src/bug219_verify.rs`, on this machine's build at HEAD of this branch). D2/D3/D4 proceed regardless per the design's D1 — the throwaway validation `GpuDevice` and the doubling this entry already narrowed are still deleted in P2 on their own merits, independent of whether P1 convicted them for the crash specifically. See the P1 finding below for what the evidence actually supports and Peter's build/hardware is asked for to close the gap.

**P2 outcome (2026-07-17, lane/import-responsiveness):** deleted the per-import `manifold_gpu::GpuDevice::new()` at `app_lifecycle.rs:666` (D2). `import_model_file` now calls a new `Application::validation_gpu_device()` (`app.rs`), which reuses `self.gpu`'s existing device when it exists (the normal case — `resumed()` runs before a window can receive a drop) or, when `self.gpu` is still `None` (P1's harness proved this reachable — a headless/pre-`resumed()` caller), lazily creates and caches ONE fallback device on a new `Application.import_validation_device` field, reused by every subsequent import for the rest of the process — never a fresh device per import either way. Negative gate `rg -n 'GpuDevice::new' crates/manifold-app/src/app_lifecycle.rs` → 0 hits. `validate_def`'s signature and behavior are untouched.

RSS-per-import, re-running the P1 harness plus a new sibling test (`bug219_interactive_import_with_existing_gpu_context_reuses_device`, added this session) that pre-populates `self.gpu` to exercise the branch that matters for the real app (P1's original harness only ever exercises the `gpu: None` fallback branch, since `resumed()` never runs headless):

| Scenario | before any import | after import 1 | after import 2 | after import 3 |
|---|---|---|---|---|
| Pre-P2 baseline (P1, documented above) | ~71 MB | ~633 MB (Δ~561) | — | ~707 MB (plateau) |
| Post-P2, `gpu: None` (fallback-device path) | 78 MB | 641 MB (Δ563) | 728 MB (Δ87) | 728 MB (Δ0) |
| Post-P2, `gpu: Some(..)` pre-populated (the realistic shape) | 80 MB | 632 MB (Δ552) | 724 MB (Δ92) | 733 MB (Δ9) |

**Honest read of the numbers:** no measurable shrink in total RSS growth. This is expected, not a fix that underperformed: the dominant memory cost per import is the trial-build's own GPU-resident textures/buffers (validate.rs's `PresetRuntime::from_def_with_device` — kept intentionally, per D2's own text and D6's "never a silent partial import"), which each import pays independently of whether the validation device is shared or private. D2 removes the redundant Metal *device object* itself (command queue, shader/pipeline compile caches, allocator pools) — real waste, correctly deleted — but that object is small next to a 15-material asset's decoded texture memory, so it was never going to show up as a large RSS delta. The structural win is verified by the negative gate and code inspection (one shared device instance instead of N throwaway ones), not by this RSS table. P1's live-hardware watchdog hypothesis (UI-thread stall crossing a responsiveness watchdog during a real set) remains the untested, more likely proximate cause — P3's background worker is what actually removes the UI-thread stall this entry's ~2.3s-per-import measurement named.

**P1 finding (2026-07-17, lane/import-responsiveness, HEAD `adaa1eb9` + this branch's merge of `lane/glb-triage`):** built the first real caller of `Application::import_model_file` outside its live drag-drop call site (`crates/manifold-app/src/bug219_verify.rs`, `#[ignore]`d, `journey-proofs` feature, macOS-only — run with `RUST_BACKTRACE=full cargo test -p manifold-app --features journey-proofs bug219_interactive_import_sequential_pressure -- --ignored --nocapture`). `Application::new()` needs no window/GPU/winit context (`gpu`/`content_tx` are `None` until `resumed()`, which `import_model_file` never calls), so a headless `Application` with a stubbed `content_tx` channel drives the exact production code path unmodified. A background thread runs a real headless `ContentThread` (own real GPU device, ticking frames) concurrently, matching the app's actual runtime shape. Ran the full 43MB `ABeautifulGame.glb` through it sequentially — 2 runs of 3 imports, 1 run of 8 imports (probing BUG-180's cumulative-pressure hypothesis) — **zero panics, zero process aborts, all three runs exited 0**. RSS evidence (sampled via `ps -o rss=` around every call): first import jumps RSS from ~71MB to ~633MB (a ~561MB jump importing a 43MB file — consistent with this entry's "full GPU runtime built twice" diagnosis), each subsequent import in the same process adds only ~0-80MB more and RSS **plateaus at ~707MB by import 3 and stays flat through import 8** — no unbounded growth/leak across repeated imports in this environment. Each `import_model_file` call blocks the calling thread for a consistent ~2.3s (measured via `Instant`, 3 runs). 36GB RAM / plenty of headroom on this machine (M4 Max) — the plateau plus the flat 8-import run argue against a slow leak or GPU-resident accumulation being the crash's proximate cause **on hardware/memory conditions like this one**.

**Conviction — does not implicate GPU memory doubling as sufficient cause, on its own, for a crash; live-hardware watchdog is the untested remaining hypothesis:** the doubled-full-GPU-runtime mechanism this entry already narrowed is real (the RSS jump proves it) but is NOT, by itself, enough to crash a process with normal desktop headroom — 8 repeated imports plateaued instead of growing unbounded. The one variable this headless harness structurally cannot exercise: `import_model_file` runs on the actual winit event-loop / UI thread, which also owns window presentation and (per `feedback_never_unify_cvdisplaylinks`) must never block a `CVDisplayLink` callback — a live app blocking that thread for ~2.3s per import, doubled/tripled during a real set with more layers/audio/MIDI load than this synthetic harness carries, is a live-hardware condition (macOS "app not responding" watchdog force-termination, or Peter perceiving+force-quitting a beachballed app) that no headless `cargo test` process can trigger, because there is no `WindowServer`/display-link watchdog in that process. This is the honest gap: P1 disproves "GPU-memory doubling alone crashes a healthy desktop," but does not disprove — and this evidence is consistent with — "the UI-thread stall this doubling causes crosses a responsiveness watchdog on Peter's actual rig during a real set." D3's background-worker fix (this design's P3) removes the UI-thread stall regardless of which half of that is true, so it isn't blocked on resolving the ambiguity. **Asking Peter for:** does the interactive crash show a macOS crash-report / "Manifold quit unexpectedly" dialog (real process termination — supports SIGKILL/watchdog) or does the window just freeze/beachball and never return (supports a plain UI-thread stall, not a crash)? Either answer is actionable without more headless work.

**Escaped:** `docs/GLB_CONFORMANCE_STATUS.md` classifies `ABeautifulGame.glb` as `expect_pass` (`non_black_fraction_min`) — the certified conformance harness (`crates/manifold-renderer/tests/glb_conformance.rs`) never exercises the interactive drag-drop path, only the headless `PresetRuntime` build via `assemble_import_graph`, so a "pass" classification says nothing about the app crash. Reconciled: not a contradiction, a coverage gap — the harness and the crash are different code paths (see below).

**Symptom:** Peter: the app crashes importing this file; "I think we're also missing the safety check on large scenes."

**Repro attempted, did NOT reproduce standalone:** `RUST_BACKTRACE=full ./target/debug/render-import tests/fixtures/gltf/khronos/ABeautifulGame.glb --out /tmp/abg.png` (the headless binary sharing the exact same `assemble_import_graph` production import path `render-import` and the conformance harness both use) completed cleanly — `converged on frame 150 (non-black fraction 0.1168)`, correct chessboard render, no panic, no backtrace. So the production import-graph assembly and render itself is NOT what crashes for this file at this size.

**Root cause — narrowed, not fully confirmed (no interactive-app repro available in this environment):** the app's drag-drop entry point (`Application::import_model_file`, `crates/manifold-app/src/app_lifecycle.rs:584-787`) runs synchronously on the UI thread and does substantially more work per import than the headless binary:
1. Calls `assemble_import_graph` (same as headless — not the divergence).
2. Creates a **second, throwaway `manifold_gpu::GpuDevice::new()`** purely for validation (line ~666), then calls `validate_def(...)`, which for a Generator-kind def (which every glTF import is) runs `PresetRuntime::from_def_with_device(...)` — a **full runtime build**: real GPU textures, pipelines, and the Array buffer pool for the *entire* import graph, built on this throwaway device (`crates/manifold-renderer/src/node_graph/validate.rs:233-283`, see the `ValidateKind::Generator` branch at ~268-282).
3. On success, executes `ImportModelLayerCommand`/`AddClipCommand` **twice** — once locally against `self.local_project` (UI thread), then again (same boxed command) sent to the content thread to run against the real `Project`.

Net effect: for a heavy multi-material asset like ABeautifulGame, the interactive path builds essentially the **full GPU runtime twice** (once thrown away immediately after validation, once for real) on top of whatever the app's real content-thread device is already holding — all synchronous on the UI thread. This is a much sharper, session-local explanation for the "safety check on large scenes" gap Peter named than a generic streaming/OOM story: it's not (only) that images are held fully decoded (BUG-180's framing), it's that the app *redundantly builds the entire GPU-resident scene twice* on every single import, with no size-based short-circuit. This plausibly explains BUG-180's own observed non-determinism (same 114.8MB file SIGKILLed once, succeeded on an identical immediate rerun) as a memory-headroom race, worsened by this doubling.

**Relationship to BUG-180 (`large-glb-import-oom-risk`, still OPEN):** same underlying class (interactive import has no size/memory guard), but a different, more specific mechanism identified this session (double full-runtime build) alongside BUG-180's own (uncapped simultaneous decoded-image memory). Both should be considered together when this is designed — fixing one doesn't fix the other.

**Fix shape (NOT attempted this session — structural, needs a design decision, and I have no way to reproduce/verify the actual interactive crash in this environment to confirm any fix resolves it):**
- Primary candidate: stop building a full `PresetRuntime` (real GPU textures/pipelines/Array pool) for interactive-import validation. `validate_def`'s `ValidateKind::Generator` branch exists to catch `UnsizedArrayOutput`/`UnboundArrayResource` — importer-code bugs that are asset-shape-independent and already covered by the conformance/golden test suite in CI; for a machine-generated import graph on an end user's asset, this is a validation pass with low marginal value paying a real doubled-GPU-build cost. Either skip that branch for the interactive import call site specifically, or (if the check has real value there) reuse the app's existing real GPU device instead of allocating a second one, so the two builds at least share a device's resource pool rather than doubling GPU-resident memory outright.
- Complementary — the "safety check on large scenes" Peter asked for directly: a pre-import cheap size heuristic (file size, or glTF header material/texture-byte-length sum before any decode) that warns/rejects above a generous threshold with a load-report line, never a silent truncation — same "never wrong, never silent" bar as `IMPORT_ANYTHING_WAVE_DESIGN.md`. This is additive and independent of the doubling fix above.
- Both need verification against a live interactive app (drag-drop, watch for the crash) — not achievable in this headless/worktree session.

### BUG-220 (mosquitoinamber-transmission-renders-black-faceted-holes) — `MosquitoInAmber.glb` (KHR_materials_transmission + volume + IOR) renders with scattered black triangular holes across the amber's surface — MED (reported live by Peter, visual fidelity on a certified-pass asset)

**Status:** OPEN — found 2026-07-17, GLB triage session (lane/glb-triage).

**Escaped:** `docs/GLB_CONFORMANCE_STATUS.md` classifies `MosquitoInAmber.glb` as `expect_pass` (`non_black_fraction_min` only — no golden, no visual-detail check). Reconciled: the numeric check only gates "not too dark overall," which this render still satisfies (`non-black fraction 0.1450`) despite the visible defect — a coverage gap in what the check asserts, not a contradiction.

**Symptom, reproduced:** `render-import tests/fixtures/gltf/khronos/MosquitoInAmber.glb --out /tmp/mosquito.png` (production import path, converged frame 8) shows scattered black faceted triangular holes across the amber's surface, matching Peter's screenshot exactly.

**Hypotheses tested this session, all disproven by direct probe (each reverted after testing, `git diff` on `render_scene.wgsl` clean):**
1. **Depth-write self-occlusion** — ruled out by code read: the blend/transmissive draw group already uses `blend_depth_stencil` with `write_enabled: false` (`render_scene.rs` ~2545).
2. **Screen-space refraction sampling the wrong/background pixel due to mip-level blur being too sharp** — tested by forcing `mip_level = max_mip` (heaviest available blur) in `transmission_diffuse` (`render_scene.wgsl` ~980) and re-rendering: **byte-for-byte identical facet pattern**, disproving this.
3. **Tangent-space normal-mapping breakdown at UV-island seams** (the screen-derivative cotangent frame is known to misbehave at hard UV seams, and a faceted low-poly gem is exactly that shape) — tested by short-circuiting `resolve_normal` to always return the raw interpolated vertex normal (skip the normal map / cotangent frame entirely): **facet pattern unchanged**, disproving this too.

**Root cause — found, in the SOURCE ASSET's own texture, not a shader bug:** extracted material 0's `baseColorTexture` (image 0, embedded JPEG) directly from the GLB and viewed it — it IS the black-faceted-crack pattern, byte-for-byte the same shape as the render defect (`/tmp/amber_basecolor.jpg` this session, a 4096×4096 UV-atlas-style texture: orange/tan mottled "shard" regions separated by pure-black gutters, visually a baked crack/fracture pattern or UV-island padding). `resolve_albedo` samples this texture directly (`u.base_color.rgb * t.rgb`) — wherever a triangle's UV falls on a black texel, albedo is exactly 0 there, zeroing every albedo-dependent shading term (direct diffuse, diffuse IBL). The material's own glTF sampler (`wrapS`/`wrapT` = `10497` = REPEAT, checked directly in the GLB JSON) is REPEAT by the asset's own authoring, ruling out BUG-164 (our hardcoded-REPEAT-sampler bug) as the mechanism here — it happens to already match.

**Why it reads as literal BLACK rather than a duller amber tint:** with albedo=0, `transmission_diffuse`'s mixed result (`final_diffuse = mix(diffuse_component, transmitted_diffuse, transmission_factor)`) still depends on the background sample the screen-space refraction ray hits — but this scene's "opaque scene color" backdrop is mostly bare clear-color black (no HDRI/environment behind the gem, and the amber itself isn't in its own opaque snapshot, a documented single-bounce SSR limitation), so most interior facets' refracted rays land on black background too, compounding the zero-albedo terms into a fully black pixel instead of a tinted-but-visible one.

**Fix shape — NOT attempted (touches asset interpretation + a structural SSR limitation, not a one-line change; needs a design call):**
- If the black IS the artist's intended crack/fracture texture (plausible — amber-with-visible-fractures is a real look): the actual bug is that our transmission model has no environment fallback for off-screen/background-less refraction rays, so a zero-albedo crack line reads as a flat hole instead of a subtly darker but still-glowing fracture. Fix shape: give `sample_transmission` an environment-probe fallback (the same IBL split-sum resources already ensured for every scene, `ensure_ibl_resources`) when the refracted ray's screen UV misses real backdrop geometry, so black-albedo/off-screen samples don't collapse straight to (0,0,0).
- Needs a design call on scope (this is a generic SSR-transmission limitation, not specific to this asset) and re-render verification against the other transmission conformance assets (`CompareTransmission`, `TransmissionTest`, `TransmissionRoughnessTest`, `TransmissionOrderTest` — baseline renders captured this session at `/tmp/{name}_before.png` for a future session's regression comparison) before landing.

### BUG-221 (glb-import-rotation-pivot-far-from-visual-center) — imported glTF objects can show large/odd default `Position` values, and rotating them via the Properties panel swings the object about a point far from its own visual center — MED-HIGH (UX correctness, reported live by Peter with a repro asset)

**Status:** FIXED 2026-07-17 (lane/glb-triage, session 2) — fix-shape candidate 2 (composed per-object recenter, no primitive change), as the orchestrator directed. `gltf_load.rs`'s `summarize_node` now tracks a per-material world-space bbox alongside the scene-wide one (`per_material_bbox`, keyed the same as the existing `per_material` vertex-count map), exposed as `GltfMaterialInfo::own_center`. `gltf_import.rs`'s `build_object_group` (non-skinned objects only — see below) splits the old single `-scene_center` recenter into two: the mesh source (`node.gltf_mesh_source`, new `translate_x/y/z` params — see below) is shifted by `-own_center` so local `(0,0,0)` becomes THIS object's own visual center, and the outward-facing `node.transform_3d`'s `pos` becomes `own_center - scene_center` (the object's own center's position within the recentered scene) instead of the old shared `-scene_center`. The two compose back to exactly the old `-scene_center` net offset (proved algebraically and at value level — see the test below), so net world placement at import time is BYTE-IDENTICAL to pre-fix; only the rotation pivot moves, onto the object's own geometry, by construction — `node.transform_3d`'s own math (`model_matrix`) is untouched, per the rejected-alternative call above. The animated-object path (`node.gltf_animation_source`'s `recenter_x/y/z`, which port-shadow-wins over the SAME outer transform node) gets the identical `own_center - scene_center` fold-in, so an animated rigid object pivots the same way a static one of its shape does.
  - **Mesh-side seam:** `node.gltf_mesh_source` gained three new params, `translate_x/y/z` (Float, default 0.0, part of the background-parse re-trigger key like `fit`/`recenter`) — a plain constant per-vertex offset applied once at parse time, AFTER `fit`, decoupled from its uniform-scale behavior (`apply_translate`, gltf_mesh_source.rs). `[0,0,0]` is a strict no-op, so every hand-built or pre-existing node using this primitive is byte-identical. This is the ONLY primitive-definition change; it is NOT `node.transform_3d`/`node.scene_object` (the pivot-param shape explicitly rejected above).
  - **Skinned objects excluded, deliberately:** BUG-205's existing doctrine already excludes skinned objects from rigid `transform_3d` positioning (their world placement comes entirely from the joint palette) — no mesh-side shift is applied for them, and their outer transform keeps the old whole-scene `-center` recenter unchanged. Their rotation-pivot behavior is out of this bug's scope (transform_3d rotation doesn't govern a skinned mesh's shape at all).
  - **New imports only** (importer-authored graph shape change) — no load-migration, no saved-def changes, no mesh-file edits, per the committed seams.
  - **Tests:** `bug221_object_transform_recenters_about_own_bbox_center_not_scene_center` (`gltf_import.rs`, default sweep, non-GPU) — a synthetic two-material `GltfImportSummary` with hand-picked `own_center` values proves the formulas at value level, including that `mesh.translate + transform.pos == -scene_center` (net placement unchanged) for arbitrary `own_center`. `bug221_layout_preserved_before_and_after_fix` (`--features gpu-proofs`) renders `EmissiveStrengthTest.glb` (real multi-object asset, confirmed own_center/scene-center offset ~6 units on one of its 5 cubes) pre-fix (mechanically reconstructed from the post-fix def, not hand-duplicated) vs post-fix at default params: `mean_abs_diff = 0.000000`. `bug221_pivot_spins_in_place_after_fix_but_not_before` (`--features gpu-proofs`) rotates ONLY the far object by 45° pre- vs post-fix and renders both (`target/mesh-snap/bug221_pivot_{pre,post}_fix_45deg.png`) — looked at both: pre-fix the far cube shrinks to a barely-visible sliver at the frame edge (swung far out of its slot about the distant scene-origin pivot); post-fix the same cube stays in its slot, same footprint, just showing a rotated (diamond-on/corner-forward) silhouette. Full crate sweep (`cargo nextest run -p manifold-renderer`, 1440 tests) and the `gltf`-scoped `--features gpu-proofs` suite (136 tests) both green; scoped clippy clean (both with and without `gpu-proofs`).

**Symptom:** Peter, on an imported statue: Position shows `(-0.34, 39.13, -39.42)`; using the Rotation controls in the panel swings the object about that distant point instead of spinning it in place. His own diagnosis: "a wrong coordinate frame problem" — confirmed correct by mechanism, below.

**Mechanism, confirmed by reading (not guessed):**
- `node.transform_3d` composes `world = R(rot) * S(scale) * local_vertex + pos` (`model_matrix`, `render_scene.rs:1755` — rotation is a plain matrix multiply on the vertex BEFORE translation by `pos`). Rotation therefore always happens about the object's own LOCAL-SPACE origin `(0,0,0)`, never about wherever the mesh's own geometry happens to sit in that local space.
- The importer's per-object recenter (`gltf_import.rs` ~1443-1449, `pos_x/y/z = -center[...]`) uses ONE `center` value computed ONCE from the WHOLE SCENE's bounding box (`gltf_import.rs:2032`, from `summary.bbox_min/max` — computed before the per-material object loop, then reused for every object inside that loop). This is deliberate and correct for framing/layout: it recenters the whole imported scene as a group at the origin while preserving every object's position *relative to the other objects* — per-object recentering would instead collapse every object onto the origin and destroy the scene's layout.
- The consequence: each object's OWN local vertex data is never re-centered about ITS OWN bbox — it still sits wherever the source glTF authored that object's local coordinate space (e.g., a statue placed off-center within a larger multi-object scene, un-"frozen" at its own local origin during authoring). Rotating that object's `transform_3d` swings its mesh about its own local `(0,0,0)`, which can be far from where the mesh's vertices actually sit — exactly "the mesh's baked origin sits far from its visual center."
- `Position (-0.34, 39.13, -39.42)` is consistent with this: it's `-scene_center` (the shared recenter offset), and a large Y in particular is consistent with a tall multi-object scene where this object sits well above the scene's overall bbox center.

**Why not fixed in the triage session that found it:** the fix changes shared, load-bearing import/rotation mechanics that ~150 conformance-tested Khronos assets and every existing project depend on for golden-stable rendering — needed an explicit shape call before landing, not an improvised same-session patch. Also: computing a true per-object local bbox center required new plumbing — no per-object (per-material) bbox was tracked at the time, only the scene-wide `summary.bbox_min/max`. Fixed in a follow-up session (see FIXED note above) once the orchestrator picked a shape from the two candidates below.

**Fix-shape candidates that were evaluated (both keep mesh files untouched, per the standing graph-not-mesh-files rule) — candidate 2 is what shipped:**
1. **Pivot param inside `node.transform_3d`/`scene_object`** — add a `pivot` such that `model = T(pos) * T(pivot) * R * T(-pivot) * S`. Rejected: `transform_3d` is a widely-reused primitive (every scene object, plus `GLTF_ANIMATION_DESIGN`'s per-object animation source port-shadows its `pos_x/y/z` directly) — changing its core math ripples into animation wiring and every other consumer, a much bigger blast radius than the bug needs.
2. **Shipped: importer-side compensating transform, composed in the graph, no `transform_3d`/`scene_object` primitive change.** The mesh source (`node.gltf_mesh_source`, which DID get a new `translate_x/y/z` param set — see the FIXED note; this is a different, importer-only primitive from the rejected candidate 1's target) recenters this object's OWN local geometry about ITS OWN local bbox center; the outer `node.transform_3d`'s `rot` is what the panel exposes and its `pos` is `own_center - scene_center`. Rotation happens about the object's own visual center by construction, and the Properties panel's Position field is now a more legible per-object value, exactly as predicted here.

### BUG-222 (playsetlighttest-shadow-bias-not-slope-scaled) — Peter reports blotchy/cloudy shadow acne on `PlaysetLightTest.glb`'s rug at some angles; probed but NOT reproduced headlessly this session — MED (found not fixed, needs Peter's exact repro angle)

**Status:** OPEN — found 2026-07-17, GLB triage session (lane/glb-triage). Characterization only — could not reproduce the reported artifact from the angles tried.

**Symptom (Peter's report):** blotchy/cloudy shadow patches on the rug receiver under `PlaysetLightTest.glb`'s toys, shifting with view angle.

**Probes run, did not reproduce:** rendered `PlaysetLightTest.glb` through `render-import` (production import path) at the default framing (matches the committed golden `tests/fixtures/gltf/goldens/playset_light_test.png` visually), then at 6 orbit angles (0/60/120/180/240/300° with tilt), a brightened pass (`sun_int=4.0`), and a close-up pass (`cam_dist=0.35`, `sun_int=2.5`) directly over the rug near the toys. None showed the described blotchy/cloudy pattern — the rug rendered clean at every angle tried, including close up.

**Mechanism read (no code changed) — a real, live candidate even though unreproduced:** `render_scene.wgsl`'s `shadow_factor` (`~554-593`) computes `ref_depth = ndc.z - bias` with `bias = params.x` sourced straight from `Light::shadow_bias` (`light.rs:144`, default `0.003`) — a **constant** depth bias, not slope-scaled (no `n·l`-dependent term) and not paired with any normal-offset technique. This is a textbook shadow-acne source on a large flat receiver (the rug) at a grazing light-to-surface angle; the param's own tooltip already documents the tradeoff ("Raise it if you see speckled self-shadowing, lower it if shadows float away from contact points" — `param_tooltips_bulk.rs:463`). The shadow frustum fit itself is NOT the issue: `range`/`shadow_resolution` are already tightly fit per-asset at import time (`gltf_import.rs:2224-2230`, `range = radius * 1.5`, `shadow_resolution = 4096`), ruling out the "resolution vs. scene extent" hypothesis.
- Precision-infra question (Peter asked whether another session's in-flight graph-kernel precision work could be related, or fix this): **no** — `shadow_factor`/the depth compare run inside `render_scene.wgsl`'s fragment shader, a direct Metal draw-call pipeline (`node.render_scene`'s own `create_render_pipeline`/`GpuEncoder::draw` path), not a `primitive!`-macro fused compute kernel. `render_*` draw calls are explicitly exempt from the freeze/fusion codegen path per the repo's own standing rule (CLAUDE.md, GPU atom codegen section) — any precision-level infra scoped to fused compute kernels has no code-path overlap with this shader at all. Code anchor: `render_scene.rs`'s pipeline creation and `render_scene.wgsl`'s `shadow_factor`/`fs_pbr` are the sole owners of this compare; no shared kernel/codegen machinery is involved.

**Not fixed — could not pin Peter's exact angle/zoom/distance,** so a bias tweak can't be verified against his actual sighting. Fix shape if reproduced: slope-scale the bias (`bias * max(1.0, 1.0/max(n_dot_l, 0.05))`-shape term, computed either CPU-side per-caster or in-shader per-fragment) — small, localized to `shadow_factor`/the caster-table fill in `render_scene.rs` (~2118-2121), testable against the existing shadow gpu-proof renders for regression. Needs Peter's exact viewing angle/distance to confirm the artifact and validate any fix.
 (modifier-commands-splice-at-dead-group-output-vertices-port) — the D6 modifier-stack commands still target the pre-D12 splice point, so "Add modifier" silently no-ops on every real grouped object — found 2026-07-17, SCENE_OBJECT_AND_PANEL_V2 P5 flow-script verification
