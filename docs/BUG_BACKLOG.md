...
|---|---|---|
| BUG-196 | **ui-automation-key-event-has-no-global-undo-seam** | Headless `Key { Z, command }` steps in `scripts/ui-flows/` never trigger Undo — `UIRoot::key_event`/`process_key` only queues a `KeyDown` for a focused text field, never reaching the app-level `M::Undo` menu shortcut. Pre-existing gap (two earlier flows already sent a no-op `Key Z` without asserting on it); confirmed this session building P5's modifier-stack flow. LOW (the command-level `execute()`/`undo()` unit tests are the real undo proof and already cover it; the harness gap only affects flows that want to prove undo purely through simulated keystrokes). |
...
### BUG-196 (ui-automation-key-event-has-no-global-undo-seam) — headless `AutomationAction::Key { key: Z, modifiers: { command: true } }` never triggers Undo — found 2026-07-17 during SCENE_SETUP_PANEL_DESIGN.md P5 (modifier stack)
...
**Fix shape:** profile first — same designated instrument BUG-189 names (PERF_BUDGET_GATE_DESIGN.md P2b's `--profile` attribution mode) run against `BrainStem.glb` specifically, to separate "pre-existing many-object render_scene cost" (BUG-189's territory) from anything genuinely new to the A2 skinning chain. If it's (a)/(c), this is a duplicate of BUG-189's class, not a new bug — fold in. If it's (b), fix is a per-file parse cache keyed by path (shared across the 24 `node.gltf_skinned_mesh_source` instances) rather than 24 independent full-document re-parses. If it's (d), profile the CPU side directly (the harness in `gltf_import.rs`'s `skinned_import_hot_path_stays_under_20ms_per_frame` test isolates GPU-encode-to-submit wall time, which INCLUDES blocked-on-CPU time inside that window — a CPU-side flame graph would separate the two). Measurement harness: `cargo test -p manifold-renderer --features gpu-proofs --lib skinned_import_hot_path_stays_under_20ms_per_frame -- --nocapture` (currently asserts only on CesiumMan/Fox; re-add BrainStem to the asset list with a raised or removed budget once this is diagnosed, so it stays a live regression guard rather than a one-time measurement).
...
**Fix shape:** the attribution above narrows this to `render_scene`'s shadow pass (suspect (a)) — next step is instrumenting `render_scene` internally (or bisecting by disabling shadow casting) to confirm the shadow-map re-render specifically, then cache the shadow map for static scenes (dirty-flag on light/geometry change) per the original fix shape. (b)/(c) are now de-prioritized — the flat single-dominant-node profile argues against many-small-passes overhead being the floor's main cost, though `ssao_gtao`+`bilateral_blur` (~11% combined) may still be worth a cheaper AO pass if the shadow fix alone doesn't close the 4K p95 gap. Measurement harness: `cargo xtask perf-soak <glb> --size WxH [--profile]` (PERF_BUDGET_GATE_DESIGN.md P1+P2b, landed 2026-07-16, `7afcb059`) — no longer a throwaway patch, it's the standing tool.
