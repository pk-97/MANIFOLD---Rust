...
|---|---|---|
| BUG-295 | **live-structural-scene-edit-params-never-reconcile-into-preset-instance** | `PresetInstance.params` is only ever rebuilt from `def.preset_metadata.params` at deserialize/load time (`build_param_manifest`, gated by `pending_wire`); a LIVE structural scene edit (Add Fog/Environment/Light/Object) correctly stamps the graph's `preset_metadata` but nothing re-derives the running instance's `params`, so the freshly-added node's exposure is invisible in the Scene Setup panel until the project is saved and reloaded — confirmed via direct trace to affect BOTH the new Fog/Environment work AND the already-shipped P1 Add Light/Add Object commands — HIGH |
