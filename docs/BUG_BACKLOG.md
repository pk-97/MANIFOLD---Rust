...
### BUG-105 (audio-mixdown-analysis-only-test-order-flaky) — a playback mixdown test fails intermittently inside the full workspace sweep but passes deterministically alone — LOW
...
### BUG-104 (audio-trigger-vs-shared-param-mod-not-isolated) — enabling an audio Trigger interferes with a card slider on a param the graph's trigger option also drives — MED (repro pending)
**Symptom:** reported by Peter 2026-07-10. With a Trigger enabled, another parameter — one of the params in the graph's trigger-parameter option — that is also shared on the perform card sliders no longer works independently: trigger and slider/modulation interfere instead of composing. Exact repro not yet pinned (which generator/params, and what the failure looks like: slider dead while the trigger is armed vs. trigger fires moving the shared param vs. slider edits disturbing the trigger).
**Root cause:** unknown — three suspects at different levels, un-adjudicated until there's a repro:
1. **Preset wiring** (same family as BUG-094): generators whose clip-trigger option muxes a burst onto a user-exposed param (e.g. FluidSim3D `noise_factor` = turbulence slider × `mux_noise` envelope) compose trigger and slider through a mux/multiply — if the envelope baseline or the mux's non-selected input isn't identity while the trigger is enabled, the slider gets scaled/overridden whenever the trigger is armed, not just on fires.
2. **Engine seam:** `TriggerPulse` is keyed by `layer_id` only (`modulation.rs:692`) and the renderer folds all of a layer's trigger-gate fires into one per-layer `audio_count` — every trigger-consuming param in that layer's graph reads the same Trigger response, so per-param isolation doesn't exist at this seam by construction.
3. **UI state:** the inspector AUDIO TRIGGERS section fabricates synthetic `clip_trigger_{i}` ParamIds and shares `ParamModState`/drawer machinery with `ParamCardPanel` (`audio_trigger_section.rs:189`) — an id/index collision could route an enable-toggle or drawer edit onto the wrong row.
**Fix shape:** observe before wiring anything — reproduce with Peter's setup while logging `evaluate_instance_audio_mods` param writes and the renderer's `audio_count` fold, then fix at the level the cause lives at. If it's (2), isolation likely means keying pulses per instance/param instead of per layer.
