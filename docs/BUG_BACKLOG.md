...
### BUG-265 (p1-exposure-bindings-cross-wired-onto-the-wrong-node-for-gltf-imports) — some P1-stamped scene-exposure bindings for glTF-imported scenes target the WRONG node, surfacing one item's params under another's selection in the new section-filtered scene panel (slice 2a)
**Status:** OPEN
...
**Root cause:** not fully isolated — this session's job was the render path, not the importer. Two live hypotheses, un-adjudicated: (a) a genuine cross-node reference (the environment's procedural sky simulation legitimately reads the light's position/an "ambient" contribution as an input, and whoever wrote the exposure stamping for `gltf_import.rs`/`bake_environment` intentionally reused the light's binding id or aimed a binding at the wrong node id while composing the shared "Sun"/"Environment" section labels), or (b) a real bug in the gltf importer's or `migrate_scene_exposures`'s binding-target assignment for this node pairing (a copy-paste `node_id` mistake, or an id computed from the wrong node during stamping). The section-filtering code added this slice (`sections_for_doc_ids`, `state_sync.rs`) reads `meta.bindings`/`meta.params` completely at face value — it is not inventing or misreading this crossover, it is faithfully surfacing data that was already wrong (or at least surprising) before this slice; the OLD bespoke per-family scene rows never rendered raw exposure bindings this directly, which is why it went unnoticed.
**Fix shape:** find where `gltf_import.rs`/`scene_exposure.rs` stamps the bake_environment node's exposures for this scene shape and confirm whether the `7_pos_x`/`scene_ambient` bindings are meant to alias the light/material (in which case the section labels are correct and this entry should be closed as intentional — just confusing without a comment) or are a genuine id mixup (in which case the fix is in the stamping call site, not in the panel). Either way, add a doc comment at the stamping site recording the decision so a future session doesn't have to re-derive it via a PNG. Out of scope for P2 slice 2a — flagged, not fixed, per this session's brief.
