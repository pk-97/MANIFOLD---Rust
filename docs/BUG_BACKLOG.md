...
### BUG-143 (gpu-proofs-prewarm-tests-order-dependent-under-full-suite) — two BUG-037 prewarm tests fail under the full in-crate GPU suite but pass standalone — LOW (gate-only)
...
**Status:** DEFERRED (scheduled — no longer blocked on a design pass) — owned by the widget-unification track (Peter 2026-07-11); fully designed 2026-07-13 as `UI_WIDGET_UNIFICATION_DESIGN.md` P5a–P5d (D16: one `TextEditModel` in `manifold-ui/src/text_edit.rs`; standard click/drag/shift-click selection; **no IME** — Peter's call 2026-07-13, composition input explicitly unsupported; buffer-then-one-command commit on Enter/blur). **P5c is the phase that closes this bug.** Code note (2026-07-13 audit): a rudimentary append-only label rename NOW exists in the popover (`enter_edit`/`on_text_char`, mapping_popover.rs:591/:604) — the module doc's "label is read-only" claim is stale; `section` still has no field. P5c replaces that bespoke buffer with the model. Found 2026-07-10 building SCENE_BUILD_AND_GROUP_PARAMS_DESIGN.md P3's "section in the param mapping editor" deliverable.
