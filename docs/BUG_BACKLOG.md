...
|---|---|---|
| BUG-213 | **scene-object-handle-collides-with-material-named-mat-n** | `MetalRoughSpheresNoTextures.glb`'s 98 materials are literally named `mat_0`…`mat_97`; the scene_object handle (= the material's own name, SCENE_OBJECT_AND_PANEL_V2 D6) collides with the inner `node.pbr_material`'s synthetic `mat_{k}` handle after flatten-prefixing, panicking `Graph::add_node_named` on `mat_0/mat_0` — reproduces on a clean checkout, unrelated to any wave lane's own diff — MED (crashes the conformance sweep on this one Khronos asset; found 2026-07-17 via IMPORT_ANYTHING_WAVE_DESIGN.md W1's gate run, Fable advisor-diagnosed) |
...
### BUG-213 (scene-object-handle-collides-with-material-named-mat-n) — `MetalRoughSpheresNoTextures.glb` crashes the conformance sweep with a duplicate-handle panic — found 2026-07-17, IMPORT_ANYTHING_WAVE_DESIGN.md W1's gate run

**Status:** OPEN — MED (crashes `cargo test -p manifold-renderer --features gpu-proofs --test glb_conformance` on this one Khronos asset; every other asset in the sweep passes).

**Symptom:** `thread 'glb_conformance_sweep' panicked at crates/manifold-renderer/src/node_graph/graph.rs:137:13: Graph::add_node_named: duplicate handle 'mat_0/mat_0' (already mapped to NodeInstanceId(9), just tried to remap to NodeInstanceId(11))`. Reproduces bit-identically on a clean `main` checkout with no diff applied (verified against tip `cb46d326`, the design doc's own stated-green tip) — not caused by any wave lane's changes. A Fable read-only advisor diagnosed it in-session so the W1 lane could keep moving without fixing scene-object-v2's bug itself.

**Root cause:** `MetalRoughSpheresNoTextures.glb` names its 98 materials literally `mat_0` through `mat_97`. Per SCENE_OBJECT_AND_PANEL_V2 D6, a `node.scene_object`'s handle IS the object's group name (`unique_group_name`, derived from the material's own name when present) — so the group's handle is `mat_0`. The inner `node.pbr_material` for that same object independently gets the synthetic handle `mat_{k}` at the same index (`build_object_group`, `gltf_import.rs`). After the flattener prefixes inner handles with their group's handle, both nodes land on `mat_0/mat_0`, and `add_node_named` panics on the collision. Bug class: any glTF material NAMED to collide with a synthetic inner handle (`mat_k`, `mesh_k`, `transform_k`, …) — introduced by scene-object-v2 P3 (`1a5786cb`), landed just ahead of `cb46d326`.

**Fix shape:** disambiguate the scene_object's handle from the synthetic inner handles it may collide with — e.g. `unique_group_name` reserves/uniquifies against the inner-handle namespace it will end up nested inside, or the inner synthetic handles get a prefix/suffix that can't collide with a user-authored material name. Out of scope for the import-anything wave (this is scene-object-v2 infra, not an import-format gap) — needs its own small fix, ideally by whichever session picks up SCENE_OBJECT_AND_PANEL_V2 follow-ups.
