...
**Root cause (strong suspect, not fully pinned):** the project-local preset install/clear plumbing exists — `set_project_presets` / `clear_project_presets` (`manifold-renderer/src/preset_loader.rs:220,247`, whose own doc says to call it "with empty vecs on project close/switch so a stale preset doesn't persist") — but the ONLY non-test caller is the internal delegation inside `clear_project_presets`; the app's project-load path (`app_lifecycle.rs`) never calls either. So a second project's embedded presets are neither installed nor is the first's cleared, and layer generators resolving by `PresetTypeId` (see the load-time "preset template unresolved — project-local preset not registered yet?" warnings) bind to the first project's cached templates. The exact render-path cache (PresetRuntime / chain pool / generator instantiation keyed by type id) that serves the stale graph is not yet confirmed — needs a repro trace.
**Fix shape:** wire `set_project_presets(project.embedded_presets…)` into the project-load path and `clear_project_presets()` (or a full swap) on project close/switch, AND confirm the render-side generator/preset cache is keyed so a project switch invalidates it (a type-id-only key is the suspect — may need the project/instance identity in the key). Repro first (load A, load B sharing a type id with a divergent graph, assert B's graph renders), then fix, then the repro becomes the regression test.
**Workaround:** relaunch Manifold between project loads.
