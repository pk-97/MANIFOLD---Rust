...
### BUG-182 (hdri-exr-files-fail-or-fail-silently) — Peter's real .exr HDRI files don't work through `node.hdri_source`, despite the atom's claimed .exr support — MED (glb-import lighting / HDRI env_mode)
**Status:** OPEN, narrowed further 2026-07-18 — Peter's actual file (`tests/fixtures/gltf/kloppenheim_07_puresky_4k.exr`, 74MB) now confirmed to decode AND relight correctly through the full production path; the "doesn't do anything" symptom is not a decode/wiring failure for this file. The UI card-level error surface (b) and the card's own file picker (c) are still owed; a live-app repro (decode latency vs. a real propagation bug) is still owed.
