...
|---|---|---|
| BUG-193 | **switch-texture-blocks-ibl-generation-gate** | `node.switch_texture` (the D6 env_mode bake/hdri selector every glTF import wires between the envmap producer and `node.render_scene`) copies its selected input into its own output every frame via an unconditional `dispatch_compute` and never calls `mark_outputs_unchanged` — so the envmap slot's write generation never stabilizes on a real import, and RENDER_SCENE_PERF_OPTIMIZATION_DESIGN.md P3's IBL re-convolution gate (which reads that generation) never hits. Measured: AMG @4K unprofiled p50 13.554ms → 13.333ms (P3 landed) — a ~0.22ms/1.6% drop, not the ~multi-ms/~41%-of-render_scene drop P0 measured and P3's own gate requires. HIGH (P3's mechanism is correct and gate-passing on any direct envmap wiring — proven by gpu-proofs parity tests — but does not deliver its headline payoff for glTF imports, the actual BUG-189 target, until switch_texture gets the same last_key/mark_outputs_unchanged gate). |
...
**Status:** PARTIALLY RE-DIAGNOSED 2026-07-17 (RENDER_SCENE_PERF_OPTIMIZATION_DESIGN.md P0, diagnosis only, no fix attempted) — the ~370ms figure no longer reproduces (re-measured at ~30ms, see below); a smaller, still-over-budget CPU-dominant cost remains, root cause of THAT not yet identified. NOT blocking A2 (BrainStem was the design doc's joint-count re-derivation note, never a named gate fixture — the actual A2 gate fixtures, `CesiumMan.glb` and `Fox.glb`, measure 5–7ms/frame, comfortably inside budget).
...
**Status:** OPEN — attributed 2026-07-16.
...
### BUG-161 (interaction-overlay-automation-callback-type-complexity) — `manifold-ui --all-targets` clippy fails on 4 `type_complexity` findings in `interaction_overlay.rs`, unrelated to BUG-112 — LOW (lint-only)
**Status:** OPEN — found 2026-07-14 during bug-wave3 lane D while re-running BUG-112's exact gate (`cargo clippy -p manifold-ui --all-targets -- -D warnings`) to verify that fix.
...
**Root cause:** unknown. Suspects: a recent preset edit or new atom in a bundled preset introducing a fusion boundary (BUG-141 family), or the baseline number simply not updated after an intentional preset change. `git log` over `crates/manifold-renderer/assets/` and the freeze cut rules since the last time this test was green would bisect it quickly; the test itself should name which preset fell out (if it doesn't, make it print the set difference — that's half the fix).

**Fix shape:** identify the preset that stopped fusing, decide whether the regression is the preset's wiring or a compiler gap, fix that at its level — do NOT just lower the floor to 32. Note the full `--features gpu-proofs --lib` suite is minutes-long and not run by every session, so this could have been red for several days.

### BUG-187 (khr-animation-pointer-channels-fail-to-deserialize) — any glTF asset with a `KHR_animation_pointer` animation channel fails to import at all, even when the extension is only `extensionsUsed` (not required)
**Status:** OPEN — found 2026-07-16 during GLTF_ANIMATION_DESIGN.md A1 (BoxAnimated.glb rigid-animation vertical slice).

**Symptom:** `gltf_load::import_glb("AnimatedColorsCube.glb")` (a Khronos sample asset, `KHR_animation_pointer` in `extensionsUsed` only) returns `Err("... gltf parse failed: missing field \`node\` at line 1 column 750")` — the file fails at the RAW deserialize step (`Gltf::from_slice_without_validation`), before `import_glb`'s own `extensionsRequired`-only validation filter (BUG_XFAIL_BURNDOWN D1) ever runs.

**Root cause:** `KHR_animation_pointer` channels legally OMIT `target.node` (the pointer replaces node-targeting entirely — see the extension's own spec). The pinned `gltf-json` 1.4.1's `animation::Target` struct declares `pub node: Index<scene::Node>` with no `#[serde(default)]`, so serde hard-fails deserializing any channel target that omits it — a crate-level gap unrelated to `import_glb`'s validation-filter trick (that trick only patches `json::Root::validate()`'s output, and this failure happens one step earlier, at `serde_json` deserialize).

**Fix shape:** none attempted this session (A1's held-out smoke test substituted `InterpolationTest.glb`, which doesn't hit this path, and documented the substitution rather than papering over it). A real fix needs either: (a) a crate patch/fork making `Target::node` optional (`Option<Index<Node>>`) whenever `KHR_animation_pointer`'s extension is present on the channel target, re-deriving the JSON shape by hand-rolling a `Deserialize` impl for `Target` instead of the derived one — sizable, touches a vendored dependency; or (b) pre-processing the raw glb JSON bytes to inject a dummy `"node": 0` into any channel target carrying `extensions.KHR_animation_pointer` before handing it to `gltf::Gltf::from_slice_without_validation`, then detecting and skip-reporting those synthetic-node channels downstream (matches this codebase's existing raw-JSON-sniff doctrine for clearcoat/sheen/iridescence, but for a structural field, not a material factor). Either way this is real, scoped work — not a one-liner. GLTF_ANIMATION_DESIGN.md D5 already deferred `KHR_animation_pointer` PROPERTY TARGETS (animating arbitrary properties); this bug is one level upstream of that — the file won't even PARSE, so D5 can't be reached until this is fixed.
