...
**Fix shape:** (a) protocol — only the landing session runs `--workspace` gates (already the rule; today's run by a second agent violated it); (b) hygiene — kill rust-analyzer in idle/stale sessions (respawns on demand); (c) structural — investigate capping rust-analyzer memory (`lru.capacity`, cache priming off) or sharing/disabling analyzers in worker sessions; (d) when a freeze occurs, check `/Library/Logs/DiagnosticReports` for gpuEvent vs JetsamEvent before attributing — the two bugs are only separable by that oracle.
