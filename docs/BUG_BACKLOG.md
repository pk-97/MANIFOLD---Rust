...
**Status:** OPEN — found 2026-07-17, GLB triage session (lane/glb-triage). **Needs a design call** before any fix — flagged explicitly below.
...
**Why not fixed this session:** the fix changes shared, load-bearing import/rotation mechanics that ~150 conformance-tested Khronos assets and every existing project depend on for golden-stable rendering — genuinely needs a design pass, not an improvised same-session patch. Also: computing a true per-object local bbox center requires new plumbing — no per-object (per-material) bbox is currently tracked, only the scene-wide `summary.bbox_min/max` (checked: `GltfImportSummary`, `gltf_load.rs:1320-1323` — no per-material bbox field).

**Fix-shape candidates, evaluated honestly (both keep mesh files untouched, per the standing graph-not-mesh-files rule):**
1. **Pivot param inside `node.transform_3d`/`scene_object`** — add a `pivot` such that `model = T(pos) * T(pivot) * R * T(-pivot) * S`. Rejected as first choice: `transform_3d` is a widely-reused primitive (every scene object, plus `GLTF_ANIMATION_DESIGN`'s per-object animation source port-shadows its `pos_x/y/z` directly) — changing its core math ripples into animation wiring and every other consumer, a much bigger blast radius than the bug needs.
2. **Preferred: importer-side compensating transform, composed in the graph, no primitive change.** Insert a SECOND `node.transform_3d` per object (or reuse the existing recenter one purely as an inner static translation) that recenters this object's OWN local geometry about ITS OWN local bbox center (needs the new per-object bbox, threaded through `per_material` aggregation the same way the scene-wide one already is), feeding an outer `transform_3d` whose `rot` is what the panel exposes and whose `pos` is set to place the pivot at the correct location in the recentered scene (`pos = local_center - scene_center`). Rotation then happens about the object's own visual center by construction, using only the existing primitive composed twice — no primitive semantic change, and the Properties panel's Position field becomes a more legible per-object value as a side effect. Existing animation wiring (which targets "the per-object transform node") would need to keep targeting the OUTER node, which stays compatible with today's port-shadow wiring.
- Either way: needs a full re-render/golden-diff pass across the Khronos conformance suite (per-object recenter is load-bearing for every multi-object import's camera framing) before landing — explicitly out of scope for this triage session.
