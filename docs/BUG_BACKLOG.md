...
**Status:** EXPECTED FIXED as a side effect of GLB_CONFORMANCE_DESIGN G-P2 (2026-07-15, `wave/glb-conformance-s1`) — pending Peter's AMG render confirmation. The diagnosed root cause below (`dropped_over_cap: 14` eating the body's base-color map) no longer exists: G-P2 deleted the object cap entirely (`render_scene`'s clamp rose from `OBJECT_SLIDER_MAX = 64` to a real 1024 safety bound; `ImportReport.dropped_over_cap` is gone; the importer wires every material 1:1). `over_cap_asset_imports_one_to_one` and `amg_gt3_glb_imports_and_renders_without_error_if_present` (now asserting `object_count == 78`) prove the mechanism; the AMG fixture itself is untracked and was absent from the G-P2 worktree, so the actual render hasn't been re-looked-at yet — do that before flipping this to FIXED.
...
**Status:** OPEN — found 2026-07-15 during GLB_CONFORMANCE_DESIGN G-P1 while classifying Khronos `TextureSettingsTest` for the conformance manifest.

### BUG-165 (boombox-multi-texture-never-converges) — Khronos `BoomBox.glb` (4 material maps: base color, metal-rough, normal, emissive, all 2048×2048) never converges through `render-import` — 300 frames, non-black fraction stuck at 0.0000 the whole run, `ImportReport.textures_wired: 1` (out of 4 maps the material references) — MEDIUM (held-out input failure, distinct from BUG-164/BUG-163)
**Status:** OPEN — found 2026-07-15 during GLB_CONFORMANCE_DESIGN G-P1's orchestrator-run held-out gate (`BoomBox.glb`, deliberately not in the conformance manifest, fetched ad hoc from the same pinned Khronos commit). `render-import` exits 2 (the documented "never converged" exit, not a panic — the held-out gate's literal bar of "no panic" is met), but the substance is wrong: the JSON's `materials[0]` wires 4 distinct texture indices (baseColor 0, metallicRoughness 1, normal 2, emissive 3), yet only 1 shows as wired in the report, and the frame stays fully black the entire 300-frame budget — not merely dim.

**Root cause:** unknown, not investigated — two live hypotheses, neither confirmed: (a) the background texture-decode pipeline can't keep up with 4× 2048×2048 source images inside the 300-frame convergence window (a throughput/prioritization gap, not a correctness bug — DamagedHelmet's smaller textures converge in 6 frames); (b) `textures_wired: 1` points at an actual wiring bug specific to this material's port layout (e.g. `occlusionTexture` and `metallicRoughnessTexture` sharing index 1 — packed ORM — may be mishandled upstream of decode, not merely slow). Distinguishing these needs instrumentation this session didn't add (this phase's forbidden moves list bars touching importer code).

**Fix shape:** first isolate (a) vs (b) with a longer `--frames-max` / decode-progress trace on `BoomBox.glb` alone; if (a), the fix is decode scheduling/prioritization (likely shared surface with BUG-163's texture-cap rationing); if (b), the fix is wherever the ORM-shared-image case is resolved to ports (`gltf_import.rs`'s material→port wiring). Not scoped to any G-P phase yet — surfaced for triage.
