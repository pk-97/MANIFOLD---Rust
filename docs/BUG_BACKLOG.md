...
|---|---|---|
| BUG-216 | **modifier-commands-splice-at-dead-group-output-vertices-port** | `InsertMeshModifierCommand`/`RemoveMeshModifierCommand`/`MoveMeshModifierCommand` still splice at the group's `system.group_output` `vertices` port — a port D12 (SCENE_OBJECT_AND_PANEL_V2) deleted from the importer's shape (the group only exports `object` now); "Add modifier" is a silent no-op on every real GLB-imported/grouped object — HIGH (a whole P5 dock affordance is dead against real content), found 2026-07-17 during P5 flow-script verification |
| BUG-215 | **conformance-sweep-panics-on-duplicate-mat-0-handle** | `glb_conformance` panics on `MetalRoughSpheresNoTextures.glb`: `Graph::add_node_named: duplicate handle 'mat_0/mat_0'` — the full gpu-proofs conformance sweep is RED at `origin/main` tip `cb46d326`, introduced by `1a5786cb` (SCENE_OBJECT_AND_PANEL_V2 P3), never caught because that landing's own test additions didn't rerun the full sweep — HIGH (gate-blocking; not caused by Lane W6, which made no code changes) |
...
### BUG-216 (modifier-commands-splice-at-dead-group-output-vertices-port) — the D6 modifier-stack commands still target the pre-D12 splice point, so "Add modifier" silently no-ops on every real grouped object — found 2026-07-17, SCENE_OBJECT_AND_PANEL_V2 P5 flow-script verification

**Status:** OPEN — HIGH (the P5 dock's "Add modifier"/remove/reorder chips are unreachable affordances against any real GLB import or `AddSceneObjectCommand`-built object; only exercised in this session's own synthetic-def unit tests, which construct the OLD splice shape directly and so never caught this).
...
### BUG-215 (conformance-sweep-panics-on-duplicate-mat-0-handle) — `glb_conformance` gpu-proofs sweep panics on `MetalRoughSpheresNoTextures.glb`, RED at `origin/main` tip `cb46d326` — found 2026-07-17, IMPORT_ANYTHING_WAVE Lane W6 landing gate (Fable-advisor-confirmed diagnosis)

**Status:** OPEN — HIGH (blocks the mandatory conformance-sweep-green landing gate; `docs/IMPORT_ANYTHING_WAVE_DESIGN.md`'s own attestation that the sweep "passes at this doc's tip [`9b8a6919`]" is no longer true at the current main tip — a regression landed after that doc was written, not something the doc's claim got wrong).

**Symptom:** `cargo test -p manifold-renderer --features gpu-proofs --test glb_conformance` panics deterministically (reproduced twice, identical) partway through the sweep:
```
RUNNING MetalRoughSpheresNoTextures.glb (expect_pass):
...
```
Confirmed NOT a cross-asset or device-contention flake: the harness (`crates/manifold-renderer/tests/glb_conformance.rs`) builds a fresh graph per asset via `assemble_import_graph`, so the collision is within one asset's own import.

**Root cause (confirmed by a read-only Fable advisor consult, per this wave's escalation clause):** `MetalRoughSpheresNoTextures.glb` has 98 materials literally named `mat_0`…`mat_97`. In `build_object_group` (`crates/manifold-renderer/src/node_graph/gltf_import.rs`), THREE things end up with the identical handle `"mat_0"` for this asset: the mechanical inner `pbr_material` node handle (`format!("mat_{k}")`, lines 730/1062 — doc-commented "purely cosmetic, namespaced away by the group-name-prefixing flattener"), the group node's handle (`group_name`, derived from the glTF material's own name, lines 735/1826), and — new in `1a5786cb` — the `node.scene_object` node, which SCENE_OBJECT_AND_PANEL_V2_DESIGN D6 stamps with `&group_name` as its handle (lines 1802-1807, "the name is its handle"). The flattener (`crates/manifold-core/src/flatten.rs`) prefixes every inner handle with its group's handle, so BOTH the pbr_material node and the scene_object node flatten to `mat_0/mat_0` — two different nodes, one handle — and `graph_loader.rs`'s `HandleScope::Global` registration (~lines 461-474) panics on the duplicate via `Graph::add_node_named` (`graph.rs:137`). Before `1a5786cb`, no inner node's handle could equal the group name, so the collision was structurally impossible at the wave doc's own tip (`9b8a6919`) — introduced by that commit, never caught because gpu-proofs is not part of the default sweep and that landing's own new test coverage (`0d41743b`) only touched DamagedHelmet/rosetta assertions, not a full conformance rerun. A real-world exposure of the same class: `unique_group_name` dedupes sibling GROUP names, but nothing dedupes an inner mechanical handle (`mesh_{k}`/`mat_{k}`) against a group name that happens to equal it — any asset whose material is literally named `"mat_N"` for its own index `N` (unusual but not invalid glTF) hits this.

**Fix shape (not applied by this entry — Lane W6, which found this, is analysis-only by design):** in `build_object_group`, just before pushing `scene_object_node` (~`gltf_import.rs:1808`), sweep the group's inner nodes and rename any whose handle equals `group_name` (they're the ones documented as cosmetic — safe to suffix) — leave `scene_object_node`'s own handle untouched, since it carries D6's real display/binding duty. Do NOT make the loader tolerate duplicates (`graph_loader.rs`'s comment: a node's document id defaults to its handle — silent dedupe there would corrupt binding resolution, violating no-silent-fallbacks). Two follow-ons the same fix-lane should pick up: (1) a GPU-free regression test — `assemble_import_graph` + `flatten_groups` on the `MetalRoughSpheresNoTextures` fixture asserting flattened-handle uniqueness, so this class is caught by the default 8s sweep instead of the non-default gpu-proofs one; (2) `RenameSceneObjectCommand` (`manifold-editing/src/commands/graph.rs:3390`) sets a scene object's handle with no visible collision guard against sibling inner handles — a performer renaming an object to e.g. `"mesh_0"` could hit the same panic at next graph rebuild; worth its own guard.
