...
### BUG-143 (macros-panel-ableton-trim-drag-outside-p7-inventory) — `MacrosPanel`'s Ableton-range trim-bar drag is a hand-rolled sentinel machine, outside every P7 fold — LOW, found not fixed 2026-07-13 (UI_WIDGET_UNIFICATION P7.6 closing inventory)
...
**Why not fixed this session:** P7.6's brief scopes it exactly to viewport scrubs, audio-setup band/calibration, and dock dividers — "don't invent additional scope." Folding this in would be a fourth, unplanned target for a phase whose own doc named three.
