...
| BUG-151 | **graph-editor-node-browser-container-fill-not-drawn** | The graph editor's node-spawn browser (shared `BrowserPopupPanel`) draws its search bar and cell rows but not the popup container fill/scrim — graph + inspector bleed through between cells. Main-window instance of the same component renders correctly, and `MODAL_BG` is alpha-250, so it's a render-path defect in the editor window's separate UIRenderer cycle (suspect: the §17 overlay loop / `overlay_draw` bookkeeping not running there), not a style issue. Needs runtime instrumentation to trace; excluded from the popup-pass brief on purpose. MED. |
...
**Status:** OPEN — `docs/EDITOR_WINDOW_UNIFICATION_DESIGN.md` P1 attempted 2026-07-14, blocked (not fixed) by a design-premise gap discovered at impl — see the phase report / escalation. Root class still holds: the editor window has NO overlay pass — `composite_editor_frame` renders one flat root scan (`editor_frame.rs:264`, unchanged) and never reads `overlay_draw`. What P1 additionally found: `UIRoot::overlay_draw`/`overlay_region_start` are populated in exactly one place, `UIRoot::build_overlays` (`ui_root.rs:1085`), called only from `UIRoot::build()` (`ui_root.rs:994`/`1008`), which only the MAIN window ever calls (`ui_frame.rs:221`) — the editor's `Workspace::ui_root` is built via plain `UIRoot::new()` (`workspace.rs:86`) and `.build()` is never called on it anywhere. So `overlay_draw` is permanently empty and `overlay_region_start` permanently `0` for the editor — a shared tree-overlay pass that reads `overlay_draw` (D1 of the design) draws nothing for the editor's node browser until the editor also gains SOME path that records the browser-popup region the way `build_overlays()` does for the main window's six overlays. This is a real, verified (not assumed) gap in the design's §1 audit, which had asserted the opposite. Do not point-fix a copy of the loop into `editor_frame.rs`.
...
**Root cause** — the "last inch" (exactly why `render_tree_range(0, usize::MAX)` draws cells but not the container fill/scrim) remains unobserved-by-design (P1's brief explicitly deferred it to a PNG observation that never happened, since the phase blocked before reaching the demo step). What's now additionally known: `MODAL_BG` is alpha-250 (effectively opaque, so not a style divergence), and the editor's browser-popup region IS a registered `begin_region`/`end_region` entry in `UITree::regions` (so `traverse_range`'s region-root scan should structurally find it) — but the modal dim-background scrim panel that `UIRoot::build_overlays()` normally adds ahead of a modal overlay's content (`ui_root.rs:1102-1117`, gated on `Modality::Modal { dim_background: true }`) is NOT added by the editor's manual node-picker-build block (`app_render.rs:3677-3693`), which only calls `browser_popup.build(&mut tree)` directly with no modal-scrim step. That may fully explain the missing scrim; whether it also explains the missing CONTAINER fill (as opposed to just the scrim) is still unconfirmed by observation.

**Fix shape** — needs a design decision, not a mechanical fix: how does the editor register ONE overlay (its own `BrowserPopupPanel`) into `overlay_draw`/`overlay_region_start` without adopting the main window's six-overlay `Z_ORDER` driver wholesale? Candidates to evaluate: (a) a small `UIRoot` helper (e.g. `record_overlay_region(start, end)`) any caller can invoke right after building an ad hoc overlay-tier region, called from `app_render.rs`'s editor node-picker block; (b) extending `build_overlays()`'s `Z_ORDER` loop to run for the editor too, scoped to just `OverlayId::BrowserPopup`; (c) something else. Whichever is chosen must also decide what `overlay_region_start` means for the editor (there's no panel-cache to narrow the base render against, per D5 — cacheless immediate-mode), and needs the modal dim-background scrim step ported alongside. Verify with a headless PNG of the editor + open browser, before/after. Do NOT restyle around it — the popup pass (`handoff_popup_professional_pass_prompt`) deliberately excludes this.
