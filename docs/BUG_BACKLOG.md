...
| BUG-150 | **mute-chip-press-motion-teleports-hit-bounds-after-scroll** | The layer-header mute chip's press animation (`tick_mute_motion`) rewrites the chip's bounds as `mute_base_y[i] + press_offset`, but `mute_base_y` is only captured at build (`layer_header.rs:2373`) and `try_update_vertical_scroll` (the set-only scroll fast-path, `layer_header.rs:2193`) shifts every node WITHOUT updating it — so the first hover after any scroll snaps the chip (draw + hit geometry, same bounds) back by the full scroll delta. Peter's click then lands on the row background → `LayerClicked` (selects the layer) → selection rebuild recaptures the Y → second click works. Confirmed end-to-end 2026-07-14 via MANIFOLD_INPUT_TRACE probes: two clicks at the same pixel resolved to different widgets (row bg then `layer_header.mute`). Explains "mute/solo needs the layer selected first", the inconsistency (only after scroll+hover), and the M chip visibly drifting off its row during scroll. Fix shape (Peter's call, 2026-07-14, root-level for ALL animations): press/hover feedback is COLOUR-ONLY — delete the 1px `press_offset_y` bounds write outright, and sweep every other animation-driven `set_bounds` on interactive nodes (inspector drawer, param_card, interaction_overlay lift/ghost sites) against the rule "animations never move hit geometry". HIGH — perform-surface control fails first click on stage. |
...
**Root cause** — unknown. What static reading established (2026-07-14): the graph editor's browser IS the shared `BrowserPopupPanel` (`graph_palette.rs`'s doc comment — the old palette was retired into it via the `OpenNodePicker` → `AddGraphNodeAt` flow), and `MODAL_BG` is alpha-250, effectively opaque — so this is a render-path defect, not a style divergence. Suspects: the graph editor window renders through its own separate UIRenderer prepare/render cycle (`ui_frame.rs:484`, `app_render.rs:4127` comment) which may skip or re-order the §17 overlay loop that draws the shell container before the content; or the overlay range bookkeeping (`overlay_draw`) not being populated for the editor window's tree.

**Fix shape** — reproduce with the editor open, instrument the editor window's render pass (which node ranges it draws and in what order vs the main window's overlay loop), find where the container/scrim nodes drop out, fix at that seam. Verify with a headless PNG of the editor + open browser. Do NOT restyle around it — the popup pass (`handoff_popup_professional_pass_prompt`) deliberately excludes this.
...
**Status:** OPEN — root-caused 2026-07-14 (Fable + Peter, live trace repro); fix assigned to a Sonnet brief (see `handoff_popup_professional_pass_prompt` memory, prompt 2), not fixed yet.
