...
Companion docs: `GRAPH_TOOLING_DESIGN.md` (lint h, `validate_def`, the declared-excuse precedent), `docs/CARD_AUTHORING.md` (cards as the creative-range owner — gains a section on this split at P2).
...
| Range lint | `node_graph/validate.rs:566–598` lint (h), a WARNING | Compares card-mapped bounds against the conflated range; cannot be an error until ranges mean one thing. GRAPH_TOOLING section 7 defers promotion to this design. |
...
| Seed audit | the 10-preset diagnostic table (section 2 below), worker-produced 2026-07-13 | Every currently-known card-vs-range conflict, each with a physical-vs-arbitrary read. |
...
- **D6 — The sweep direction is remove-by-default.** P2 seeds contracts ONLY where the kernel/shader read proves the boundary (Index/Count are mechanical: every mux select and num_inputs; the floors and clamps in section 2 get their reads). Everything else keeps its min/max as a hint and gains no contract. No per-param agonizing: no proof, no contract.
