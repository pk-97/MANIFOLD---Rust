...
vertex-cache import path in section 8 is the cheap bridge).

Companions: `MATERIAL_SYSTEM_DESIGN.md` (surface shading — required reading),
`CAPABILITY_ROADMAP.md` section 4 (the original gap analysis), `GRAPH_EDITOR_REDESIGN.md`
...
| Material port + 4 atoms (unlit/phong/pbr/cel), per-kind pipelines | `MATERIAL_SYSTEM_DESIGN.md` | Designed, now un-held. Multi-light + shadows are its section 7 (Extension points (forward-looking)) extension points — consumed here, not reopened |
...
- **D1 — The scene model is import-shaped.** Peter's call: Blender/glTF import is the
  ...
  types). Import itself is a later phase with its own design pass (section 8) — this design
...
- **D3 — Object port group = `mesh_n` (Array(MeshVertex)) + `transform_n` (TRS params,
  ...
  **[AMENDED 2026-07-06 — SCENE_BUILD_AND_GROUP_PARAMS_DESIGN section 8: the transform member
...
- **P0 — MATERIAL_SYSTEM M1–M5** ✅ SHIPPED (verified in-repo 2026-07-04; as-built
  record in MATERIAL section 11.1 — note the single-wgsl entry-point layout, which
  `render_scene`'s per-kind pipelines should reuse, not re-file). **MATERIAL M6**
  (albedo/metallic maps + alpha cutout + back-face fix, MATERIAL section 11) slots before
...
- **P1 — `render_scene` + multi-light (no shadows).** The dynamic node, object
  ...
  (MATERIAL section 7 multi-light extension point). Read-back: this doc whole, MATERIAL section 5/section 7,
...
- **P4 — Camera atoms.** `node.free_camera`, `node.look_at_camera`, full descriptors,
  section 2.5 audit first (expect: pure emitters, no new math beyond `look_at_rh`). Gate:
...
- **P5 — Viewport Tier 1 (navigate).** ✅ **COMPLETE 2026-07-17** on `lane/realtime3d-viewport`
  ...
  **P5b/P5c (2026-07-17, same branch):** the persistent-session architecture
  ...
  paragraphs in the doc header and section 5 intro above for the as-built detail.
...
- ~~**Instanced objects in scenes**~~ — **PROMOTED 2026-07-11 → section 10 (D11, P8).** The
...
- **section 2.5 audit at impl** (expect: genuinely new; `scatter_particles_3d` scatters in
...
Promotes the section 8 "Instanced objects in scenes" deferral: MESH_DEFORM P4's Garden demo
...
- **Entry state:** P2+P3 in-tree (`gpu_proofs::render_scene_shadows` /
  `render_scene_fog` pass); re-verify every anchor in the section 10 audit table (rg/read;
  a moved anchor is an escalation); Garden.json still composites two passes via a
  Max-blend `node.mix`.
- **Read-back:** this section 10 whole; D3/D4; `render_scene.rs` rebuild + both draw
  loops; `render_instanced_3d_mesh.wgsl` TRS + normal math; standard section 5–section 6.
...
build.** Graduates RENDERING_INFRA_V2 section 8. P2's PCF shipped fixed-width softness
