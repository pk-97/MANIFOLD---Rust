...
**Status: IN PROGRESS — P-P (projection split + scratch rider) SHIPPED 2026-07-21 (landing report `docs/landings/2026-07-21-ui-funnel-p-p.md`); P-F2a (ui_root split, WS2) SHIPPED 2026-07-21 (census record `docs/landings/2026-07-21-ui-funnel-p-f2a-census.md`); P-B executing on `lane/ws1-bridge` (S1/S2/S2b done, chain-router + ctx landed on lane per amended D6); Peter unblocked execution 2026-07-21 ("my review does not block you unless it's critical") · 2026-07-21 · Fable**
...
|---|---|
| INV-G1 Pure-move commits have zero non-wiring residue | `scripts/move_identity_check.py <commit>` exit 0 — run per lane pre-gate and per landing batch |
...
- **P-B Bridge split + context struct**: `dispatch_inspector` → `dispatch/` modules (D6, pure moves; entry deliverable = the 150-variant census); then `DispatchCtx` (D3) as its own semantic commit — signature change, bodies untouched, compiler-driven; `SliderReset` recursion collapses to `dispatch(snapshot, ctx)`. Gate: `undo_baseline` family green unmodified; INV-G6/G7.
