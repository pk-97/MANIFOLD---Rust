...
Peter's directives, verbatim (2026-07-21): *"no deferred-with-revival-triggers. We design to prevent bug classes, not to wait for the next failure"* — every WIDGET_TREE section 10 item is designed in below (section 2 D8–D10); *"It is the fundamental app infrastructure, so no bandaids, no cheap hacks, no lazy work"*; *"I want god files gone, I want proper software architecture, designs, and boundaries."*
...
**Binding constraints** (DESIGN_AUTHORING section 1): *Hot path* — `push_state`/`sync_card_values` run per-frame; the split must not move structural work onto the per-frame path (INV-W4). *Thread residency* — untouched: UI thread owns everything here; mutations stay `PanelAction` → dispatch → `ContentCommand`. *Time model* — untouched. *Persistence* — none of the moved types serialize; the fixture round-trip gate is a belt-and-braces net. *Performance surface* — every dispatch path IS the live-control path; the flow suite + `undo_baseline` family are the behavior oracle.

Companion docs: `WIDGET_TREE_DESIGN.md` (the layer vocabulary this extends; its section 10 (Deferred — TOMBSTONE (closed 2026-07-22; absorbed by UI_FUNNEL_DECOMPOSITION per Peter's no-deferred-with-triggers directive)) Deferred is absorbed and closed by this doc), `docs/ARCHITECTURE_DEBT.md` (campaign register), `AGENT_ROUTING.md` (orchestration), `.claude/GIT_TREE_DISCIPLINE.md` (landing mechanics).
...
**D4 — The scrub wire: one gesture engine, addresses not families.** Committed shapes (P-I; the WIDGET_TREE section 10 priced design, promoted):
...
**D8 — Direct-set flow verbs ride the scrub wire (WIDGET_TREE section 10 item 6, designed in).** A flow verb `SetParam { selector/value-ref, value }` that emits `Scrub(Begin) → Scrub(Move, v) → Scrub(Commit)` through the REAL dispatch path — fast setup, no bypass, undo-correct by construction. Ships with P-I (it is three lines once the wire exists) + one flow using it as its own acceptance test.

**D9 — Widget catalog rides the surface layer (WIDGET_TREE section 10 item 7, designed in).** The catalog is the declarations: a `--catalog` dump mode enumerating, per panel, every `ParamSurface` row id + `RowRole` + named chrome (the dump already serializes durable ids; this adds the enumeration view). No new protocol (widget-tree section 5's rule). Ships with P-S. Kills the BUG-239 (headless-script-harness-shows-stale-value-after-…) class structurally: a row without a queryable name cannot exist on the sanctioned path.

**D10 — Remaining WIDGET_TREE section 10 items, dispositioned:** macros/settings sliders → P-S flows them through `param_surface` hosts (they are small; the point is zero unsanctioned row paths remain). `param_slots_to_ui` scratch buffer → P-P does it while touching `push_state` (pre-allocated scratch, INV-W4's no-new-per-frame-alloc rule makes it free to prove). With D4/D8/D9 this closes section 10 entirely — supersession sweep at the final landing flips WIDGET_TREE's Deferred section to a tombstone pointing here.
...
- **P-Z Supersession + ceilings**: WIDGET_TREE section 10 tombstone; register updated (inventory, not status); regrowth ceilings finalized (D11); docs path-reference sweep (`CORE_ENGINE_MAP`, `WIDGET_TREE_DESIGN`, memory files); `gen_docs_index.py`; `.git-blame-ignore-revs` entries.

Phasing-completeness check: every section 2 commitment lands in exactly one phase (D1/D2→P-P/P-B/P-F; D3→P-B; D4→P-I; D5→P-D; D6→P-B; D7→P-F1; D8→P-I; D9/D10→P-S (+P-P rider); D11→P-Z ceilings, test scaffold at first landing). Nothing deferred; nothing trigger-clause'd.
...
10. WIDGET_TREE section 10 closes with this wave; no revival triggers anywhere in this doc (Peter's directive).
