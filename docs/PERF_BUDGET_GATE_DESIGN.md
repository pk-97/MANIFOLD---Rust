...
**Status:** SHIPPED (P1/P2/P2b/P3) 2026-07-16/17 — `cargo xtask perf-soak` headless frame-budget gate; profiled GPU attribution with scoped multi-executor tags (D6 as corrected); bare-glb import-graph mode (BUG-189 (import-graph-10ms-resolution-independent-gpu-floor) its first customer); landing-protocol + pre-gig-checklist wiring; D8 per-label `passes` breakdown. Tool gaps tracked as bugs, not phases: BUG-191 (`--start` seek-frame spike); continuously-animated imports never satisfy the byte-stable warmup gate (noted in BUG-190 (brainstem-24-skinned-objects-370ms-per-frame)). Approved 2026-07-09 (Peter); the 2026-07-14/16 amendments are folded into the decisions. · Fable
