...
| Moments cold-start | `raytrace.rs:2818` (comment), `:2916` (`moments_write.write(float4(cur_luma, cur_luma², cur.a, 1.0), tid)` on reset) | a rejected texel resets variance to ZERO and history to 1 — the stage-3 early-out (`√var < EARLY_OUT && spatial_sd < EARLY_OUT`) then passes it through unfiltered. **The measured mechanism of BUG-27bs (RT spatial denoise inert under continuous motion), stated as hypothesis pending P0 confirmation** |
...
| Reflection-channel machinery | variance gain + Karis neighborhood clamp (BUG-dx6w (specular history neighborhood clamp) / BUG-axe9 (tone-mapped variance clip) lineage, stage-3 D1); SV-ACCUM has its own moments per caster slot (`raytrace.rs:2819`) | ⚠ VERIFY-AT-IMPL at P0: whether refl carries per-texel history length in an addressable channel — read the refl accumulate kernel's moments writes, do not trust this doc |
...
- **D1 — Filter scope for motion is decided by measurement, not by guess (P0).** Stage-3 scoped the post-filter to demodulated irradiance; under motion the dominant boil carrier is unknown (the helmet's glossy shell points at reflections; the shell's GI gradients point at irradiance). P0 runs a per-channel temporal-boil census under continuous motion (irr vs refl vs sv, rt-capture channel taps already exist) and the winning channel(s) become P2's scope. Rejected: extending to all channels up front (stage-3 D1's bandwidth argument stands); rejected: assuming irradiance (today's evidence says filtering it changed ~nothing visible).
...
### 3.3 P2 scope extension mechanics (shape only — P0 names the channel)

Whichever channel(s) P0 names get the same treatment as irradiance: a `rt_<ch>_filtered` pair ensured alongside the existing history, `atrous_post` reused (it is channel-agnostic: params/depth/normal/moments/src/dst), composite rebind at the channel's existing consumption seam. If P0 names reflections: ⚠ VERIFY-AT-IMPL — refl history-length channel existence (section 1 table); if refl has no per-texel n, P2a adds it to the refl moments write (mirroring `moments_write.w` at `raytrace.rs:3322`) before any filtering, as its own committable step with a gpu-proofs value test.
...
### P0 — Motion-noise census (lead, not a lane; no production code)

- **Entry state:** stage-3 on main (tip 5c8d55d4b); rt-capture harness (`--frame-clock --capture-every 1 --animate`); RtMotionHelmet fixture; the 2026-08-28 /tmp capture sets (rerun if cleaned).
- **Read-back:** this doc's D1, section 1 (Audit); the rt-capture channel list (`render_scene.rs:6493-6560`).
- **Deliverables:** per-channel (irr_accum, refl_history_write, mask, moments) frame-to-frame |delta| series under continuous motion (RtMotionHelmet orbit + helmetGlitches rotation), mean/p99; the mechanism confirmation — dump `moments.w` distribution across frames under motion (hypothesis: median n_eff stays ≪ 20); the refl history-length ⚠ VERIFY-AT-IMPL resolved (read the refl accumulate kernel, state the channel or its absence); a one-paragraph scope verdict written into section 9: which channel(s) P2 filters.
- **Gate:** the numbers exist in the doc; scope verdict names channel(s) with their measured share. No code, no clippy.
- **Demo:** none — L1 (numbers are the artifact; Peter reads the verdict).

### P1 — History-aware strength (lane: pro)

- **Entry state:** P0 verdict in section 9. Re-verify anchors: `atrous_post` sigma arithmetic and early-out (raytrace.rs, post-stage-3); `moments.w` clamp site (`raytrace.rs:3089`).
...
### P2 — Scope extension to the P0 channel(s) (lane: pro)

- **Entry state:** P1 on main; P0 verdict. Re-verify: the named channel's history/moments textures and composite consumption seam (file:line from P0).
- **Read-back:** D1, section 3.3, I1/I3; stage-3 D1 (why these channels were excluded) — restate what P0's numbers changed.
- **Deliverables:** `rt_<ch>_filtered` pair; dispatch + composite rebind at the named seam; if the channel lacks per-texel history length: P2a's moments-write addition with its value test (section 3.3); gpu-proofs value test mirroring P1's.
- **Gate:** clippy `-p manifold-gpu -p manifold-renderer`; gpu-proofs green (lead); helmet motion A/B: the named channel's boil down ≥15%, composite down with it; perf on apricot 4K rays 100%: added ms recorded, ≤2 ms over stage-3.
...
- **Deliverables:** `irr_filtered_feedback_valid` persistence + clearing rules; the feedback read-address change; reset-frame bypass; I2′ poison test; I6 convergence test; (same for P2's channel if P0 scoped one in — its feedback rides the same seam).
- **Gate:** clippy `-p manifold-renderer`; gpu-proofs green (lead); helmet motion A/B: composite motion boil vs post-P2 build — the convergence number, target ≥2× reduction in median per-texel n_eff-shortfall frames (metric stated at P0 from the census); **gesture ghost gate**: RtEmissiveStrength continuous intensity ramp + one-shot ambient snap (apricot fixture) — frame-to-frame |delta| tail after the cue returns to baseline within stage-3's window +2 frames, no sustained offset (the ghost detector); strobe leg: 4 Hz intensity square, no frame shows the previous phase's content (pixel-diff at phase boundary vs raw-history control run).
...
1. Motion filter scope is P0-measured, not assumed (D1).
...
- **sv (shadow visibility) channel post-filtering** — SV-ACCUM + per-caster moments + snap-hold already own that channel; trigger: P0's census names sv as a top-2 boil carrier AND its machinery reads insufficient after P1/P2 land.
