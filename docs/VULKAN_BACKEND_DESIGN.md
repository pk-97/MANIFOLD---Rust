...
**Prerequisites:** none (Phase 0 scaffold shipped). Background track in `docs/DESIGN_BUILD_ORDER.md` — nothing in waves 0–3 waits on this.
**Execution contract:** read `docs/DESIGN_DOC_STANDARD.md` section 5 (Phase briefs)–section 6 (Seam briefs) before starting any phase. The `manifold-gpu` inventory here is a 2026-07-02 snapshot and this is the deepest doc in the build order — re-derive the API-surface inventory at the start of every phase.
