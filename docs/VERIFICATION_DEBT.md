...
Landed 2026-07-20 (P3 lane `cb2347b6` + P1a, merged together). The P3 gate names "drag flow (the `drag-clip.json` pattern, card variant)"; no card-drag flow exists on disk, so the landing ran the existing `timeline` `drag-clip.json` (green, L3, proves the drag input path generally) plus the new pure-math drop-target family and the INV-3 pin (`inv3_drag_targets_follow_live_bounds_after_in_place_scroll`, which drives through to the dispatched `EffectReorder`). Burn-down (updated 2026-07-21, P5 sweep): authoring was ATTEMPTED and is blocked on BUG-296 — the drag fires the real input path (`dispatched EffectReorder(0, 2) (structural=true)` in the `inspector` fixture scene) but the `--script` driver never rebuilds the inspector cards after a structural dispatch, so no honest post-drag assert can pass. Close when BUG-296 (with siblings BUG-234/293/294, the flow-driver verification-infra family) is fixed and the flow exits 0. The dispatch-level evidence above is the current best proof of the drag path.
