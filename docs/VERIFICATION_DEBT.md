...
**BUG-234 half burned down 2026-07-21** (lane/bug234-modulation-tick): `Step`'s frame loop now runs `evaluate_modulation` (drivers + envelopes) plus `reconcile_state` every stepped frame, so a flow CAN assert a changing param value across `Snapshot`s. `scripts/ui-flows/envelope-modulation.json` (new `envmod` fixture) proves it: a Bloom `amount` envelope reads base 0.50 → 4.85 one frame after the clip's rising edge → back to exactly 0.50 once elapsed clears 1 beat, confirmed to fail without the wire — and `reconcile_state`'s `sync_param_value` drives the slider's fill/thumb norm off the same `effective` value as the text, so the visual-fill half this entry originally flagged as "BUG-235" is very likely fixed too, riding the same wire. **Stale cross-reference found, not resolved this session:** the "BUG-235" this line pointed at (2026-07-17, slider-fill-mid-flow) is no longer that bug — `docs/BUG_BACKLOG.md`'s current BUG-235 is `manifold-own-kick-fixtures-systematic-adtof-timing-bias`, an unrelated audio-timing entry; the slider-fill bug has no live BUG-NNN. Burn-down remaining: whoever next touches this — confirm the slider-fill visual with a rendered PNG diff (not just the value-text assert this session added), file a fresh BUG-NNN only if it's still actually broken, then close this entry.
