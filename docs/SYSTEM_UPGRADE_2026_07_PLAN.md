...
- **W1-B gate cleanup:** CORRECTED 2026-07-20 at briefing — BUG-252 (eight-scene-flow-scripts-dead-at-step-2-on-stale…)'s 8 flows were retargeted and fixed 2026-07-18 (`f101a585`/`d28bfff4`), NOT dead; only `scene-setup-scrub-fine.json` (BUG-240 (scrub-fine-flow-tests-a-retired-shift-fine-delta…)) dies. Scope now: that one script + retire ALL generated-vs-hand kernel parity tests and their hand-kernel oracles (Peter's ruling 2026-07-20 — migration scaffolding; fusion proofs and freeze proofs KEPT, headless render oracle untouchable) + look-oracle demotion note in HEADLESS_UI_HARNESS.md. Exact lists: the Wave 1 brief set (adversarially reviewed 2026-07-20, 11 findings folded).
- **W1-C BUG-266 (inspector-tab-pin-dies-on-incidental-selection-c…):** tab pin dies on incidental selection changes — decouple pin invalidation from `selection_version` (disjoint: `ui_state.rs`/`state_sync.rs`).
- **W1-D BUG-267 (inspector-duplicated-card-lists):** unify master/layer card vecs in `inspector.rs` (the wave's real-code lane; structural, goes BEFORE BUG-265 (inspector-card-drag-indicator-stale-geometry) which touches the same lines).
...
## Wave 2 — COMPLETE 2026-07-20 (W2-A landed `ab388a35`: eye-toggle + fog-slider dispatch gap tests; its display-value family returned ZERO tests because the lane's git-blame check found the chain covered since 2026-07-18 — fork gap-claims must be verified against recent landings, same rule as dispatch. W2-B + W2-C LANDED 2026-07-20: `f2ac71d9` BUG-265 live-bounds fix (closed `30f0cf05`), `66fe273f` drag survey — BUG-265-class recurs nowhere; BUG-280 (marker-drag-unguarded-mid-gesture-race)/281 stomp-flicker + BUG-282 (graph-canvas-unbound-param-scrub-floods-undo-sta…) per-tick undo flood filed. W2-A remains. Incident note: both W2 lanes were double-dispatched by the orchestrator after context compaction erased the memory of the first dispatch+landing; W2-C's re-run verified-and-reported, W2-B's re-run produced a duplicate commit `b8c0b683`, dropped 2026-07-20 — branch reset to the merged tip. Lesson enforced: BEFORE dispatching any lane, check git for the lane branch and landed work — never dispatch from memory of the plan alone.)

- **W2-A test families:** state-level UI tests copied from exemplars Fable hand-writes (hit-test geometry math; click→command dispatch; display-value resolution à la BUG-260 (scene-panel-bound-rows-display-stale-def-value-n…) conviction test). Lanes replicate patterns; zero new infrastructure permitted.
