...
**Interaction follow-up (2026-09-26):** the searchable layer parameter chooser and independent lane hide/pin/order are implemented. Point previews appear on press, curve insertion preserves shape, and bending follows the pointer on rising and falling segments. Drag selection now captures time ranges across lanes; clipboard operations include sampled boundaries, preserve outside automation, and undo as one edit. These changes supersede the corresponding gaps below; the audit remains a dated snapshot. Master/group editors, stretch/skew, and accessibility work remain tracked in BUG-i70.
