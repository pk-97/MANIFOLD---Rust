...
**Status:** IMPLEMENTED · 2026-09-23 · Codex. V2 presets, detector/tracker and masks are present; legacy Blob Track is preserved. Owed: allocation measurement (BUG-7bi8) and visual/mask refinement (BUG-sh4i). Evidence and refinement history: §7.
...
**Execution contract:** P1–P4 ownership and run constraints are recorded in §7; current AGENTS.md and landing gates still apply.
...
The BUG-sh4i visual refinement adds **Detection: Brightness / Edges** to `BlobTrackingV2`. Both modes share Threshold, Denoise and area controls; colour and motion remain separate presets. Brightness stays the default, including when saved projects omit the new parameter. Edges measures local contrast after resize and denoise using `node.edge_slope` (Texel, Clamp) → `node.vector_length` → `node.scale_offset_image` (scale 8) → detector. An existing texture switch selects the source. This is an artistic region detector: connected edges can form one large outline, and area still counts foreground pixels rather than the bounding rectangle. This addition applies to the HUD preset; `MaskBlob` keeps its filled brightness-region semantics.

**D6 — Shape fidelity first; explicit limits on temporal claims.** Smooth the tracked boxes and estimate velocity. Mask coverage defaults to the latest observed component pixels; it does not warp stale silhouettes to predicted boxes. The optional **Shape** control blends this coverage toward filled smoothed track boxes, using the same coordinates as the HUD. Only observed tracks contribute in either case. Brief disappearance retains identity, not a fabricated foreground. Feathering is spatial smoothing. Rejected: pretending box interpolation solves temporal silhouette reconstruction.

**D7 — Use the existing fixed-lag analysis schedule.** Deterministic response deadlines are preferable to output depending on thread speed. Reuse the optical-flow worker/readback infrastructure, with publication at the ready-readback update. A slow native call can still delay the deadline; measure that cost. No global analysis cache, additional thread pool, shared lock or unbounded queued work.
...
Expose **Separate** as integer 0–3 analysis pixels, default 0, on all six HUD/mask presets. After source preparation and denoise (and after edge selection in the main HUD preset), apply existing `node.mask_extrema` passes: X erosion → Y erosion → X dilation → Y dilation, then detect regions. This grayscale opening cuts thin connections while restoring the surviving regions toward their original extent. It can remove small or thin foreground and change which regions pass the area limits; it does not promise to split every touching shape. Zero is an exact bypass. Four bindings drive the shared radius, with scale −1 on erosion and +1 on dilation. Masks and HUD consume the same processed detections; mask **Expand** remains a separate post-detection coverage control.
...
The visual refinement also exposes **Max Box Area** (0–1, default 1) on all six HUD/mask presets. It limits the enclosing rectangle as a fraction of the image, independently of foreground pixel area. This rejects sparse frame-spanning outlines before the native candidate sort and Max Blobs cap, so smaller accepted regions can fill the available slots. Rejected components also disappear from the categorical label image, keeping tracking and masks in agreement. The default 1 preserves existing looks; 0 selects no regions. The native extension `BlobDetectorV2_ProcessBounded` appends the limit to the existing process arguments; the original process export and V2 record/options layouts remain unchanged and use an unrestricted bound. Rust requires the rebuilt bundle with the new symbol and reports missing exports explicitly.
...
Mask graph: observed labels + tracked membership → `node.region_mask` → signed expansion → Gaussian feather → invert → amount → validity gate → final output. The main `MaskBlob` preset adds **Shape** 0…1 (default 0: component silhouette; 1: filled smoothed bounding boxes; intermediate values crossfade coverage). Expose **Selection** All/Largest (default All), **Expand** −32…32 analysis pixels (default 0), **Feather** 0…8 analysis pixels (default 1), **Invert** false, **Amount** 0…1 default 1. Perform expansion/feather at analysis resolution and upscale coverage linearly at the end. Labels themselves must never be linearly sampled.
...
Submit a ready readback to the worker, then consume its response in that same update before scheduling another capture. This removes the additional worker-publication frame: a capture can appear on the next update when its GPU readback is ready. GPU readback remains nonblocking; native completion has a fixed deadline and may stall that update. Readback and worker share no mutable allocation concurrently. Test the deadline with actual mask pixels, including stops, reversals and empty captures. Reset cancels/drains pending readback, increments generation, clears validity/labels/tracks, and rejects late replies while reclaiming their buffers. Resize, explicit reset, source replacement, seek/retrigger, disable/re-enable and preset removal must use the existing runtime lifecycle. Prewarm pipelines, analysis textures, buffers and native handle through existing warmup hooks; no first-use compilation during the acceptance gesture.
...
| Default silhouette coverage preserves component pixels | `blob_v2_group_mask_ring_and_dry_input`; ring hole stays dry, region wet, exterior dry, nested group and partial wet/dry |
| Optional bounding-box coverage blends through the live Shape binding | `blob_v2_mask_shape_blends_to_tracking_boxes`; ring hole coverage at 0, 0.5 and 1, exterior dry |
...
Implementation and refinement evidence: IMPLEMENTED · 2026-09-23 · Codex. Six new presets, native V2 ABI, shared detector/tracker, mask/colour/motion graphs and menu entries are present; V2 shipped without changing the original Blob Track. Focused GPU, UI and 1080p app performance proofs passed. On an M4 Max, the default 600-frame V2 app-tick p95 added 0.67 ms over legacy; detector worker p95 was 0.234 ms with one-frame readback age and two-frame capture-to-output age. Process malloc-zone retained bytes and blocks are reported; transient allocation events and detector-only retained bytes remain unmeasured under BUG-7bi8. Follow-up refinement under BUG-sh4i: the 2026-09-24 V1 correction removes global histogram equalization after a held-out liquid clip exposed merged regions and detection dropouts. Modifier Group headers now offer Preview Mask, opening the existing graph monitor on final coverage with normalization off. V2 now also offers Edges detection for local contrast, with Brightness retained as the default. Further visual tuning, overlay responsiveness and mask refinement remain open.
