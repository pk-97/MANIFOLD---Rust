...
**Phase 5 (workspace test gate) and Phase 6 (docs sweep + snapshot UI extension) remain** — the workspace `cargo test --workspace` run + canonical-fixture visual sanity check on `Liveschool Live Show V6 LEDS.manifold`; the doc updates to `NODE_CATALOG.md`, `BUFFER_PORT_PLAN.md`, `DECOMPOSING_GENERATORS.md`, `PRIMITIVE_LIBRARY_DESIGN.md`, `ADDING_PRIMITIVES.md`; the editor's `PortKindSnapshot::Array` extension carrying channel-name metadata so hover-tooltips can show channel lists.

**Scheduled follow-up (post-migration):** Build the explicit-marker (`// @channel_skip`) preprocessor for `wgsl_compute` pad-field handling before user-authored `wgsl_compute` becomes a primary surface for non-core-dev authors. See §8.2 for the rationale on why the v1 `_pad[0-9]*` name-prefix heuristic ships in the meantime.

**Acceptance criteria after Phase 4b:** 839/839 manifold-renderer lib tests passing; clippy clean; `check-presets` reports 49/49 OK; three affected presets (BlackHole, ComputeStrangeAttractor, ParticleText) visually verified.
...
**v1 (shipped): skip fields whose name matches the `_pad[0-9]*` prefix.** Phase 4a ships the name-prefix heuristic as the pragmatic mechanism, consistent with the uniform-member walker at `wgsl_compute.rs:654` that has used the same convention since long before this migration. The naga struct walk's `struct_members_to_specs` helper filters fields whose `member.name` starts with `_pad`. The byte layout matches because std430 padding for `position: Vec3F, velocity: Vec3F, ...` reproduces the same offsets that the explicit `_pad` fields encoded.

**Known fragility — explicit-marker preprocessor scheduled as follow-up.** The original sign-off pass agreed to ship an explicit per-field marker (`@channel_skip` WGSL attribute or `// @channel_skip` doc-comment marker) instead of the name-prefix heuristic. The Phase 4a implementation deviated for two reasons: (a) consistency with the existing uniform-member walker — shipping a different mechanism for storage structs only would create a split-brain convention within the same file; (b) implementation cost — naga rejects unknown WGSL attributes outright, so the explicit-marker route requires a pre-naga preprocessor pass (regex-extract comment markers from the source, build a per-struct-field skip set, feed into the naga walker output). The preprocessor is ~100-200 lines including tests for the comment-placement edge cases (multi-line struct decls, mixed `//` / `/* */` comments, nested structs).

The fragility for first-time `wgsl_compute` authors is real: someone naming a non-padding field `_pad_thing` or `padding` would lose it from the wire silently *at authoring time*; the loss only surfaces later when they try to wire the field into a downstream consumer and the validator reports a missing channel. That's not silent corruption — the wire fails to connect with a clear error — but it IS authoring friction that the explicit marker would eliminate.

Slotted as a follow-up: **build the doc-comment preprocessor (`// @channel_skip`) before user-authored `wgsl_compute` becomes a primary surface for non-core-dev authors.** That work bundles naturally with whatever ergonomic pass introduces user-authored `wgsl_compute` to the editor at scale. Until then, the `_pad[0-9]*` convention is documented as the v1 mechanism and the existing core-dev shaders continue to follow it without modification.
...
### 14.9 Explicit-marker preprocessor for `wgsl_compute` pad-field handling — **scheduled follow-up, not deferred indefinitely**

The Phase 4a `wgsl_compute` naga walk ships with a `_pad[0-9]*` name-prefix heuristic for skipping padding fields (rationale in §8.2). The original sign-off had agreed to ship an explicit per-field marker (doc-comment `// @channel_skip` extracted by a pre-naga preprocessor) instead; the implementation deviated for consistency with the existing uniform-member walker and to avoid the preprocessor's ~100-200 lines of edge-case handling in the same pass.

**Why this isn't "defer indefinitely":** user-authored `wgsl_compute` is a current live surface, not a theoretical future capability. Peter authors shaders in `wgsl_compute` for his shows. Today's `_pad*` heuristic places authoring friction on those users — naming a non-padding field `padding` (without the underscore prefix) silently loses it from the wire; the error only surfaces later when wiring downstream and the validator reports the missing channel. That's not silent corruption, but it IS the kind of papercut that compounds across many sessions.

**Scope of the work:**

- Pre-naga regex pass extracting `(?m)^\s*//\s*@channel_skip\s*\n` markers and recording their (struct-name, field-name) positions.
- Plumb the skip set through to `struct_members_to_specs` alongside the existing `_pad*` filter (keep the heuristic as a fallback for the existing core-dev shaders; the explicit marker becomes the *preferred* mechanism for new shaders).
- Tests for edge cases: multi-line struct declarations, mixed `//` and `/* */` comments, nested structs, the marker on the same line as the field declaration.
- A migration sweep porting existing core-dev `_pad*` fields in `particle_common.wgsl` / mesh shaders to the new marker, *or* an explicit decision to keep both mechanisms supported permanently with the heuristic as a fallback.
- Doc update: §8.2 amends from "v1 compromise" to "v1.5 explicit marker shipped" once the preprocessor lands.

**Trigger timing:** before the next ergonomics pass on `wgsl_compute` authoring (editor improvements, syntax highlighting, error visibility, etc.) makes user-authored shaders a more prominent surface. Or sooner if Peter explicitly hits the friction in his own shader work.
