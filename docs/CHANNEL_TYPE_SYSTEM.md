...
- **Phase 4b.7 shipped** — commit `<TBD>`. Skip set threads through `introspect` → `element_to_array_type` → `struct_members_to_specs`; the storage-struct `_pad[0-9]*` name-prefix heuristic retires entirely (no current core-dev shader exercised it — the audit found every `_pad*` field in the five wgsl_compute presets lives in *uniform* structs, which keep their separate `parse_uniform` `_pad*` filter). Authors now have one mechanism (the explicit marker) and one default behaviour (every named WGSL field becomes a channel). Three new integration tests exercise the marker end-to-end + the post-heuristic behaviour + per-struct isolation through naga.
...
**Scheduled follow-up:** ~~Build the explicit-marker (`// @channel_skip`) preprocessor for `wgsl_compute` pad-field handling.~~ Shipped 2026-05-28 in commits 4b.6 + 4b.7. See §8.2 and the §14.9 historical note for the final form.
