...
**Resolution: skip fields explicitly marked as padding via a per-field annotation.** Phase 4 ships with the most ergonomic explicit-marker mechanism naga supports. The candidate mechanisms, in preference order:

1. A WGSL field attribute (e.g., `@channel_skip`) — preferred if naga can be made to accept a custom attribute cleanly (likely requires a parser extension or a pre-naga preprocessor pass that strips it before naga sees the source).
2. A doc-comment marker (`// @channel_skip` on the line immediately preceding the field) extracted by a lightweight preprocessor that maps marker positions to field indices before naga parses.

The `_pad[0-9]*` name-prefix heuristic used widely in the existing core-dev codebase (particle_common.wgsl, mesh shaders, etc.) is **NOT** elevated to a validator-recognized contract. The earlier draft of this doc proposed it; the concern that surfaced during sign-off is that user-authored `wgsl_compute` shaders — which now drive a typed wire signature naga walks at parse time — can't be relied upon to follow a name-prefix convention, and a user naming a non-padding field `_pad_thing` would silently lose it from the wire with no obvious failure mode at authoring time. The explicit marker is self-documenting and works for first-time authors.

The byte layout still matches because std430 padding for `position: Vec3F, velocity: Vec3F, ...` reproduces the same offsets that the explicit `_pad` fields encoded. Migration: Phase 4 ports the existing core-dev shaders from `_pad*` naming to the chosen marker mechanism in the same pass that lands the naga walk.
