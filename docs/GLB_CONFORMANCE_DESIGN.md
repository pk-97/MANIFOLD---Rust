...
**Status: SHIPPED · 2026-07-15 · Fable 5 (authored) + Sonnet 5 (G-P1+G-P2 executed and landed same day, `909976d2`; G-P3+G-P4+G-P5 executed and landed same day, session 2; G-P6 executed and landed same day, session 3, `017e1e41`; G-P7 executed and landed same day, session 4). All seven phases SHIPPED: G-P1 (conformance harness), G-P2 (cap deleted, import is 1:1, BUG-163 fixed as a side effect), G-P3 (anisotropic filtering), G-P4 (KHR_texture_transform all five map families + specular/ior F0), G-P5 (clearcoat lobe, factor-only), G-P6 (node.hdri_source, env_mode card switch, Softbox stays default), G-P7 (full 148-asset Khronos manifest classified — 56 expect_pass / 92 xfail / 0 unclassified; certification record `docs/GLB_CONFORMANCE_STATUS.md`; 8 new bugs logged BUG-166–BUG-173 for genuinely-new gaps the full sweep surfaced, none fixed this phase per its own scope).**
...
### G-P7 — burn-down and certification — SHIPPED 2026-07-15 (session 4)

- **Entry state:** G-P1–G-P6 landed; conformance sweep green on its expect_pass
  set.
- **Executed 2026-07-15:** manifest grew from 8 to 148 assets — the FULL
  Khronos glTF-Binary suite (118 assets) at the pinned commit, minus the 7
  already classified plus TextureTransformTest (already `expect_pass` via its
  G-P4 sidecar fetch) = 111 newly classified, plus 29 assets with no
  glTF-Binary variant at the pin (`xfail:G-P7`, explicit-absence entries per
  ...
  crate-level JSON parse gap, a materialless-primitive-skipped gap, a
  no-default-scene rejection, and an intentional-by-design 1024-object-cap
  hit) — none fixed this phase, per its own scope (classification +
  certification, not fixing). BUG-163 was already flipped to FIXED in session
  2 (G-P2) — verified, untouched. Certification record:
