...
- **D5 — Reflection math + the shading fix.** Plane: unit axis `â` (from `axis`), point `d·â` (from `plane_offset`); `M = I − 2ââᵀ`. Per instance: `pos' = M(pos − dâ) + dâ`; uniform scale negates (`w' = −w`); rotation stored as the PROPER rotation `R' = M·R·M` (det +1, Euler-representable; for axis-aligned M the conjugation is a closed-form sign/permutation of the three angles — derive against the shader's actual Euler order, ⚠ anchor in section 1). `rot_pad.w` becomes an in-band marker: `0` = original slot, `axis+1` = mirrored slot. The mesh vertex shader, after applying the stored rotation to the normal, flips the normal component `int(rot_pad.w)−1` when the marker is nonzero — yielding the exact mirrored normal `M·R·n`. Rejected alternatives: per-draw uniform flag (buffer side-channel the draw path can't see without plumbing), two-sided lighting for all draws (touches every scene's shading for one feature), wrong-by-one-axis normals (visible exactly on the surfaces a mirror shows most). Consequences, stated honestly: `rot_pad.w` semantics change from "always zero" to "producer-defined marker" — every existing producer already writes 0 (bytemuck zero-fill + explicit writes), and the marker is only meaningful on a buffer this atom produced; a future second marker consumer must coordinate.
...
Kernel semantics (pseudocode, one thread per OUTPUT slot, `cap` = input capacity):

- slot `j < cap`: copy input `j`; if input slot `j` has zero scale (dead, per INV-MR5) or `enabled == 0` → zero-scale.
- slot `cap + j`: if input slot `j` has nonzero scale and `enabled != 0` → mirrored transform per D5, marker `axis+1`; else zero-scale.
...
- **INV-MR3 (marker discipline):** `rot_pad.w ∈ {0, axis+1}`; originals always 0; only `node.reflect_array` writes nonzero. *Enforcement:* gpu_test asserts markers; phase-brief negative `rg` gate on other primitives touching the marker word.
