...
- **D7 — RT inherits the instancing limitation.** RESOLVED 2026-09-05 by RT_INSTANCING_DESIGN.md (rt-instancing-wave, BUG-mmkr (rt-instancing-wave)) — the RT accel is now instance-aware end to end: one TLAS slot per live raster slot, mirror marker folded GPU-side, proven exact for all three mirror planes (RT_INSTANCING_DESIGN.md D5 P2 verdict). Mirrored copies trace in RT reflections/shadows/GI exactly as the raster draws them. Original text: mirrored copies rendered in the raster path only; RT kept seeing the object's base transform. The "shipped look that needs mirrored geometry IN RT reflections" trigger fired same-day.
