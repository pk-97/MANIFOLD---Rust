...
**Status:** IN PROGRESS 2026-07-16 (Sonnet 5) — P1+P2 SHIPPED: BUG-165/169 (P1, root cause: fixed near-clip plane) and BUG-166/172 (P2, D1 slice-based import + D5 scene fallback) fixed; certification 56 → 61/148. BUG-170 (D8) verdict: no crate bump exists, deferred to `GLTF_ANIMATION_DESIGN.md`. P3 SHIPPED WITH TWO NAMED GAPS, both logged rather than silently passed: D2 spec-gloss→metal-rough conversion is built and BUG-167-fixing (imports, converts factors/roughness/specular, no longer black), but its own Deferred item (specularGlossinessTexture RGB tint) turns out to visibly matter on `SpecGlossVsMetalRough.glb` itself — the two halves no longer look broken but aren't pixel-alike yet (documented in the manifest note, not silently claimed). D4 synthetic default-material entry is built and BUG-171-fixing (materialless geometry no longer silently dropped, imports as 1 object), but COLOR_0 (vertex color) wiring — which the doc flagged VERIFY-AT-IMPL and said was in-scope if missing — turned out to require a structural MeshVertex GPU-ABI change affecting ~30 shared primitives, past this phase's escalate line; logged as BUG-177, not built. Certification 61 → 63/148 (both flipped assets pass their (honest, capability-scoped) checks). P4 (D3 per-map samplers + D6 instancing) not yet built. New finding during P2: BUG-174 (unlit materials import as lit, not routed to `UnlitMaterial`) — logged, not in this doc's scope. · 2026-07-16 · Fable 5 design, Sonnet 5 execution
