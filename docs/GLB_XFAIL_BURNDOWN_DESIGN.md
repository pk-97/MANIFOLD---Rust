...
**Status:** SHIPPED 2026-07-16 — all four phases on main; certification 56 → 65/148. Residual gaps logged, not built here: BUG-177 (COLOR_0 vertex color never read — structural MeshVertex ABI change), BUG-174 (unlit materials import as lit), the spec-gloss RGB-tint deferred item, and `VirtualCity.glb`'s separate convergence gap. Per-phase as-built record: section 9 (As-built record). · 2026-07-16 · Fable design, Sonnet execution
...
- **P3:** BUG-167 (spec-gloss-pbrSpecularGlossiness-unhandled) / BUG-171 fixed with two named gaps rather than silent passes — D2 spec-gloss→metal-rough conversion ships but its Deferred item (specularGlossinessTexture RGB tint) visibly matters on `SpecGlossVsMetalRough.glb` itself (halves aren't pixel-alike; manifest note); D4 synthetic default-material entry ships but COLOR_0 wiring needed a structural MeshVertex GPU-ABI change past the escalate line — logged as BUG-177 (COLOR_0 never read), not built.
