...
**Status:** SHIPPED 2026-07-16 — all four phases on main; the historical certification count remains 56 → 65/148. The residual-gap wording below is historical: current material and vertex-layout corrections are governed by [GLTF_MATERIAL_EXTENSIONS_DESIGN.md section 7](GLTF_MATERIAL_EXTENSIONS_DESIGN.md#7-material-fidelity-corrections-2026-09-26); previously imported graphs that discarded those fields require reimport. `VirtualCity.glb` retains its separate convergence gap. Per-phase as-built record: section 9 (As-built record). · 2026-07-16 · Fable design, Sonnet execution
...
- **D4 — Materialless primitives get the glTF default material (fixes BUG-171 (boxvertexcolors-no-material-primitive-skipped-en…)).** The materials list (`gltf_load.rs:656-667`) gains a synthetic entry for `material().index() == None` geometry when `default_material_vertex_count > 0`: glTF default material per spec (base color [1,1,1,1], metallic 1.0, roughness 1.0). The `COLOR_0` verification note is historical; the current 80-byte `MeshVertex` contract carries RGBA vertex colour through import and shading, as specified in [GLTF_MATERIAL_EXTENSIONS_DESIGN.md section 7](GLTF_MATERIAL_EXTENSIONS_DESIGN.md#7-material-fidelity-corrections-2026-09-26).
- **D5 — No default scene falls back to the union of all scenes' roots (fixes BUG-172 (recursiveskeletons-no-default-scene-rejected)).** All three `default_scene()` sites route through one helper: default scene if present; else all nodes of all `scenes[]`; else all parentless nodes. Spec-conformant (the spec says a missing `scene` means "display nothing" is *allowed* but importing is the useful reading — Khronos ships `RecursiveSkeletons.glb` this way deliberately).
- **D6 — Instancing imports as N wired copies, bounded by OBJECT_SAFETY_MAX (fixes BUG-168 (ext-mesh-gpu-instancing-unhandled)).** Parse `EXT_mesh_gpu_instancing` (raw-JSON sniff; no typed support at 1.4.1 — ⚠ VERIFY-AT-IMPL) and expand instances into per-object entries at summary time, so the existing 1:1 object pipeline and D4-of-GLB_CONFORMANCE's safety bound apply unchanged. Rejected: true GPU instancing through `render_instanced_3d_mesh`, because it forks the import graph shape for the one Khronos fixture; revival trigger (Deferred): a real asset whose instance count materially exceeds the object budget.
...
- **Entry:** re-run the two repros; both still fail (commands in section 6).
- **Read-back:** this doc section 2 D7, `render_import.rs:196-291`, BUG_BACKLOG entries 165/169.
- **Deliverables:** per-frame fraction/io_pending trace in `render-import` (a `--trace` flag, default off); root-cause diagnosis written into BUG_BACKLOG for both; the fixes, at whatever level the causes live (escalate per section 4 of the standard if a cause crosses a crate boundary the doc doesn't).
...
## 8. Deferred
- ~~Spec-gloss specular RGB tint → trigger: a real asset where tint visibly diverges (D2).~~ Superseded: RGB tint preservation is part of the active importer contract in [GLTF_MATERIAL_EXTENSIONS_DESIGN.md section 7](GLTF_MATERIAL_EXTENSIONS_DESIGN.md#7-material-fidelity-corrections-2026-09-26).
...
- **P1:** BUG-165 (near-clip plane, `MetalRoughSpheresNoTextures.glb`) / BUG-169 (`BoomBox.glb`) fixed — root cause was the near-clip plane. `VirtualCity.glb` stays `xfail:BUG-165` under a separate, pre-existing convergence gap (multi-texture asset never converges within 300 frames — out of scope).
- **P2:** BUG-166 (gltf-crate-vetoes-extensionsRequired-we-already-support) / BUG-172 fixed — D1 slice-based import + D5 scene fallback. BUG-170 (D8) verdict: no crate bump exists; deferred to `GLTF_ANIMATION_DESIGN.md`.
- **P3 (historical):** BUG-167 (spec-gloss-pbrSpecularGlossiness-unhandled) / BUG-171 fixed with two named gaps rather than silent passes — D2 spec-gloss→metal-rough conversion shipped but its then-Deferred RGB tint visibly mattered on `SpecGlossVsMetalRough.glb`; D4 synthetic default-material entry shipped while COLOR_0 wiring was then logged as BUG-177. Those boundary statements are superseded by the current RGB-preserving spec-gloss and 80-byte vertex-layout contracts in [GLTF_MATERIAL_EXTENSIONS_DESIGN.md section 7](GLTF_MATERIAL_EXTENSIONS_DESIGN.md#7-material-fidelity-corrections-2026-09-26); the 56 → 65/148 certification record remains historical.
- **P4:** BUG-164 / BUG-168 fixed — D3 per-map-family samplers (5 bindings replacing 1; all 65 prior expect_pass goldens re-verified, 18 shifted under tolerance for the correct reason, none regressed) and D6 instancing-as-N-copies (`SimpleInstancing.glb` → 125 wired cube instances).
- Also logged from the sweep: BUG-178 (pre-existing clippy lint under `--tests`).
