...
**Prerequisites:** none for P1–P3 (all phases run against shipped code: the muda menu, the overlay driver, the Chrome API). Future-wave slots (section 8) bind their own waves, not this doc.
...
- Perform-mode entry stays menu-only: **"I think it's best we tuck the perform mode away in the menu so it can't accidentally be clicked and entered or exited."** (2026-07-06). This generalizes into taxonomy rule R5 (section 7).

Companions: `CHROME_API_DESIGN.md` (the widget substrate every shell panel builds on) · `OVERLAY_SYSTEM_DESIGN.md` (the modal/modeless driver settings windows ride) · `UI_LAYOUT_DESIGN.md` + `UI_DESIGN_SYSTEM_AND_INSPECTOR_REDESIGN.md` (layout SSOT + tokens/kit; this doc adds no visual language) · `PERFORM_SURFACE_DESIGN.md` (perform modes constrain what furniture appears when; its section 7.5 owns editor workspaces) · the future-wave docs reconciled in section 8.
...
| **Venue** (the copper and glass) | venue profile (MULTI_DISPLAY #13, future) | stage layout, projector warps, LED patch | venue surfaces (section 8), never a settings window |
...
Persisted as `settings.json` beside the legacy `prefs.json`; on first load, if `settings.json` is absent and `prefs.json` exists, migrate the keys and leave `prefs.json` in place (a downgrade still works); `UserPrefs` the *type* is deleted. `⚠ VERIFY-AT-IMPL:` the exact key set — re-run `rg -o '"MANIFOLD_[A-Za-z_]*"' crates/manifold-app/src/` before writing the migration; the table in section 1 is the 2026-07-06 snapshot. (`MANIFOLD_Export`/`MANIFOLD_Frame` at `app_lifecycle.rs:127/215` — read those call sites to type them correctly.)
...
Help       Keyboard Shortcuts… · [slot: MANIFOLD Manual — when public docs exist (BUSINESS_PLAN section 7)]
```
...
| Page | Rows (v1) | Notes |
|---|---|---|
| **General** | Autosave: debounce interval (30s/60s/2min/5min segmented) · history cap (numeric, default 50) · "Clear Recent Projects" button | The two autosave constants become `AppPrefs` fields (section 3); the archive keeps enforcing the cap. |
...
v1 Settings is one page. That is honest — MANIFOLD has few app-scope settings today, and a page that exists to look complete is forbidden (standard section 7). The window's value is the idiom the slots inherit.
...
| **Sync** | OSC send port · OSC sync mode | Cold config only. Clock authority, Link, MIDI-clock device, SYNC out **stay in the transport bar** — they are live show state (R2, section 7). Quantize stays in the footer for the same reason. |
...
|---|---|---|---|
| MULTI_DISPLAY | Stage View (arrangement, test patterns, Identify, per-output advanced flap) · venue file import/export · output windows | T1 surface + View menu · Project Settings ▸ Displays & Stage summary page · Window menu list | **AMENDED 2026-07-06 (its section 5a):** Stage View is the app's ONE physical-stage surface — hosts projector and LED-fixture objects too; deep tools open as focused modes with breadcrumb. |
| PROJECTION_MAPPING | Mapping surface (Warp/Masks/Blend) | Focused mode inside Stage View (no own panel) | **AMENDED 2026-07-06 (its section 6 home amendment):** entered by selecting a projector on the Stage; tools/math verbatim. |
| LED_STRIPS | Fixture patch (venue profile), protocol outputs | Fixture objects on Stage View; patch detail = focused mode (its section 5a) + Project Settings ▸ LED summary/link | **AMENDED 2026-07-06:** no separate patch panel. D5 venue-profile storage untouched; section 6.3 records the interim field locations. |
...
| AUDIO_INFRASTRUCTURE / AUDIO_SENDS_UX | Audio Setup evolution (rename/color sends, meters, grouped channels) · default-device pref | Audio Setup stays T2 modal (its section 7 decision, quoted in section 1) · Settings ▸ Audio slot | This doc adds nothing to Audio Setup; the sends UX doc owns it. |
| SESSION_MODE / PERFORM_SURFACE | Session View toggle · perform surfaces · editor workspaces | View menu slot · perform is its own chrome-hosted mode (D2 there) · workspaces = PERFORM_SURFACE section 7.5's own future design | Perform entry affordance decided here (D4); everything else is theirs. |
...
Entry state, every phase: re-verify the section 1 anchors the phase touches (each brief lists its own). Test scope per phase, verified once at phase end. All three phases are `manifold-app`/`manifold-ui` only — no content-thread work, no GPU-path work (no gpu-proofs runs; no render-trace gates — nothing here adds content-thread load).

### P1 — Command table + the full menu (one session)

- **Entry state:** `menu.rs` builds by hand (`rg 'fn build' crates/manifold-app/src/menu.rs`); Edit menu has 2 items; `input_handler.rs` owns ⌘X/C/V/D/A/G/⇧⌘G/⌘E (`rg 'Cmd\+' crates/manifold-app/src/input_handler.rs` matches section 1's list).
- **Read-back:** this doc section 2 D5/D6, section 4, section 5; `menu.rs` whole; `input_handler.rs:60-230`; restate the double-fire hazard and the text-input-first routing rule before writing code.
- **Deliverables:** `commands.rs` (table + `EditIntent` + router); `menu.rs` building from the table; Edit/View/Window/Help completed per section 5 (including Monitor → Window, Zoom items); winit branches for claimed keys **deleted**; Keyboard Shortcuts overlay (new `OverlayId` variant, popup_shell, renders the table).
...
- **Read-back:** section 2 D7/D9, section 6.2, section 7 R1/R2; `settings_popup.rs` whole; `audio_setup_panel.rs` sizing/`resize_to_viewport`; the `SetRenderScale` dispatch chain end-to-end (`ui_bridge`) as the worked example for new rows.
- **Deliverables:** sidebar-shell component (`manifold-ui`, Chrome API views: sidebar + scope-header + page body); Project Settings window on it with Video/Playback/Sync pages per section 6.2; new `PanelAction`s (`SetVsync`, `SetPlayerPoolSize`, `SetMaxLayers`, `SetDefaultRecordingLayer`, `SetOscSendPort`, `SetOscSyncMode`) routed like `SetRenderScale`; File ▸ Project Settings… command-table row; **`settings_popup.rs` deleted**, `OverlayId::Settings` re-pointed to the new window (⌘, temporarily opens Project Settings until P3 lands Settings — say so in the phase report, it's a one-phase interim the next phase closes, not a silent fallback).
- **Gate (positive):** new `ui-snap` scene `project-settings` renders the window (Video page active) — PNG read by the orchestrating session; round-trip gate (standard section 5): set VSync off + pool size 6 → save → reload → values persist AND the rows show them (this is serialized state; create-path green is half a gate). Focused libs + clippy.
...
- **Forbidden moves:** keeping the old popup alive behind the old ⌘, path (parallel old path) · OK/Apply buttons (D7) · moving BPM/quantize/clock controls off the performance surface into the window (R1) · inventing settings not in section 6.2's table · storage migration of venue-class fields (D8).
...
- **Entry state:** P2 merged. `user_prefs.rs` exists; re-run the section 3 key sweep (`rg -o '"MANIFOLD_[A-Za-z_]*"' crates/manifold-app/src/`) — if the key set differs from section 1's snapshot, list the delta before coding.
- **Read-back:** section 2 D2/D3/D7, section 3, section 6.1; `user_prefs.rs` whole; every `get_string`/`set_string` call site; `autosave.rs` + `archive.rs` constants.
- **Deliverables:** `app_prefs.rs` per section 3 (typed struct, `settings.json`, one-time migration from `prefs.json`); every `UserPrefs` call site moved to typed fields; **`user_prefs.rs` deleted**; autosave debounce + history cap read from `AppPrefs`; Settings window (same shell, General page per section 6.1); ⌘, re-pointed to Settings; MANIFOLD ▸ Settings… table row confirmed.
...
- **Forbidden moves:** keeping `UserPrefs` as a compatibility layer (D3 rejection) · seeding pages from section 6.1's *slot* rows (no empty License/Audio pages) · writing `settings.json` schema without `#[serde(default)]` on every field (old files must load forever).
- **Test scope:** focused libs + clippy; workspace sweep before merge (persistence).

Future-wave slots (section 8) are not phases of this doc — each lands inside its own wave, bound by R7.
...
9. Audio Setup stays a modal (AUDIO_INFRASTRUCTURE section 7's decision, honored here).
...
- **Editor workspaces / docking** — PERFORM_SURFACE section 7.5 owns it; gets its own design when scheduled.
- **Command palette (⌘K)** — revive if/when MCP's command surface proves the table's coverage; it would read the same table.
- **Menu-bar presence for session/stage/mapping views** — slots reserved (section 5); land with their waves.
...
- **Venue-field storage migration out of `ProjectSettings`** — owned by MULTI_DISPLAY #13; section 6.3 is the classification it consumes.
