...
**Status:** APPROVED design, C-P1 attempted 2026-07-17 (Sonnet, `wave/scene-card-convergence`, slot-1) — **diagnosis done, row-swap implementation NOT done**. The session found and value-proved that Camera/World/Light param addressing/dispatch is correct for a fresh layer (BUG-231), and that the "dead" eye toggles on those rows are an intentional dimmed-placeholder per SCENE_PANEL_UX D5, not a bug (BUG-232) — the orchestrator's eye-slot amendment below fixes that by removing the eye entirely on param-less rows. The full D1–D5 row swap (build_param_row wiring, id map, SceneCardState, drag-protocol migration, bespoke-builder deletion, flow updates, PNG dialect-drift review) was correctly scoped by this doc but exceeded this session's remaining capacity after the diagnosis detour — see BUG-231's entry for the exact next-step brief. Re-run C-P1 as a fresh, fully-resourced session; do not re-diagnose BUG-231/BUG-232, start from their entries.
