...
| `EditValue` | The value cell's existing type-in, opened on **DoubleClick** (D13) and constructed at INPUT time (`inspector.rs:2375` → `route_value_typein` → `value_cell_typein` param_card.rs:877 → `BeginParamTextInput`) — input-time contract consult per D14, never a registry entry | **LANDED P5d (2026-07-13):** a double-click on the value-cell zone (`param_slider_zones`, the same P1 geometry) emits `EditGraphNodeNumericParam`; the app opens a `TextInputField::GraphNumericParam` session, commit parses/clamps/dispatches `SetGraphNodeParam` (or `SetOuterParam` for a group-face mirror row, D4 parity) |
...
today; this is pure internal-plumbing unification, matching every other P7 fold.
Deliverable: `enum AbletonTrimDrag { Min(usize), Max(usize) }` (or an equivalent struct
carrying `index: usize, is_min: bool` — follow whichever shape the existing
`handle_press`/`handle_drag`/`handle_release` call sites in `macros_panel.rs` make
mechanical, per BUG-143's own fix-shape note), `DragController<AbletonTrimDrag>`
replacing the two sentinel fields, begin/read/end accessors at the call sites exactly
as P7.1/P7.2 did for `ParamDragState`/`CanvasDrag`. Pinning test(s) driven through the
real `handle_press`/`handle_drag`/`handle_release` path, written and green BEFORE the
switch, re-verified green AFTER. Gate: `cargo test -p manifold-ui --lib` + `cargo
clippy -p manifold-ui -- -D warnings`; a negative `rg 'dragging_ableton_trim'
crates/manifold-ui/src` after landing should be zero. Close BUG-143's Status line to
FIXED (with commit SHA) in the same landing — supersession-sweep rule, one place per
fact. No design pass required; this is the smallest possible P7-shaped fold, staged as
its own phase only because it was found after P7 itself had already been fully briefed
and landed.
