...
3. **Read the nearest reference preset end-to-end** ([NODE_CATALOG.md section 5 (Effect presets) / section 6.1](NODE_CATALOG.md), [DECOMPOSING_GENERATORS.md section 2.5](DECOMPOSING_GENERATORS.md)).
...
Skipping the audit produces the recurring "argue from snippets" anti-pattern and the bundle-as-primitive shortcut. The section 2.5 precondition is a hard rule in `CLAUDE.md`.

## When to add one

The `≥2-use` filter applies (design doc section 1.2):
...
- **Don't touch the kernel without re-reading the purpose.** `purpose` states the math (`docs/archive/NODE_VOCABULARY_AUDIT.md` section 2.6) and lives right next to the shader in the same file, on screen during the edit — there's no excuse for it drifting. Touch the kernel, re-read the purpose.
