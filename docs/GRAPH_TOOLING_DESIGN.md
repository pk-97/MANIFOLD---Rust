...
| Structured errors | `node_graph/graph_loader.rs:98` `GraphBuildError` | **Exists.** Variants carry `node_id`, `type_id`, `param`, `port` — the MCP section 6 "errors name node + port" requirement is already satisfied at the type level. |
...
| Exemption taxonomy | `ADDING_PRIMITIVES.md` section"The codegen path is mandatory" (landed `ca145923`, 2026-07-11) | **Exists in prose only.** 4 exempt categories + BLOCKED≠exempt. No machine check. |
...
| Mux short-circuit | executor per-frame liveness (`node_graph/execution.rs`, see FREEZE_COMPILER_MAP section"execution") | **Exists** — a mux's unselected branch never executes. This is why discrete switching via `mix`-blend-at-the-rails is a performance bug, not just a style miss: both branches render every frame. |
...
- **D1 — One validation implementation, extracted as a library seam.** New module `node_graph::validate` with `validate_def(...) -> ValidationReport` (section 3), built by MOVING `check_presets::check_one`'s body (including `check_bindings_resolve`) into the library. `check_presets` becomes a thin directory-walker over `validate_def`. Every future consumer — CLI verb, importer hook, MCP `validate_graph` — calls this one function. **Rationale:** the validator's entire value is fidelity by construction (DESIGN_AUTHORING section 4): a validator that is not the real loader will eventually approve graphs the loader rejects. **Rejected:** a standalone JSON-schema/xtask validator that parses graph JSON without the registry — that is a parallel reimplementation and will drift; forbidden by name.
...
- **D6 — The importer validates its own output.** `assemble_import_graph`'s result runs through `validate_def` before it reaches the project; a failure surfaces as the import error path with the report's messages (never a silent partial import — the forbidden move of load paths, DESIGN_DOC_STANDARD section 5 round-trip corollary). **Rationale:** the assembler is code and has bugs; today its mistakes surface as wrong pixels or a load failure far from the cause.
- **D7 — MCP P2 becomes an adapter.** When MCP_INTERFACE executes, its `validate_graph` tool wraps `validate_def` and serializes `ValidationReport` — no new checks in the MCP layer. At this design's landing, add one line to MCP_INTERFACE_DESIGN section 6 pointing here (landing-updates-the-doc rule).
...
Landing (per GIT_TREE_DISCIPLINE): batch P1–P4 as one workstream; full clippy+nextest sweep in the warm main checkout at landing; landing updates this Status line and adds the D7 one-liner to MCP_INTERFACE_DESIGN section 6.
