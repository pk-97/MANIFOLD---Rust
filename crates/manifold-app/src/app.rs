    ...
    fn window_event(
        ...
        match event {
            ...
            WindowEvent::MouseWheel { delta, .. } => {
                if is_graph_editor && self.editor_mouse_wheel(window_id, delta) {
                    return;
                }
            ...
            WindowEvent::KeyboardInput {
                ...
                // scope nav, and the editor shortcuts — runs through one owner
                // (Phase 4.6). Called unconditionally because the leading
                // popover-editing block is ungated (its `is_editing` guard only
                // fires in the editor); the rest gate on `is_graph_editor`.
