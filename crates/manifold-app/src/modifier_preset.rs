...
use manifold_core::{GraphTarget, effect_graph_def::{BindingTarget, EffectGraphDef}, project::Project};
...
pub(crate) fn export_def(project: &Project, target: &GraphTarget) -> Option<EffectGraphDef> {
    let GraphTarget::SceneModifier { modifier_id, .. } = target else { return None; };
    let owner = project.graph_target_owner(target)?;
    let outer = crate::graph_target::resolve(project, target.host_target()?)?.preset_metadata.as_ref()?;
    ...
    for spec in &mut local.params {
        ...
        let Some(source) = sources.next() else { continue; };
        if sources.next().is_some() {
            log::error!("[preset] cannot export modifier control {} with multiple host sources", spec.id);
            return None;
        }
        ...
        *spec = live.spec.clone();
        ...
        for binding in local.bindings.iter_mut().filter(|binding| binding.id == spec.id) {
    ...
    if let Some(recipe) = &mut local.scene_modifier {
        let captured = |id: &str| outer.bindings.iter().any(|binding|
            matches!(&binding.target, BindingTarget::SceneModifier { modifier_id: mid, param_id }
                if mid == modifier_id && param_id == id));
        recipe.calibrations.retain(|calibration| !captured(&calibration.param_id));
        recipe.initializers.retain(|initializer| !local.bindings.iter().any(|binding|
            captured(&binding.id) && matches!(&binding.target, BindingTarget::Node { node_id, param }
...
mod tests {
    ...
    fn local_recipe() -> EffectGraphDef {
        ...
        let mut local = EffectGraphDef {
            ...
            preset_metadata: Some(PresetMetadata {
                ...
                is_line_based: false,
                layer_types: None,
                params: vec![spec("gain", 0.25, MacroCurve::Linear), spec("other", 0.4, MacroCurve::Linear)],
                bindings: vec![BindingDef {
                    id: "gain".into(),
                    label: "Gain".into(),
                    default_value: 0.25,
                    target: BindingTarget::Node { node_id: node_id.clone(), param: "gain".into() },
                ...
                scene_modifier: Some(SceneModifierRecipe {
                    ...
                    initializers: vec![
                        SceneNodeInitializer {
                            target: SceneNodeRef { scope: Vec::new(), node: node_id.clone() },
                            param: "gain".into(),
                            value: SceneScalarExpr::Constant { value: 0.25 },
                        },
                        SceneNodeInitializer {
                            target: SceneNodeRef { scope: Vec::new(), node: node_id },
                            param: "other".into(),
        ...
        local.nodes[0].params.insert("other".into(), SerializedParamValue::Float { value: 0.4 });
        local
    }
    ...
    fn owner_graph() -> EffectGraphDef {
        ...
        let frame = SceneMeshReferenceFrame {
            target: SceneNodeRef { scope: Vec::new(), node: NodeId::new("object") },
            source: SceneNodeRef { scope: Vec::new(), node: NodeId::new("mesh") },
        ...
        EffectGraphDef {
            ...
            preset_metadata: Some(PresetMetadata {
                ...
                is_line_based: false,
                layer_types: None,
                params: vec![spec("outer_a", 0.2, MacroCurve::SCurve), spec("outer_b", 0.3, MacroCurve::SCurve)],
                bindings: vec![
                    BindingDef {
                        id: "outer_a".into(), label: "A".into(), default_value: 0.2,
                        target: BindingTarget::SceneModifier { modifier_id: NodeId::new("a"), param_id: "gain".into() },
                        convert: manifold_core::effects::ParamConvert::Float, user_added: false,
                        scale: 1.4, offset: 0.2, default_mirrors_node_param: false,
                    },
                    BindingDef {
                        id: "outer_b".into(), label: "B".into(), default_value: 0.3,
                        target: BindingTarget::SceneModifier { modifier_id: NodeId::new("b"), param_id: "gain".into() },
                        convert: manifold_core::effects::ParamConvert::Float, user_added: false,
                        scale: 0.8, offset: -0.1, default_mirrors_node_param: false,
                    },
                ],
                param_aliases: Vec::new(), value_aliases: Vec::new(), string_params: Vec::new(),
                string_bindings: Vec::new(), scene_bounds: None, scene_modifier: None,
            }),
            scene_modifiers: vec![
                SceneModifierInstanceDef { id: NodeId::new("a"), scene: SceneNodeRef { scope: Vec::new(), node: NodeId::new("scene") }, targets: SceneTargetSelection::AllObjects, mesh_frames: vec![frame.clone()], graph: Box::new(local.clone()) },
                SceneModifierInstanceDef { id: NodeId::new("b"), scene: SceneNodeRef { scope: Vec::new(), node: NodeId::new("scene") }, targets: SceneTargetSelection::AllObjects, mesh_frames: vec![frame], graph: Box::new(local) },
            ],
            nodes: Vec::new(),
            wires: Vec::new(),
        }
    ...
    fn scene_modifier_export_preserves_affine_calibration_and_isolates_instances() {
        ...
        assert_eq!(gain.curve, MacroCurve::SCurve, "host nonlinear calibration is retained");
        let binding = meta.bindings.iter().find(|binding| binding.id == "gain").unwrap();
        ...
        assert!((expected_before - after).abs() < f32::EPSILON, "exported affine binding preserves forward reshape");
        assert!(exported.scene_modifiers.is_empty(), "host modifier instances never leak into a bare export");
        ...
        assert_eq!(project.graph_target_owner(&target).unwrap().graph, before, "export does not mutate the owner");
        assert_eq!(project.graph_target_owner(&target).unwrap().params, before_params, "export does not mutate live host params");

        let target_b = GraphTarget::SceneModifier {
            owner: Box::new(target.host_target().unwrap().clone()),
            modifier_id: NodeId::new("b"),
        };
        let exported_b = export_def(&project, &target_b).expect("second identical modifier exports");
        let binding_b = exported_b.preset_metadata.as_ref().unwrap().bindings.iter().find(|binding| binding.id == "gain").unwrap();
        assert!((binding_b.scale - 0.4).abs() < f32::EPSILON);
        assert!((binding_b.offset - 0.05).abs() < f32::EPSILON);
        let gain_b = exported_b.preset_metadata.as_ref().unwrap().params.iter().find(|param| param.id == "gain").unwrap();
        assert_eq!(gain_b.default_value, 0.35);
        assert_eq!(gain_b.curve, MacroCurve::SCurve);
        assert_eq!(project.graph_target_owner(&target).unwrap().graph, before, "second export also leaves the owner unchanged");
        assert_eq!(project.graph_target_owner(&target).unwrap().params, before_params, "second export also leaves live host params unchanged");
    }
}
