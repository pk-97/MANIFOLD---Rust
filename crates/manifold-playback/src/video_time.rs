...
pub fn compute_video_time(
    ...
    let in_point_f = in_point.as_f32();
    ...
    let video_time = in_point_f + source_local_time;

    if is_looping && media_length > 0.01 {
        let loop_len_sec = if loop_duration_seconds > 0.0 {
            (loop_duration_seconds * safe_rate).min(media_length - in_point_f)
        } else {
            media_length - in_point_f
        };
...
pub fn compute_video_time_beats(
    ...
    in_point: Seconds,
    is_looping: bool,
    loop_duration_beats: Beats,
    media_length: f32,
    current_spb: f32,
    playback_rate: f32,
) -> f32 {
    let loop_duration_seconds = if loop_duration_beats.0 > 0.0 {
        loop_duration_beats.as_f32() * current_spb
    } else {
        0.0
    };
...
mod tests {
    ...
    fn test_simple_playback() {
        let t = compute_video_time(2.0, 1.0, Seconds(0.0), false, 0.0, 10.0, 1.0);
        assert!((t - 1.0).abs() < 0.001);
    }

    #[test]
    fn test_in_point() {
        let t = compute_video_time(2.0, 1.0, Seconds(5.0), false, 0.0, 10.0, 1.0);
    ...
    fn test_looping() {
        ...
        let t = compute_video_time(12.0, 1.0, Seconds(0.0), true, 0.0, 5.0, 1.0);
        assert!((t - 1.0).abs() < 0.001);
    }
    ...
    fn test_looping_with_in_point() {
        ...
        let t = compute_video_time(4.0, 1.0, Seconds(2.0), true, 0.0, 10.0, 1.0);
    ...
    fn test_beats_overload() {
        ...
        let t = compute_video_time_beats(3.0, 1.0, Seconds(0.0), true, Beats(2.0), 10.0, 0.5, 1.0);
    ...
    fn test_playback_rate() {
        let t = compute_video_time(2.0, 1.0, Seconds(0.0), false, 0.0, 10.0, 2.0);
