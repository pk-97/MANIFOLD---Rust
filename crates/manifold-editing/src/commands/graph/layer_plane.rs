...
mod tests {
    ...
    fn add_layer_plane_builds_group_with_skin_and_undo_restores() {
        ...
        let mut cmd = AddSceneLayerPlaneCommand::new(
            ...
            1.6, // width
            0.9, // height
            vec![super::super::test_support::scene_param_meta(
                "color_r", "Colour R",
            )],
        ...
        let plane = body
            .nodes
            .iter()
            .find(|n| n.type_id == "node.plane_mesh")
            .expect("plane node");
        assert_eq!(
            plane.params.get("width"),
            Some(&SerializedParamValue::Float { value: 1.6 })
        );
        assert_eq!(
            plane.params.get("height"),
        ...
        assert!(
            body.wires
                .iter()
                .any(|w| w.from_node == scene_object_id && w.from_port == "object")
        );
    ...
    fn add_layer_plane_stamps_exposures_and_undo_restores_them() {
        ...
        cmd.undo(&mut project);
        assert!(
            def_of(&project, &lid).preset_metadata.is_none(),
