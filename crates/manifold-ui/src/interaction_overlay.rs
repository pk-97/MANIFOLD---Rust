...
impl InteractionOverlay {
    ...
    fn finalize_move_snap(
        ...
        if let Some(anchor_start) = anchor_start {
            ...
            // D15 "magnetic snap ... visual settle": this correction is the
            // ONE discrete snap-position jump a move-drag makes (per-frame
            // dragging already tracks `magnetic_snap` continuously — see
            // `handle_move_drag` — so residual is normally small). Seed the
            // settle tween with the screen-space gap BEFORE applying the
            // model correction below, so `settle_dx` starts exactly at the
            // pre-correction visual position and eases to 0 (the model's
            // now-final position) — data snaps instantly, only the drawn
            // rect eases (`docs/UI_CRAFT_AND_MOTION_PLAN.md` D15).
            let settle_px = viewport.beat_to_pixel(anchor_start) - viewport.beat_to_pixel(snapped);
            self.settle_dx = AnimF32::new(settle_px, color::MOTION_MED_MS);
            self.settle_dx.set_target(0.0);
            self.landing_flash_beat = snapped;
            self.landing_flash_layers = (self.drag_selection_min_layer, self.drag_selection_max_layer);
