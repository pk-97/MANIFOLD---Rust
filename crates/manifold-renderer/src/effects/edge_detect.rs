...
impl EdgeDetectFX {
    pub fn new(device: &manifold_gpu::GpuDevice) -> Self {
        let spec = |mode: u32, label: &str| {
            device.create_render_pipeline_with_overrides(
                EDGE_DETECT_WGSL,
                "vs_main",
                "fs_main",
        ...
        Self {
            ...
            pipeline_sobel: spec(0, "EdgeDetect Sobel"),
            pipeline_laplacian: spec(1, "EdgeDetect Laplacian"),
            pipeline_frei_chen: spec(2, "EdgeDetect Frei-Chen"),
        }
    }
}
...
impl PostProcessEffect for EdgeDetectFX {
    ...
    fn apply(
        ...
        let mode = mode_raw.round() as u32;
