// Compute variant of fx_infrared.wgsl — same math, no TBDR tile overhead.
...
fn cs_main(@builtin(global_invocation_id) id: vec3<u32>) {
    ...
    let hot_l = dot(textureSampleLevel(source_tex, tex_sampler, uv + vec2<f32>(-texel.x, 0.0), 0.0).rgb, vec3<f32>(0.299, 0.587, 0.114));
    let hot_r = dot(textureSampleLevel(source_tex, tex_sampler, uv + vec2<f32>( texel.x, 0.0), 0.0).rgb, vec3<f32>(0.299, 0.587, 0.114));
    let hot_u = dot(textureSampleLevel(source_tex, tex_sampler, uv + vec2<f32>(0.0,  texel.y), 0.0).rgb, vec3<f32>(0.299, 0.587, 0.114));
    let hot_d = dot(textureSampleLevel(source_tex, tex_sampler, uv + vec2<f32>(0.0, -texel.y), 0.0).rgb, vec3<f32>(0.299, 0.587, 0.114));
    let hot_avg = (hot_l + hot_r + hot_u + hot_d) * 0.25;
    let hot_glow = smoothstep(0.6, 1.0, hot_avg);
    thermal += hot_glow * uniforms.hot_spot * 0.4;

    // Sensor noise
    let noise = (hash(uv * vec2<f32>(uniforms.texel_size_z, uniforms.texel_size_w) + uniforms.time * 137.0) * 2.0 - 1.0) * uniforms.noise * 0.15;
    thermal += noise;

    // Scanline overlay
    let scanline = 1.0 - uniforms.scanline * 0.3 * step(0.5, fract(uv.y * uniforms.texel_size_w * 0.5));
    thermal *= scanline;
