// Compute variant of fx_transform.wgsl — same math, no TBDR tile overhead.
// TransformEffect.shader -> fx_transform_compute.wgsl
...
fn cs_main(@builtin(global_invocation_id) id: vec3<u32>) {
    ...
    let pixel_uv = (vec2<f32>(id.xy) + 0.5) / vec2<f32>(dims);

    // TransformEffect.shader frag — same math, same order
    var uv = pixel_uv - vec2<f32>(0.5, 0.5);
