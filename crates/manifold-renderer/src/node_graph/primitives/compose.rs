...
crate::primitive! {
    ...
    composition_notes: "Use Lerp for pure crossfades; Add/Screen for additive bloom-style merges; Multiply for darkening masks; Max for tonemap-safe brightening; Overlay for contrast-preserving combines; Divide for per-channel `a/b` (useful for normalising one field by another — e.g. density-driven scaling fields in fluid sims). Divide guards against divide-by-near-zero by returning 0 when `b` is below 1e-6. BUG-217: every non-Lerp mode is RGB-only and passes `a`'s alpha through unchanged (BUG-181's deliberate contract — an AO/mask `b` with alpha=1 can't overwrite a real display chain's alpha). In a feedback-accumulation graph (Max/Add blending a delayed `node.feedback` trail over `a`) this means the blend's alpha never widens to cover the trail RGB it just wrote, so a transparent-background source culls its own trails at display time. The idiom: wire `node.set_alpha` (opaque, e.g. 1.0) onto the source BEFORE it reaches this node's `a`/`b` — proven by the depth-relight look probe. No alpha-mode enum on this node yet; see BUG-217 for the fuller fix under discussion.",
