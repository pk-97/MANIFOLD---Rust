    ...
    fn run(&mut self, ctx: &mut EffectNodeContext<'_, '_>) {
        ...
        let pipeline = self.pipeline.get_or_insert_with(|| {
            gpu.device.create_compute_pipeline(
                include_str!("shaders/sample_texture_at_particles.wgsl"),
                "cs_main",
