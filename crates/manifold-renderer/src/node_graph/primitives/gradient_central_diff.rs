...
"#;
    ...
    /// matches the hand-maintained oracle `gradient_central_diff.wgsl`
    /// (both wrap modes).
    #[test]
    fn generated_matches_hand_kernel() {
        ...
        for wrap_mode in [0u32, 1u32] {
            let uniforms = GradientUniforms { channel: 1, scale_mode: 0, wrap_mode, _pad0: 0 };
            let bytes = bytemuck::bytes_of(&uniforms);

            let hand_wgsl = include_str!("shaders/gradient_central_diff.wgsl");
            let hand_pipeline =
                device.create_compute_pipeline(hand_wgsl, "cs_main", "edge-slope-hand");
            let hand_out =
                RenderTarget::new(&device, w, h, GpuTextureFormat::Rgba16Float, "hand-out");
            dispatch_uniform(
                &device,
                &hand_pipeline,
                &[
            ...
            let gen_wgsl =
                crate::node_graph::freeze::codegen::standalone_for_spec::<GradientCentralDiff>()
                    .expect("node.edge_slope standalone codegen");
            let gen_pipeline = device.create_compute_pipeline(&gen_wgsl, ENTRY, "edge-slope-gen");
            let gen_out =
                RenderTarget::new(&device, w, h, GpuTextureFormat::Rgba16Float, "gen-out");
            dispatch_uniform(
                &device,
                &gen_pipeline,
                &[
            ...
            assert_pixels_close(
                ...
                &gen_pixels,
                1e-4,
                &format!("hand vs generated, wrap_mode={wrap_mode}"),
            );
        }
    }
}
