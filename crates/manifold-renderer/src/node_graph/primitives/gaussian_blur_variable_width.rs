...
crate::primitive! {
    ...
    depth_rule: Inherit,
    composition_notes: "step_size = width_sample × max_radius + 1.0 along the chosen axis. width_sample < 0.005 produces a pass-through (in-focus). For a full 2D blur: dispatch this primitive twice with axis=Horizontal then axis=Vertical, ping-ponging between two Rgba16Float textures. ScatterAsGatherByCoC: a neighbor's weight ramps linearly from 1.0 at neighbor CoC == center CoC down to 0.0 over a 0.1 CoC drop (CoC sampled from the `width` texture's R channel), OR is 1.0 outright when the center is very blurry (CoC > 0.5) — the ramp replaced a binary step that flickered per tap across CoC discontinuities and renormalized the survivors brighter (silhouette-edge speckle); smooth CoC fields are byte-identical to the old gate. The DepthOfField preset runs this at quarter resolution behind a node.downsample 4x box prefilter (max_radius = 4.0 ≈ the old 16px full-res radius) with a node.clamp highlight knee (RGB ≤ 4.0) ahead of the prefilter and a second 4x downsample feeding `width`: every gathered texel and every weight sample is an area average, so sparse bright pixels cannot become dot grids and the weight field cannot alias across edges. BUG-138: at step_size (px) > 8.0 each of the fixed 9/17/25 taps densifies into up to 4 sub-samples that fill the gap back toward the previous tap, so large CoC radii (e.g. 64px) no longer leave visible ring gaps between samples; below that threshold the kernel is bit-identical to the original fixed-tap-per-sample behaviour.",
    examples: [],
