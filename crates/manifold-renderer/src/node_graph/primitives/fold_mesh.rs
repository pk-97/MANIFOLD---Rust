        ...
        let pipeline = self.pipeline.get_or_insert_with(|| {
            ...
            gpu.device.create_compute_pipeline(
                &crate::node_graph::freeze::codegen::standalone_for_spec::<Self>()
                    .expect("node.fold_mesh standalone codegen"),
...
mod tests {
    ...
    #[test]
    fn fold_mesh_declares_ports() {
        use crate::node_graph::ports::{ArrayType, PortType, ScalarType};
        let mesh_layout = ArrayType::of_known::<MeshVertex>();
        let f32_layout = ArrayType::of_known::<f32>();

        assert_eq!(FoldMesh::TYPE_ID, "node.fold_mesh");

        let in_port = FoldMesh::INPUTS.iter().find(|p| p.name == "in").unwrap();
        assert!(in_port.required);
        assert_eq!(in_port.ty, PortType::Array(mesh_layout));

        let weights_port = FoldMesh::INPUTS.iter().find(|p| p.name == "weights").unwrap();
        assert!(!weights_port.required);
        assert_eq!(weights_port.ty, PortType::Array(f32_layout));

        let amount_port = FoldMesh::INPUTS.iter().find(|p| p.name == "amount").unwrap();
        assert!(!amount_port.required);
        assert_eq!(amount_port.ty, PortType::Scalar(ScalarType::F32));

        let axis_param = FoldMesh::PARAMS.iter().find(|p| p.name == "axis").unwrap();
        assert_eq!(axis_param.ty, ParamType::Enum);

        assert_eq!(FoldMesh::OUTPUTS.len(), 1);
        assert_eq!(FoldMesh::OUTPUTS[0].ty, PortType::Array(mesh_layout));
    }

    #[test]
    fn fold_mesh_output_follows_in_input() {
        use crate::node_graph::effect_node::ParamValues;
        let prim = FoldMesh::new();
    ...
    fn primitive_registers_as_palette_atom() {
        ...
        assert_eq!(node.type_id().as_str(), "node.fold_mesh");
    }
}
