...
mod gpu_tests {
    ...
    //! actually dispatches (built via `standalone_for_spec::<DrawTicks>()`)
    //! must reproduce `shaders/draw_ticks.wgsl` (the hand oracle) texel-for-texel.
    use manifold_gpu::{
        GpuBinding, GpuDevice, GpuSamplerDesc, GpuTexture, GpuTextureDesc, GpuTextureDimension,
        GpuTextureFormat, GpuTextureUsage,
    };

    use super::{DrawTicks, TicksUniforms};
    ...
    fn readback_rgba(device: &GpuDevice, tex: &GpuTexture, w: u32, h: u32) -> Vec<[f32; 4]> {
        ...
        let mut enc = device.create_encoder("draw-ticks-readback");
    ...
    #[test]
    fn generated_draw_ticks_matches_hand_kernel() {
        ...
        let alpha = 1.0_f32;
        let right_offset_px = 8.0_f32;
        let long_tick_px = 12.0_f32;
        let short_tick_px = 6.0_f32;
        let thickness_px = 1.5_f32;

        // Hand layout (`shaders/draw_ticks.wgsl`'s `struct U`): color as
        // vec3<f32> + alpha + right/long/short/thickness — NOT the generated
        // Params layout (`TicksUniforms`, PARAMS order: color as 4×f32 then
        // alpha/right/long/short/thickness + 3×u32 pad).
        ...
        hand_bytes.extend_from_slice(&right_offset_px.to_le_bytes());
        hand_bytes.extend_from_slice(&long_tick_px.to_le_bytes());
        hand_bytes.extend_from_slice(&short_tick_px.to_le_bytes());
        hand_bytes.extend_from_slice(&thickness_px.to_le_bytes());

        let gen_uniforms = TicksUniforms {
            color,
            alpha,
            right_offset_px,
            long_tick_px,
        ...
        let hand_wgsl = include_str!("shaders/draw_ticks.wgsl");
        let hand_pipeline = device.create_compute_pipeline(hand_wgsl, "cs_main", "draw-ticks-hand");
        let gen_wgsl = crate::node_graph::freeze::codegen::standalone_for_spec::<DrawTicks>()
            .expect("node.draw_ticks standalone codegen");
        let gen_pipeline = device.create_compute_pipeline(
            &gen_wgsl,
            crate::node_graph::freeze::codegen::ENTRY,
            "draw-ticks-generated",
        );

        let sampler = device.create_sampler(&GpuSamplerDesc::default());

        let hand_out = RenderTarget::new(&device, w, h, GpuTextureFormat::Rgba16Float, "hand-out");
        let mut enc = device.create_encoder("draw-ticks-hand-dispatch");
        enc.dispatch_compute(
            ...
            [w.div_ceil(16), h.div_ceil(16), 1],
            "draw-ticks-hand-dispatch",
        );
        enc.commit_and_wait_completed();

        let gen_out = RenderTarget::new(&device, w, h, GpuTextureFormat::Rgba16Float, "gen-out");
        let mut enc = device.create_encoder("draw-ticks-gen-dispatch");
        enc.dispatch_compute(
            &gen_pipeline,
            &[
