...
//
// `out = near_result.rgb + far_result.rgb * (1 - near_result.a)` — the near
...
fn cs_main(@builtin(global_invocation_id) id: vec3<u32>) {
    ...
    // Near RGB is already coverage-weighted (acc / tap_count). Applying
    // coverage again would darken a flat surface at the near blur cutoff.
    let rgb = near_sample.rgb + far_sample.rgb * (1.0 - clamp(near_sample.a, 0.0, 1.0));
