...
fn body(idx: u32, count: u32, density: i32, radius: f32, source_offset_x: f32, source_offset_y: f32, source_offset_z: f32) -> Element {
    ...
    if z >= d { return Element(vec3<f32>(0.0), vec3<f32>(0.0,1.0,0.0), vec2<f32>(0.0), vec2<f32>(0.0), vec4<f32>(0.0), vec4<f32>(1.0)); }
    ...
    return Element(p, vec3<f32>(0.0,0.0,1.0), vec2<f32>(0.0), vec2<f32>(0.0), vec4<f32>(0.0), vec4<f32>(1.0));
}
