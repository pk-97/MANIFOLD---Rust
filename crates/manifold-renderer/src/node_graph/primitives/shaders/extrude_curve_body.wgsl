...
fn body(idx: u32, count: u32, depth: f32, steps: i32, close: u32, outline_len: u32) -> Element2 {
    ...
    if idx >= total {
        return Element2(vec3<f32>(0.0, 0.0, 0.0), vec3<f32>(0.0, 0.0, 0.0), vec2<f32>(0.0, 0.0), vec2<f32>(0.0, 0.0), vec4<f32>(0.0));
    }
    ...
    return Element2(pos, vec3<f32>(0.0, 0.0, 0.0), uv, vec2<f32>(0.0, 0.0), vec4<f32>(0.0));
}
