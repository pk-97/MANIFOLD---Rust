// node.plasma_pattern_2d — eight curated Plasma pattern variants in one
// primitive. Bit-exact port of the legacy PlasmaGenerator shader: same
// algorithms, same constants, same per-pattern sin-sum math. The
// `pattern` uniform picks which variant; the rest of the params are
// shared (complexity, contrast, speed, scale).
//
//   pattern 0 — Classic:  five fixed-direction sine sums (canonical Plasma)
//   pattern 1 — Rings:    asymmetric concentric radial waves
//   pattern 2 — Diamond:  axis-aligned interference (smooth L1)
//   pattern 3 — Warp:     domain-warped sine field
//   pattern 4 — Cells:    voronoi-like sine interference
//   pattern 5 — Noise:    multi-octave sine noise
//   pattern 6 — Fractal:  self-similar sine stacks with smooth folding
