...
//!
//! Same skeleton as Bloom plus a ColorMatrix tint pass between the blur
...
use crate::node_graph::primitives::{Blend, Blur, ColorMatrix, MipChain, Threshold};
...
///
/// ```text
///  source ──→ Threshold ──→ MipChain ──→ Blur ──→ ColorMatrix ──→ Blend.overlay
///       │                                                              ↑
///       └──────────────────────────────────────────────────────→ Blend.base
/// ```
///
/// The ColorMatrix is pre-loaded with a warm-red tint matrix. Blend
...
pub fn build_halation(
    ...
    let tint = graph.add_node(Box::new(ColorMatrix::new()));
    let blend = graph.add_node(Box::new(Blend::new()));

    // Warm-red tint: dampen green and blue, keep red full. Identity-ish
    // matrix with G and B rows scaled down. (The ColorMatrix default is
