...
/// - normal(12)   + pad(4)
/// - uv(8)        + pad(8)
/// - tangent(16)  — glTF TANGENT, xyz direction + w bitangent sign
///
/// `tangent` follows the glTF TANGENT attribute: xyz is the
/// right-handed direction vector, w is the sign of the bitangent
/// (handedness correction). A zero vector `[0,0,0,0]` means the
/// attribute is absent — the shader uses a derived-frame fallback
/// (Gram-Schmidt orthogonalization from the normal).
#[repr(C)]
