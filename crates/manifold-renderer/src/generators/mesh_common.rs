...
/// - tangent(16)
///
/// The `tangent` field stores the glTF TANGENT attribute for
/// tangent-space normal mapping. A zero tangent (`[0,0,0,0]`) is the
/// sentinel for "not present" — consumers should derive an
/// orthonormal tangent frame from the normal and UVs when needed.
#[repr(C)]
