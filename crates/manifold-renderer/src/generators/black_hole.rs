...
//
// 5 compute passes per frame:
//   [on camera/disk param change only:]
//   1. Deflection Map Bake — trace geodesics, store where each pixel's ray ends up
//   [every frame:]
//   2. Particle Simulate — update 2M particles under Schwarzschild gravity + turbulence
//   3. Particle Scatter — splat particles onto 2D polar disk density texture (atomic)
//   4. Scatter Resolve — atomic accumulator → RGBA density texture + self-clear
...
const _: () = assert!(std::mem::size_of::<DeflectionUniforms>() == 32);
const _: () = assert!(std::mem::size_of::<ParticleSimUniforms>() == 48);
const _: () = assert!(std::mem::size_of::<ScatterUniforms>() == 32);
const _: () = assert!(std::mem::size_of::<ResolveUniforms>() == 16);
...
impl BlackHoleGenerator {
    ...
    fn ensure_particle_resources(&mut self, device: &manifold_gpu::GpuDevice) {
        ...
        let accum_size = (DISK_DENSITY_W as u64) * (DISK_DENSITY_H as u64) * 4;
...
impl Generator for BlackHoleGenerator {
    ...
    fn render(
        ...
        // Update active count (may trigger reseed)
        ...
        gpu.native_enc.dispatch_compute(
            ...
            &[
                ...
                manifold_gpu::GpuBinding::Buffer {
                    binding: 1,
                    buffer: self.scatter_accum.as_ref().unwrap(),
                    offset: 0,
                },
                manifold_gpu::GpuBinding::Bytes {
                    binding: 2,
        ...
        // ── Pass 4: Resolve scatter accumulator → density texture ──
        ...
        gpu.native_enc.dispatch_compute(
            ...
            &[
                ...
                manifold_gpu::GpuBinding::Texture {
                    binding: 1,
                    texture: self.disk_density_tex.as_ref().unwrap(),
                },
                manifold_gpu::GpuBinding::Bytes {
                    binding: 2,
            ...
            [DISK_DENSITY_W.div_ceil(16), DISK_DENSITY_H.div_ceil(16), 1],
            "BlackHole Resolve",
        );

        // ── Pass 5: Display — compose final output ──
    ...
    fn resize(&mut self, device: &manifold_gpu::GpuDevice, width: u32, height: u32) {
        // Deflection map must match render resolution
