...
/// BGL entries for the hal splat pipeline:
///   binding 0: uniform (dynamic offset)
///   binding 1: texture_2d filterable (state read)
///   binding 2: sampler (filtering)
///   binding 3: texture_2d non-filterable (position texture, Rgba32Float)
///   binding 4: storage_texture (state write)
#[cfg(all(target_os = "macos", feature = "hal-encoding"))]
const HAL_BGL_ENTRIES: [wgpu::BindGroupLayoutEntry; 5] = [
    ...
    pub fn new(
        ...
        let (hal_pipeline, hal_sampler) = if let Some(ctx) = hal_ctx {
            ...
            let hal_pipe = crate::hal_pipeline::create_compute_pipeline(
                ctx,
                include_str!("shaders/strange_attractor_splat_compute.wgsl"),
                "cs_main",
                &HAL_BGL_ENTRIES,
                "Attractor Splat HAL",
            );
...
impl Generator for StrangeAttractorGenerator {
    ...
    fn render(
            ...
            && gpu.has_hal_encoder()
        {
            ...
            let pos_ptr = unsafe {
                extract_hal_view(self.position_view.as_ref().unwrap())
            };
            let write_ptr = unsafe { extract_hal_view(state.write_view()) };

            let uniform_size = std::mem::size_of::<AttractorUniforms>() as u64;
            ...
            let bg = unsafe {
                hal_ctx.device().create_bind_group(
                    &hal::BindGroupDescriptor {
                        ...
                        entries: &[
                            ...
                            // binding 1: state read texture → textures[0]
                            hal::BindGroupEntry {
                                binding: 1,
                                resource_index: 0,
                                count: 1,
                            ...
                            // binding 3: position texture → textures[1]
                            hal::BindGroupEntry {
                                binding: 3,
                                resource_index: 1,
                                count: 1,
                            },
                            // binding 4: state write storage → textures[2]
                            hal::BindGroupEntry {
                                binding: 4,
                                resource_index: 2,
                                count: 1,
                ...
                .expect("Failed to create Attractor hal bind group")
            };
