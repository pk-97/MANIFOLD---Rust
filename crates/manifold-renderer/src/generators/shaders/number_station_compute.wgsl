...
fn cs_main(@builtin(global_invocation_id) id: vec3<u32>) {
    ...
    let cell_x = uv.x * f32(num_cols);
    let cell_y = uv.y * f32(actual_rows);
    ...
    // Scanline effect
    let scanline = 0.85 + 0.15 * sin(uv.y * f32(actual_rows) * 3.14159 * 2.0);
    intensity *= scanline;
    ...
    textureStore(output, vec2<i32>(id.xy), vec4<f32>(color, max(color.r, max(color.g, color.b))));
}
