...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let dR = textureSample(t_density, s, uv + vec2<f32>(texel.x, 0.0)).r;
    let dL = textureSample(t_density, s, uv - vec2<f32>(texel.x, 0.0)).r;
    let dU = textureSample(t_density, s, uv + vec2<f32>(0.0, texel.y)).r;
    let dD = textureSample(t_density, s, uv - vec2<f32>(0.0, texel.y)).r;
