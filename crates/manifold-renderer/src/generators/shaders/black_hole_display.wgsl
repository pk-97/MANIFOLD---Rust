...
fn cs_main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    // ── Mipmap-sampled upscale of all deflection data ──
    // The deflection bake's mipmap chain was generated by a hardware
    // blit pass after the bake compute. Sampling at a fractional mip
    // level returns a bilinear-interpolated result between two
    // box-filtered downsamples — far cheaper than an inline gaussian.
    // We sample TWO mip levels and average them: this converts the
    // box filter into a tent filter, hiding the quantization artifacts
    // that pure single-level mipmap sampling produces at boundaries.
    let lod_lo: f32 = 1.5;
    let lod_hi: f32 = 2.5;
    let d1_lo = textureSampleLevel(deflection1, s_linear, uv, lod_lo);
    let d1_hi = textureSampleLevel(deflection1, s_linear, uv, lod_hi);
    let d2_lo = textureSampleLevel(deflection2, s_linear, uv, lod_lo);
    let d2_hi = textureSampleLevel(deflection2, s_linear, uv, lod_hi);
    let sky_lo = textureSampleLevel(sky_dir_tex, s_linear, uv, lod_lo);
    let sky_hi = textureSampleLevel(sky_dir_tex, s_linear, uv, lod_hi);
    let d1 = (d1_lo + d1_hi) * 0.5;
    let d2 = (d2_lo + d2_hi) * 0.5;
    let sky = (sky_lo + sky_hi) * 0.5;
