...
fn cs_main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    // box-filtered downsamples — equivalent in look to a wide
    // gaussian blur, but for ONE bilinear sample per texture instead
    // of 81+ taps per pixel. Tune `mip_lod` to control blur radius:
    // 2.5 ≈ averaging a ~22 source-texel region (similar to the
    // previous 13×13 σ=3 inline blur).
    let mip_lod: f32 = 2.5;
    let d1 = textureSampleLevel(deflection1, s_linear, uv, mip_lod);
    let d2 = textureSampleLevel(deflection2, s_linear, uv, mip_lod);
    let sky = textureSampleLevel(sky_dir_tex, s_linear, uv, mip_lod);
