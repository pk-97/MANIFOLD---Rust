...
fn compute_camera_vectors_euler(rot_x: f32, rot_y: f32, rot_z: f32, cam_dist: f32) -> (
    ...
    let cam_pos = [sin_y * cos_x * cam_dist, -sin_x * cam_dist, cos_y * cos_x * cam_dist];
...
struct Splat3DUniforms {
    active_count: u32, vol_res: u32, vol_depth: u32, scaled_energy: u32,
    _pad: [u32; 24],
}
...
struct Resolve3DUniforms {
    vol_res: u32, vol_depth: u32, _pad0: u32, _pad1: u32,
    _pad: [u32; 24],
}
...
struct Blur3DUniforms {
    vol_res: u32, axis: u32, radius: f32, _pad: u32,
}
...
struct GradientCurl3DUniforms {
    vol_res: u32, vol_depth: u32, _pad0: u32, _pad1: u32,
    curl_strength: f32, slope_strength: f32,
    ref_axis_x: f32, ref_axis_y: f32, ref_axis_z: f32,
...
struct ProjectedUniforms {
    active_count: u32, disp_w: u32, disp_h: u32, ortho: u32,
    scaled_energy: u32, _pad0: u32, _pad1: u32, _pad2: u32,
    ...
    aspect: f32, _pad7: f32, _pad8: f32, _pad9: f32,
}

#[repr(C)]
#[derive(Clone, Copy, bytemuck::Pod, bytemuck::Zeroable)]
struct SeedUniforms {
    active_count: u32, pattern_type: u32, trigger_count: u32, _pad0: u32,
    container: u32, ctr_scale: f32, flatten: f32, _pad1: f32,
    cam_fwd_x: f32, cam_fwd_y: f32, cam_fwd_z: f32, _pad2: f32,
}
...
impl FluidSimulation3DGenerator {
    pub fn new(device: &manifold_gpu::GpuDevice) -> Self {
        let splat_3d_pipeline = device.create_compute_pipeline(
            include_str!("shaders/fluid_scatter_3d.wgsl"), "splat_3d", "FluidSim3D Splat3D",
        );
        let resolve_3d_pipeline = device.create_compute_pipeline(
            include_str!("shaders/fluid_scatter_3d.wgsl"), "resolve_3d", "FluidSim3D Resolve3D",
        );
        let blur_scalar_pipeline = device.create_compute_pipeline(
            include_str!("shaders/fluid_blur_3d.wgsl"), "blur_scalar", "FluidSim3D BlurScalar",
        );
        let blur_vector_pipeline = device.create_compute_pipeline(
            include_str!("shaders/fluid_blur_3d.wgsl"), "blur_vector", "FluidSim3D BlurVector",
        );
        let gradient_curl_pipeline = device.create_compute_pipeline(
            include_str!("shaders/fluid_gradient_curl_3d.wgsl"), "main", "FluidSim3D GradientCurl3D",
        );
        let simulate_pipeline = device.create_compute_pipeline(
            include_str!("shaders/fluid_simulate_3d.wgsl"), "main", "FluidSim3D Simulate3D",
        );
        let seed_pipeline = device.create_compute_pipeline(
            include_str!("shaders/fluid_simulate_3d.wgsl"), "seed_pattern", "FluidSim3D SeedPattern",
        );
        let splat_projected_pipeline = device.create_compute_pipeline(
            include_str!("shaders/fluid_scatter_3d.wgsl"), "splat_projected", "FluidSim3D SplatProjected",
        );
        let resolve_display_pipeline = device.create_compute_pipeline(
            include_str!("shaders/fluid_scatter_3d.wgsl"), "resolve_display", "FluidSim3D ResolveDisplay",
        );
        let display_pipeline = device.create_compute_pipeline(
            include_str!("shaders/fluid_display_compute.wgsl"), "cs_main", "FluidSim3D Display",
        );
        ...
        Self {
            splat_3d_pipeline, resolve_3d_pipeline, blur_scalar_pipeline,
            blur_vector_pipeline, gradient_curl_pipeline, simulate_pipeline,
            seed_pipeline, splat_projected_pipeline, resolve_display_pipeline,
            display_pipeline, sampler, blur_sampler,
            particle_buffer: None, accum_3d: None, display_accum: None,
            density_volume: None, density_blur_temp: None,
            vector_volume: None, vector_blur_temp: None,
    ...
    fn ensure_volume_resources(&mut self, device: &manifold_gpu::GpuDevice, vol_res: u32) {
        if self.accum_3d.is_some() && self.vol_res == vol_res { return; }
        ...
        let make_vol = |fmt, label| device.create_texture(&manifold_gpu::GpuTextureDesc {
            ...
            format: fmt, dimension: manifold_gpu::GpuTextureDimension::D3,
            usage: manifold_gpu::GpuTextureUsage::RENDER_TARGET_FULL, label,
        });
    ...
    fn ensure_display_resources(&mut self, device: &manifold_gpu::GpuDevice, dw: u32, dh: u32) {
        if self.display_accum.is_some() && self.disp_w == dw && self.disp_h == dh { return; }
        self.disp_w = dw;
        self.disp_h = dh;
    ...
    fn dispatch_seed_pattern(
        &self, gpu: &mut GpuEncoder, pattern: u32, trigger_count: u32,
        container: u32, ctr_scale: f32, flatten: f32, cam_fwd: [f32; 3],
    ) {
        if self.particle_buffer.is_none() { return; }
        let uniforms = SeedUniforms {
            active_count: self.active_count, pattern_type: pattern,
            trigger_count, _pad0: 0, container, ctr_scale, flatten, _pad1: 0.0,
            cam_fwd_x: cam_fwd[0], cam_fwd_y: cam_fwd[1], cam_fwd_z: cam_fwd[2], _pad2: 0.0,
        };
        gpu.native_enc.dispatch_compute(
            &self.seed_pipeline,
            &[
                manifold_gpu::GpuBinding::Buffer {
                    binding: 0, buffer: self.particle_buffer.as_ref().unwrap(), offset: 0,
                },
                manifold_gpu::GpuBinding::Bytes { binding: 1, data: bytemuck::bytes_of(&uniforms) },
            ],
    ...
    fn render(
        ...
        if self.snap_envelope > 0.0 {
            match self.active_snap_mode {
                1 => { curl_angle += 180.0 * self.snap_envelope; }
                2 => { cur_flow += (-cur_flow - cur_flow) * self.snap_envelope; }
                _ => {}
            }
        }
        ...
        if update_volume {
            ...
            let splat_uni = Splat3DUniforms {
                ...
                scaled_energy: scaled_energy_3d, _pad: [0; 24],
            };
            gpu.native_enc.dispatch_compute(&self.splat_3d_pipeline, &[
                manifold_gpu::GpuBinding::Buffer { binding: 0, buffer: particle_buf, offset: 0 },
                manifold_gpu::GpuBinding::Buffer { binding: 1, buffer: accum_3d, offset: 0 },
                manifold_gpu::GpuBinding::Bytes { binding: 2, data: bytemuck::bytes_of(&splat_uni) },
            ], [active_count.div_ceil(THREAD_GROUP_SIZE), 1, 1], "FluidSim3D Splat3D");

            // Resolve 3D
            let resolve_uni = Resolve3DUniforms {
                vol_res, vol_depth: vol_res, _pad0: 0, _pad1: 0, _pad: [0; 24],
            };
            gpu.native_enc.dispatch_compute(&self.resolve_3d_pipeline, &[
                manifold_gpu::GpuBinding::Buffer { binding: 0, buffer: accum_3d, offset: 0 },
                manifold_gpu::GpuBinding::Texture { binding: 1, texture: density_vol },
                manifold_gpu::GpuBinding::Bytes { binding: 2, data: bytemuck::bytes_of(&resolve_uni) },
            ], [wg3d, wg3d, wg3d], "FluidSim3D Resolve3D");

            // Blur Density (X, Y, Z)
            let wg_blur = vol_res.div_ceil(4);
            let blur_x = Blur3DUniforms { vol_res, axis: 0, radius: scaled_radius as f32, _pad: 0 };
            let blur_y = Blur3DUniforms { vol_res, axis: 1, radius: scaled_radius as f32, _pad: 0 };

            gpu.native_enc.dispatch_compute(&self.blur_scalar_pipeline, &[
                manifold_gpu::GpuBinding::Bytes { binding: 0, data: bytemuck::bytes_of(&blur_x) },
                manifold_gpu::GpuBinding::Texture { binding: 1, texture: density_vol },
                manifold_gpu::GpuBinding::Sampler { binding: 2, sampler: &self.blur_sampler },
                manifold_gpu::GpuBinding::Texture { binding: 3, texture: density_temp },
            ], [wg_blur, wg_blur, wg_blur], "FluidSim3D BlurScalar X");

            gpu.native_enc.dispatch_compute(&self.blur_scalar_pipeline, &[
                manifold_gpu::GpuBinding::Bytes { binding: 0, data: bytemuck::bytes_of(&blur_y) },
                manifold_gpu::GpuBinding::Texture { binding: 1, texture: density_temp },
                manifold_gpu::GpuBinding::Sampler { binding: 2, sampler: &self.blur_sampler },
                manifold_gpu::GpuBinding::Texture { binding: 3, texture: density_vol },
            ], [wg_blur, wg_blur, wg_blur], "FluidSim3D BlurScalar Y");

            if vol_res >= 8 {
                let blur_z = Blur3DUniforms { vol_res, axis: 2, radius: scaled_radius as f32, _pad: 0 };
                gpu.native_enc.dispatch_compute(&self.blur_scalar_pipeline, &[
                    manifold_gpu::GpuBinding::Bytes { binding: 0, data: bytemuck::bytes_of(&blur_z) },
                    manifold_gpu::GpuBinding::Texture { binding: 1, texture: density_vol },
                    manifold_gpu::GpuBinding::Sampler { binding: 2, sampler: &self.blur_sampler },
                    manifold_gpu::GpuBinding::Texture { binding: 3, texture: density_temp },
                ], [wg_blur, wg_blur, wg_blur], "FluidSim3D BlurScalar Z");
            }

            // GradientCurl
            let blurred_density = if vol_res >= 8 { density_temp } else { density_vol };
            let gc_uni = GradientCurl3DUniforms {
                vol_res, vol_depth: vol_res, _pad0: 0, _pad1: 0,
                curl_strength, slope_strength,
                ref_axis_x: ref_axis[0], ref_axis_y: ref_axis[1], ref_axis_z: ref_axis[2],
                _pad2: 0.0, _pad3: 0.0, _pad4: 0.0,
            };
            gpu.native_enc.dispatch_compute(&self.gradient_curl_pipeline, &[
                manifold_gpu::GpuBinding::Bytes { binding: 0, data: bytemuck::bytes_of(&gc_uni) },
                manifold_gpu::GpuBinding::Texture { binding: 1, texture: blurred_density },
                manifold_gpu::GpuBinding::Texture { binding: 2, texture: vector_vol },
            ], [wg3d, wg3d, wg3d], "FluidSim3D GradientCurl3D");

            // Blur Vector Field (X, Y, Z)
            gpu.native_enc.dispatch_compute(&self.blur_vector_pipeline, &[
                manifold_gpu::GpuBinding::Bytes { binding: 0, data: bytemuck::bytes_of(&blur_x) },
                manifold_gpu::GpuBinding::Texture { binding: 1, texture: vector_vol },
                manifold_gpu::GpuBinding::Sampler { binding: 2, sampler: &self.blur_sampler },
                manifold_gpu::GpuBinding::Texture { binding: 3, texture: vector_temp },
            ], [wg_blur, wg_blur, wg_blur], "FluidSim3D BlurVector X");

            gpu.native_enc.dispatch_compute(&self.blur_vector_pipeline, &[
                manifold_gpu::GpuBinding::Bytes { binding: 0, data: bytemuck::bytes_of(&blur_y) },
                manifold_gpu::GpuBinding::Texture { binding: 1, texture: vector_temp },
                manifold_gpu::GpuBinding::Sampler { binding: 2, sampler: &self.blur_sampler },
                manifold_gpu::GpuBinding::Texture { binding: 3, texture: vector_vol },
            ], [wg_blur, wg_blur, wg_blur], "FluidSim3D BlurVector Y");
            ...
            if vol_res >= 8 {
                ...
                gpu.native_enc.dispatch_compute(&self.blur_vector_pipeline, &[
                    ...
                    manifold_gpu::GpuBinding::Sampler { binding: 2, sampler: &self.blur_sampler },
                    manifold_gpu::GpuBinding::Texture { binding: 3, texture: vector_temp },
                ], [wg_blur, wg_blur, wg_blur], "FluidSim3D BlurVector Z");
            }
        }

        // STEP 5: Simulate
        let vector_field = if vol_res >= 8 { vector_temp } else { vector_vol };
        let blurred_density = if vol_res >= 8 { density_temp } else { density_vol };
        ...
        gpu.native_enc.dispatch_compute(&self.simulate_pipeline, &[
            manifold_gpu::GpuBinding::Buffer { binding: 0, buffer: particle_buf, offset: 0 },
            manifold_gpu::GpuBinding::Texture { binding: 1, texture: vector_field },
            manifold_gpu::GpuBinding::Sampler { binding: 2, sampler: &self.sampler },
            manifold_gpu::GpuBinding::Texture { binding: 3, texture: blurred_density },
            manifold_gpu::GpuBinding::Bytes { binding: 4, data: bytemuck::bytes_of(&sim_uni) },
        ], [active_count.div_ceil(THREAD_GROUP_SIZE), 1, 1], "FluidSim3D Simulate");
        ...
        let proj_uni = ProjectedUniforms {
            active_count, disp_w: dw, disp_h: dh, ortho,
            scaled_energy: scaled_energy_proj, _pad0: 0, _pad1: 0, _pad2: 0,
            ...
            aspect: ctx.aspect, _pad7: 0.0, _pad8: 0.0, _pad9: 0.0,
        };
        gpu.native_enc.dispatch_compute(&self.splat_projected_pipeline, &[
            manifold_gpu::GpuBinding::Buffer { binding: 0, buffer: particle_buf, offset: 0 },
            manifold_gpu::GpuBinding::Buffer { binding: 1, buffer: display_accum, offset: 0 },
            manifold_gpu::GpuBinding::Bytes { binding: 2, data: bytemuck::bytes_of(&proj_uni) },
        ], [active_count.div_ceil(THREAD_GROUP_SIZE), 1, 1], "FluidSim3D SplatProjected");

        // Resolve Display
        let resolve_disp_uni = Resolve3DUniforms {
            vol_res: dw, vol_depth: dh, _pad0: 0, _pad1: 0, _pad: [0; 24],
        };
        gpu.native_enc.dispatch_compute(&self.resolve_display_pipeline, &[
            manifold_gpu::GpuBinding::Buffer { binding: 0, buffer: display_accum, offset: 0 },
            manifold_gpu::GpuBinding::Texture { binding: 1, texture: display_density },
            manifold_gpu::GpuBinding::Bytes { binding: 2, data: bytemuck::bytes_of(&resolve_disp_uni) },
        ], [dw.div_ceil(16), dh.div_ceil(16), 1], "FluidSim3D ResolveDisplay");
        ...
        gpu.native_enc.dispatch_compute(&self.display_pipeline, &[
            ...
            manifold_gpu::GpuBinding::Texture { binding: 1, texture: display_density },
            manifold_gpu::GpuBinding::Sampler { binding: 2, sampler: &self.sampler },
            manifold_gpu::GpuBinding::Texture { binding: 3, texture: target },
        ], [ctx.width.div_ceil(16), ctx.height.div_ceil(16), 1], "FluidSim3D Display");
