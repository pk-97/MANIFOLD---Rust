...
Rationale and incident history live in `.claude/GIT_TREE_DISCIPLINE.md`, the pointed-to docs, and git history — not here.
...
- **Debug escalation ladder (hook-enforced by `probe-loop-guard.py`).** Wrong and not obvious: (1) lead semantic review of the seam first; (2) still stuck → adversarial consult from the K3 consult seat, read-and-discuss only, hard-budgeted (repo read access mandatory; one-shot API calls fabricate citations on review tasks — `.claude/hooks/oneshot` is for mechanical non-reasoning tasks only); (3) instrumented probe loops last, always delegated to lanes, never lead-run. The hook warns at 3 lead probe actions and denies at 6 until `/tmp/manifold_seam_review.md` exists. Doctrine: `docs/AGENT_ROUTING.md`.
...
- **Agent worktrees come from the slot ring only — hook-enforced.** `scripts/agent-worktree.py acquire <task-label> <branch> [--tip REF]`, one per workstream. `POOL FULL` is a loud stop. Raw `git worktree add` and Agent-tool worktree isolation are denied. Release the slot at session end; open every worktree brief by verifying the base tip. Agent edits in the main checkout are denied except unmerged files during a landing merge and `docs/**/*.md` (new or renamed docs need `gen_docs_index.py` in the same commit). Enforcement source of truth: `worktree-guard.py`.
