...
- **Shell — read-only pipes/compounds are fine; write-capable ones aren't.** The `PreToolUse` hook ([.claude/hooks/preToolUseBash.py](.claude/hooks/preToolUseBash.py)) parses each Bash call and auto-allows it — no prompt — when *every* command-position is a known read-only tool (rg/grep/fd/git-read/cargo-read/jq/sort/etc.), including across pipes, `;`-chains, `for`/`while` loops, and `$()` substitutions. So `rg ... | sort -u`, `git log | cat`, and `for f in $(fd -e rs); do rg x "$f"; done` now run cleanly — use them freely. The static `permissions.allow` matcher can't see compounds (it only matches a call that *starts* with an allowlisted token); the hook is what covers them. What still gets denied with a rewrite nudge: a pipe whose chain includes a **non**-read-only step (`rg foo | xargs rm`, `cargo build | tee log`) — split the write into its own call or use a native form. Output redirects are allowed only to `/tmp/*` or `/dev/null`; a redirect to a repo path falls through to a normal approval prompt.
