...
- **Clippy before every commit.** Worktree: `cargo clippy -p <touched> -- -D warnings`. Landing: full `cargo clippy --workspace --tests -- -D warnings` + `cargo deny check bans` + `scripts/feature_matrix.py` (lints every non-default feature — the workspace sweep never builds them). Never blanket `cargo fmt` (the repo is not rustfmt-clean).
