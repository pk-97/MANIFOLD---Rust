...
- **Shared checkout — commit with a pathspec, never via the index.** In the main checkout use `git commit -m '…' -- <paths>`; never `git add <paths> && git commit`, and never bare `git commit`. The index is shared across live sessions, so an add-then-commit sweeps whatever another session has staged (bit 2026-07-04: 705 lines of another session's staged gltf work landed inside a docs commit and briefly reached main). Worktrees have their own index; this rule is about the main tree.
