...
- **Clippy before every commit.** Worktree: `cargo clippy -p <touched> -- -D warnings`. Landing: `scripts/landing_gate.py` — one command, touched-crate-only (clippy + nextest for touched crates, deny bans, flow gate, docs/design status, GPU proofs when GPU paths are touched). The workspace sweep + `scripts/feature_matrix.py` are NOT landing items — they run nightly on main via `scripts/trunk_health.py`, which files beads on red. Never blanket `cargo fmt` (the repo is not rustfmt-clean).
- Graph JSON authoring: pre-flight `validate --kind effect|generator` and `fusion` (`docs/GRAPH_TOOLING_DESIGN.md`). The bin is **`graph-tool`, hyphenated** (`cargo run -p manifold-renderer --bin graph-tool -- validate <file.json> --kind generator`) — the source file is `graph_tool.rs` but Cargo renames the target, so the underscore form is not runnable.
...
**Routing policy: [docs/AGENT_ROUTING.md](docs/AGENT_ROUTING.md) is authoritative.** A judgment lead (K3 or Fable as top session — the only seat that lands) driving two lane tiers. Current roster (Peter 2026-09-03): all non-lead seats (`haiku`, `sonnet`, `opus`) = Kimi K2.7 (`kimi-for-coding`) — the opencode Go quota ran out and only the Kimi plan has headroom, so lanes and the classifier all ride the Kimi subscription. Lane label for every slot is now `k27-*`. GLM holds no seat (ZAI sub lapsed 2026-08-24). The consult seat is a K3 fork, read-and-discuss only, hard-budgeted. The dispatcher seat is retired — the clerical loop belongs to workflow scripts, exit-code gates, and hooks. Lanes make one commit then stop for review; lanes never land; review throughput caps parallelism. All agents obey every rule in this file.
