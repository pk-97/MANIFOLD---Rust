...
| `manifold-renderer` | Compositor, ~185 graph primitives, 26 JSON effect presets + 20 JSON generator presets. Every effect and generator is a JSON-defined atom graph. The legacy fused effect-monolith bundles (Bloom, Kaleidoscope, Chromatic Aberration, Color Grade, Strobe, Voronoi Prism, Plasma, Quad Mirror, Infrared, Highlight Boost, Edge Stretch, Mirror/Transform) and the `mip_chain`/`blend` no-op stubs were deleted in the 2026-05-30 decomposition-completion pass. **Remaining decomposition targets:** one effect — Wireframe Depth — still ships a legacy `PostProcessEffect` Rust impl wrapped by its `node.*` primitive (48-node `WireframeDepthGraph.json` decomp in flight); and three generators still wire fused/closed bundles — DigitalPlants (`cylinder_wrap_field`, `torus_wrap_field`, `digital_plants_render`), NestedCubes (`nested_cubes_geometry`), Tesseract (`generate_tesseract_vertices`). DNN/FFI/CPU work stays as single-purpose primitives within the graph, never bundled into one kernel. See `docs/NODE_CATALOG.md` and `docs/PRIMITIVE_AUDIT_AND_DECOMPOSITION_PLAN.md`. |
