...
- **Git — `main` is the last-known-good pointer, never a dev branch.** Work happens on `wave/`/`lane/`/`feat/` branches. When a workstream lands verified (clippy green, focused tests pass, pushed), fast-forward main without touching any checkout: `git branch -f main <tip> && git push origin <tip>:main`. Everything that bases off the default branch — fresh clones, GitHub's repo view, agent worktree isolation — reads main; letting it lag hands agents months-old code (the 2026-07-04 stale-worktree incident).
