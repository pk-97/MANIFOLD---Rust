...
- **Memory is rules, not history — hook-enforced (`memory-history-guard.py`).** No commit hashes, no LANDED/SHIPPED/CLOSED stamps in memory files; status goes to `bd`/the board, history stays in git (the memory dir is its own git repo). Closed handoffs and shipped-work stories are deleted, not archived. Index lines = name + hook only.
