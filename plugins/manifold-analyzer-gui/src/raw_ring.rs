...
//!
//! The audio thread produces Mid spectrum frames at the FFT hop rate
//! (~117 Hz at 8192 / 95 % / 48 kHz); the GUI thread drains them at render
//! rate (~60 Hz). A single-slot mailbox drops every other frame. This ring
//! preserves all frames up to its capacity, so the spectrogram can place
//! one column per FFT hop — matching Vision 4X's temporal density.
//!
//! Safety: this is a classic Lamport-style SPSC queue. One producer, one
//! consumer, bounded. If the reader falls behind by `capacity` frames the
//! producer returns `false` and the frame is dropped (audio thread never
//! blocks).
...
pub struct RawFrameRing {
    // Pre-allocated, never re-grown. Each slot is `num_bins` f32s.
...
impl RawFrameRing {
    pub fn new(capacity: usize, num_bins: usize, fill_value: f32) -> Self {
        assert!(capacity >= 2, "SPSC ring needs at least 2 slots");
        let slots = (0..capacity)
            .map(|_| UnsafeCell::new(vec![fill_value; num_bins]))
            .collect();
        Self {
            slots,
            capacity,
    ...
    /// Producer: copy `frame` into the next slot. Returns `false` if the
    /// ring is full (one slot is always kept empty to distinguish full
    /// from empty).
    pub fn push(&self, frame: &[f32]) -> bool {
        ...
        unsafe {
            ...
            debug_assert_eq!(slot.len(), frame.len());
    ...
    pub fn drain<F: FnMut(&[f32])>(&self, mut f: F) {
