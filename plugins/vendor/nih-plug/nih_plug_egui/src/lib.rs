...
pub struct EguiState {
    ...
    /// The new size the host has asked us to adopt (Host→Plugin resize). The
    /// update loop picks this up and resizes egui's internal window without
    /// going back through `context.request_resize()` (which the host already did).
    #[serde(skip)]
    pub(crate) host_resize: AtomicCell<Option<(u32, u32)>>,
