    ...
    T: 'static + Send + Sync,
{
    fn spawn(
        ...
        let window = EguiWindow::open_parented(
            ...
            move |egui_ctx, queue, state| {
                ...
                // Plugin → Host resize (e.g. from `ResizableWindow` corner drag).
    ...
    fn on_size(&self, width: u32, height: u32) -> bool {
        self.egui_state.host_resize.store(Some((width, height)));
        // Also store here so `size()` reports the new size immediately even
        // before the update loop picks up `host_resize`.
