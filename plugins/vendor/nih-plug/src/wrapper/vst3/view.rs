...
impl<P: Vst3Plugin> IPlugView for WrapperView<P> {
    ...
    unsafe fn on_size(&self, new_size: *mut ViewRect) -> tresult {
        ...
        let unscaled_w = (width.max(0) as f32 / scaling_factor).round() as u32;
        let unscaled_h = (height.max(0) as f32 / scaling_factor).round() as u32;
        if editor.on_size(unscaled_w, unscaled_h) {
            kResultOk
        } else {
            kResultFalse
        }
    ...
    unsafe fn check_size_constraint(&self, rect: *mut ViewRect) -> tresult {
        ...
        let scaling_factor = self.scaling_factor.load(Ordering::Relaxed);
        let unscaled_w = (width as f32 / scaling_factor).round() as u32;
        let unscaled_h = (height as f32 / scaling_factor).round() as u32;
        let (clamped_w, clamped_h) = editor.clamp_size(unscaled_w, unscaled_h);
        r.left = 0;
        r.top = 0;
        r.right = (clamped_w as f32 * scaling_factor).round() as i32;
        r.bottom = (clamped_h as f32 * scaling_factor).round() as i32;
        kResultOk
    }
}
