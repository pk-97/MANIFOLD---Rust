...
pub trait Editor: Send {
    ...
    /// Whether the host is allowed to resize the plugin window. Returns `false`
    /// by default to preserve existing non-resizable behavior for plugins that
    /// don't opt in. Return `true` to expose native host-driven resize.
    fn can_resize(&self) -> bool {
        false
    }

    /// Given a host-proposed size (logical pixels, pre-scaling), return the
    /// closest valid size the editor can accept. Used by VST3's
    /// `checkSizeConstraint` so the host can snap to valid sizes during drag.
    /// Default: pass through unchanged.
    fn clamp_size(&self, width: u32, height: u32) -> (u32, u32) {
        (width, height)
    }

    /// Called when the host has resized the plugin view to the given size
    /// (logical pixels, pre-scaling). The editor should update its internal
    /// size state and trigger a repaint. Returns `true` if the new size was
    /// accepted. Default: reject the resize.
    fn on_size(&self, width: u32, height: u32) -> bool {
        let _ = (width, height);
        false
    }
}
